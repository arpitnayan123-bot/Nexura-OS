...
export const INTERVENTION_CATALOG: CounterfactualIntervention[] = [
  {
    id: "metformin_up",
    label: "Increase Metformin dose",
    kind: "medication",
    effects: { hba1cDelta: -0.5, weightDeltaPct: -1, egfrProtect: 0.06 },
    horizonDays: 90,
  },
  {
    id: "add_insulin",
    ...
    kind: "medication",
    effects: { hba1cDelta: -1.1, weightDeltaPct: +2, adherenceAdd: -0.02 },
    horizonDays: 90,
  },
  {
    id: "sglt2",
    label: "Add SGLT2 inhibitor",
    kind: "medication",
    effects: { hba1cDelta: -0.7, weightDeltaPct: -2.5, egfrProtect: 0.18 },
    horizonDays: 90,
  },
  {
    id: "ace_inhibitor",
    label: "Start ACE inhibitor",
    kind: "medication",
    effects: { sbpDelta: -8, egfrProtect: 0.14 },
    horizonDays: 90,
  },
  {
    id: "beta_blocker",
    label: "Start Beta-blocker",
    kind: "medication",
    effects: { sbpDelta: -6, egfrProtect: 0.05 },
    horizonDays: 90,
  },
  {
    id: "walk_30",
    label: "Walk 30 minutes daily",
    kind: "lifestyle",
    effects: { hba1cDelta: -0.4, sbpDelta: -4, weightDeltaPct: -2, adherenceAdd: 0.08 },
    horizonDays: 90,
  },
  {
    id: "quit_smoking",
    label: "Quit smoking",
    kind: "lifestyle",
    effects: { sbpDelta: -3, egfrProtect: 0.12, adherenceAdd: 0.03 },
    horizonDays: 90,
  },
  {
    id: "med_reminders",
    ...
    kind: "lifestyle",
    effects: { adherenceAdd: 0.14, hba1cDelta: -0.2 },
    horizonDays: 90,
  },
];
...
export function simulateIntervention(
  ...
  if (e.hba1cDelta && projected.hba1c !== null)
    projected.hba1c = Math.max(4, projected.hba1c + e.hba1cDelta);
  if (e.hba1cDelta && projected.hba1c === null) projected.hba1c = Math.max(4, 7.2 + e.hba1cDelta);
  if (e.sbpDelta && projected.sbp !== null)
    projected.sbp = Math.max(80, projected.sbp + e.sbpDelta);
  if (e.weightDeltaPct && projected.weightKg !== null)
    projected.weightKg = Number((projected.weightKg * (1 + e.weightDeltaPct / 100)).toFixed(2));
  if (e.adherenceAdd)
    projected.adherenceScore = Math.min(1, Math.max(0, projected.adherenceScore + e.adherenceAdd));
  ...
  if (e.hba1cDelta) {
    lines.push({
      label: "HbA1c",
      delta: `${fmt(e.hba1cDelta)}%`,
      direction: e.hba1cDelta < 0 ? "better" : "worse",
    });
  }
  if (e.sbpDelta) {
    lines.push({
      label: "Systolic BP",
      delta: `${fmt(e.sbpDelta)} mmHg`,
      direction: e.sbpDelta < 0 ? "better" : "worse",
    });
  }
  if (e.weightDeltaPct) {
    lines.push({
      label: "Body weight",
      delta: `${fmt(e.weightDeltaPct)}%`,
      direction: e.weightDeltaPct < 0 ? "better" : "worse",
    });
  }
  if (e.egfrProtect) {
    const pct = Math.round(e.egfrProtect * 100);
    lines.push({
      label: "Kidney filtration risk",
      delta: `${riskDelta <= 0 ? "-" : "+"}${pct}% relative protection`,
      direction: "better",
    });
  }
  if (e.adherenceAdd) {
    lines.push({
      label: "Adherence",
      delta: `${fmt(e.adherenceAdd * 100)}%`,
      direction: e.adherenceAdd > 0 ? "better" : "worse",
    });
  }
  lines.push({
  ...
  if (
    e.hba1cDelta &&
    state.creatinine !== null &&
    state.creatinine > 1.4 &&
    intervention.id === "metformin_up"
  ) {
...
export function simulateMany(state: TwinStateVector, ids: string[]): CounterfactualOutcome[] {
  return INTERVENTION_CATALOG.filter((i) => ids.includes(i.id)).map((i) =>
    simulateIntervention(state, i),
  );
}
