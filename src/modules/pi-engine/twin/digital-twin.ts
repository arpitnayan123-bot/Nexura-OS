...
export function computeBaseline(state: TwinStateVector, heightCm = 165): TwinBaseline {
  ...
  const bmi =
    state.weightKg !== null ? Number((state.weightKg / (heightCm / 100) ** 2).toFixed(1)) : null;
  return {
    map,
    cardiacOutput: co,
...
export function decayOneDay(
  ...
  if (state.hba1c !== null)
    next.hba1c = Number((state.hba1c + drift * 0.01 * (opts?.metabolicMult ?? 1)).toFixed(3));
  if (state.sbp !== null)
    next.sbp = Number((state.sbp + drift * 0.12 * (opts?.metabolicMult ?? 1)).toFixed(2));
  if (state.creatinine !== null)
    next.creatinine = Number(
      (state.creatinine + drift * 0.002 * (opts?.metabolicMult ?? 1)).toFixed(4),
    );
