...
export interface GraphNode {
  id: string;
  label: "Patient" | "Condition" | "Gene" | "Drug" | "LifestyleFactor" | "Environment" | "Observation";
...
export interface GraphEdge {
  type:
    | "HIGH_RISK_OF" | "INTERACTS_WITH" | "HAS_CONDITION" | "TAKES_DRUG"
    | "EXPOSED_TO" | "SIMILAR_TO" | "IMPROVES" | "WORSENS" | "CARRIES_GENE";
...
export interface GraphRepo {
  ...
  neighbors(key: string, opts?: { edgeTypes?: GraphEdge["type"][]; minWeight?: number }): Promise<{ edge: GraphEdge; node: GraphNode }[]>;
...
export async function syncPatientToGraph(
  ...
  for (const cond of patient.conditions) {
    ...
    await repo.upsertNode({ id: cKey, label: "Condition", key: `icd:${cond}`, props: { icd: cond } });
  ...
  if (patient.environment) {
    ...
    await repo.upsertNode({
      id: eKey, label: "Environment", key: `region:${patient.environment.regionKey}`,
      props: { aqi: patient.environment.aqi },
    });
    await repo.upsertEdge({ type: "EXPOSED_TO", fromKey: pKey, toKey: eKey, weight: patient.environment.aqi ? Math.min(1.5, patient.environment.aqi / 100) : 0.5 });
    nodes += 1; edges += 1;
  }
...
export const DRUG_GENE_INTERACTIONS: { drug: string; target: string; severity: number; note: string }[] = [
  { drug: "clopidogrel", target: "cyp2c19_lof", severity: 0.9, note: "loss-of-function allele → reduced antiplatelet effect" },
  { drug: "warfarin", target: "vkorc1_variant", severity: 0.8, note: "dose sensitivity variant" },
  { drug: "metformin", target: "oct1_variant", severity: 0.4, note: "reduced uptake → attenuated response" },
  { drug: "simvastatin", target: "slco1b1_variant", severity: 0.85, note: "myopathy risk allele" },
  { drug: "codeine", target: "cyp2d6_um", severity: 0.7, note: "ultrarapid metabolizer → toxicity risk" },
];
...
export const CONDITION_RISKS: { from: string; to: string; weight: number }[] = [
  ...
  { from: "icd:I50", to: "icd:A41", weight: 0.30 }, // HF → sepsis susceptibility
...
export function similarPatients(
  ...
  for (const c of cohort) {
    ...
    if (jaccard >= minOverlap) out.push({ patientKey: c.patientKey, overlap: Number(jaccard.toFixed(3)), shared });
  }
