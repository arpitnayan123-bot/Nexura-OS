...
export type AdherenceKind =
  | "med_logged" | "med_missed" | "appointment_kept" | "appointment_missed"
  | "app_login" | "plan_task_done" | "plan_task_skipped";

/** Positive events → 1, negative → 0, neutral engagement → 0.6. */
const KIND_SCORES: Record<AdherenceKind, number> = {
  med_logged: 1, med_missed: 0,
  appointment_kept: 1, appointment_missed: 0,
  app_login: 0.6, plan_task_done: 1, plan_task_skipped: 0,
};
...
export function adherenceScore(events: AdherenceInput[], now = new Date()): { score: number; n: number } {
...
export function engagementCollapse(events: AdherenceInput[], now = new Date()): { collapsed: boolean; daysSilent: number } {
  const t = now.getTime();
  const lastAny = events.length
