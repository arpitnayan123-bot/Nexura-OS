...
export interface Explanation {
  headline: string;
  summary: string;
  topFactors: { rank: number; feature: string; contribution: number; direction: "up" | "down"; detail: string }[];
...
export function explain(a: RiskAssessment, patientLabel: string): Explanation {
  ...
  const top = [...a.drivers].sort((x, y) => Math.abs(y.contribution) - Math.abs(x.contribution)).slice(0, 3);
  const headline =
    a.band === "red" ? `Critical risk for ${patientLabel}`
    : a.band === "yellow" ? `Watchlist risk for ${patientLabel}`
  ...
  return {
    headline,
    summary,
    topFactors: top.map((d, i) => ({ rank: i + 1, feature: d.feature, contribution: d.contribution, direction: d.direction, detail: d.detail })),
...
export function attributionBars(drivers: RiskDriver[]): { feature: string; normalized: number }[] {
  ...
  return drivers.map((d) => ({ feature: d.feature, normalized: Number((d.contribution / maxAbs).toFixed(3)) }));
}
