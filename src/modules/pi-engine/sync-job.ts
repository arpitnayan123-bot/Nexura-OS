...
export async function seedKnowledgeGraph(repo: GraphRepo = prismaGraphRepo): Promise<void> {
  for (const i of DRUG_GENE_INTERACTIONS) {
    await repo.upsertNode({
      id: nk("Drug", `drug:${i.drug}`),
      label: "Drug",
      key: `drug:${i.drug}`,
    });
    await repo.upsertNode({
      id: nk("Gene", `gene:${i.target}`),
      label: "Gene",
      key: `gene:${i.target}`,
      props: { note: i.note },
    });
    await repo.upsertEdge({
      type: "INTERACTS_WITH",
      fromKey: nk("Drug", `drug:${i.drug}`),
      toKey: nk("Gene", `gene:${i.target}`),
      weight: i.severity,
      props: { note: i.note },
    });
  }
  for (const r of CONDITION_RISKS) {
    await repo.upsertNode({
      id: nk("Condition", r.from.replace(/^Condition:/, "")),
      label: "Condition",
      key: r.from.replace(/^Condition:/, ""),
    });
    await repo.upsertNode({
      id: nk("Condition", r.to.replace(/^Condition:/, "")),
      label: "Condition",
      key: r.to.replace(/^Condition:/, ""),
    });
...
export async function syncGraphFromPrisma(
  repo: GraphRepo = prismaGraphRepo,
): Promise<{ patients: number; nodes: number; edges: number }> {
  ...
  for (const p of patients) {
    ...
    const drugs = conditions.includes("E11")
      ? ["metformin"]
      : conditions.includes("I10")
        ? ["amlodipine"]
        : [];
...
export function startGraphSyncJob(): void {
  const g = globalThis as unknown as {
    __pieGraphSyncTimer?: ReturnType<typeof setInterval>;
    __pieGraphSyncBooted?: boolean;
  };
  ...
  const tick = async () => {
    try {
      ...
      console.log(
        `[pie] graph sync: ${res.patients} patients, ${res.nodes} nodes, ${res.edges} edges`,
      );
    } catch (err) {
