...
export const PROTOCOL_KB: Record<string, PreEmptiveProtocolSpec> = {
  sepsis_bundle: {
    ...
    primaryDriver:
      "Trend signature: rising HR & WBC with temperature instability before qSOFA criteria",
    immediate: [
      {
        action: "Draw lactate + blood cultures ×2 before any antibiotics",
        window: "within 1h",
        role: "nurse",
      },
      {
        action: "Broad-spectrum antibiotics per survivorship pathway",
        window: "within 1–3h",
        role: "doctor",
      },
      {
        action: "Begin 30 mL/kg balanced crystalloid if hypotensive",
        window: "within 3h",
        role: "nurse",
      },
    ],
    ...
    evidence:
      "Hour-1 sepsis bundle adherence is associated with substantially lower in-hospital mortality in cohort data.",
  },
  hf_exacerbation: {
    ...
    primaryDriver:
      "Fluid retention detected via weight gain trend + decreasing urine output + rising NT-proBNP",
    immediate: [
      {
        action: "Administer IV Lasix 40mg (per standing decongestion order)",
        window: "immediate",
        role: "nurse",
      },
    ...
    evidence:
      "In 50 similar prior cases this protocol reduced ICU admissions by ~80% (retrospective cohort).",
  },
  dka_risk: {
    code: "dka_risk",
    ...
    primaryDriver:
      "Glycemic runaway: rising glucose + missed medication doses + intercurrent infection",
    immediate: [
    ...
    evidence:
      "Early ketone surveillance with sick-day protocol cuts DKA admissions in high-risk T2DM cohorts.",
  },
  htn_urgency: {
    code: "htn_urgency",
    ...
    immediate: [
      ...
      {
        action: "Oral agent adjustment per protocol (no rapid IV lowering)",
        window: "within 1h",
        role: "doctor",
      },
    ],
    ...
    evidence:
      "Gradual correction avoids cerebral hypoperfusion; structured follow-up reduces 30-day returns.",
  },
  hypoglycemia_watch: {
    ...
    monitoring: [{ action: "3 AM capillary glucose", window: "tonight", role: "nurse" }],
    dispo: "No change; continue CGM trends",
    evidence:
      "Structured nocturnal monitoring prevents severe hypoglycemia episodes in insulin-treated patients.",
  },
  aki_watch: {
    code: "aki_watch",
    ...
    immediate: [
      {
        action: "Hold nephrotoxins (NSAIDs, metformin if eGFR < 30)",
        window: "immediate",
        role: "pharmacy",
      },
      { action: "Urine output monitoring + fluid assessment", window: "now", role: "nurse" },
    ],
    monitoring: [{ action: "Repeat creatinine + K+ in 6–12h", window: "today", role: "lab" }],
    dispo: "Nephrology review if creatinine rises again",
    evidence:
      "Early nephrotoxin withdrawal + surveillance reduces dialysis-requiring AKI progression.",
  },
  copd_exacerbation: {
    ...
    immediate: [
      {
        action: "Rescue pack (prednisolone + salbutamol) readiness check",
        window: "today",
        role: "pharmacy",
      },
      { action: "SpO2 + symptom diary", window: "continuous", role: "nurse" },
    ],
    monitoring: [{ action: "Clinic tele-follow-up in 24h", window: "tomorrow", role: "doctor" }],
    dispo: "Home management with rapid-review slot",
    evidence:
      "Pre-emptive rescue packs reduce exacerbation-related hospitalizations in COPD cohorts.",
  },
};
