 ...
 */
export function validateDeltaPrivacy(d: FederatedDelta, maxAbsWeight = 5, maxKeys = 64): { ok: boolean; reason?: string } {
  if (d.samples <= 0) return { ok: false, reason: "samples must be positive" };
  if (Object.keys(d.weights).length > maxKeys) return { ok: false, reason: "delta carries too many keys — possible data payload" };
  for (const [k, v] of Object.entries(d.weights)) {
    if (!Number.isFinite(v)) return { ok: false, reason: `non-finite value in ${k}` };
    if (Math.abs(v) > maxAbsWeight) return { ok: false, reason: `|${k}|=${v} exceeds weight-delta bounds — possible raw data` };
  }
  return { ok: true };
}

/** Apply an aggregated global delta onto local weights (bounded step). */
export function applyGlobalUpdate(local: SeriesModelWeights, global: FederatedGlobalModel, lr = 0.5): SeriesModelWeights {
  const w = { ...local.w };
 ...
 */
export function sanitizeOutgoingDelta(weights: Record<string, number>, clip = 1.0, noiseScale = 0.01): Record<string, number> {
