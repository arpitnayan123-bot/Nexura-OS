...
function BookAppointment({ onClose, onDone, scheduleData }: { onClose: () => void; onDone: () => void; scheduleData: ScheduleData | null }) {
  ...
  return (
    ...
    >
      <div>
        <label htmlFor="bk-patient" className="sr-only">Patient</label>
        <select id="bk-patient" value={patientId} onChange={(e) => setPatientId(e.target.value)} className={nxField}>
          <option value="">Select patient…</option>
          {data?.patients.map((p) => <option key={p.id} value={p.id}>{p.fullName} · {p.uhid}</option>)}
        </select>
      </div>
      <div>
        <label htmlFor="bk-doctor" className="sr-only">Doctor</label>
        <select id="bk-doctor" value={doctorId} onChange={(e) => setDoctorId(e.target.value)} className={nxField}>
          <option value="">Select doctor…</option>
          {scheduleData?.doctors.map((d) => <option key={d.id} value={d.id}>{d.name} · {d.speciality}</option>)}
        </select>
      </div>
      <div>
        <label htmlFor="bk-when" className="sr-only">Date and time</label>
        <input id="bk-when" type="datetime-local" value={when} onChange={(e) => setWhen(e.target.value)} className={nxField} />
      </div>
      <div>
        <label htmlFor="bk-complaint" className="sr-only">Chief complaint</label>
        <input id="bk-complaint" value={complaint} onChange={(e) => setComplaint(e.target.value)} placeholder="Chief complaint (optional)" className={nxField} />
      </div>
      {suggestions.length > 0 && (
        <div className="rounded-md border border-accent-line bg-accent-soft px-3 py-2">
          ...
          <div className="mt-1 flex flex-wrap gap-1.5">
            {suggestions.map((s) => (
              <button key={s} type="button" onClick={() => setWhen(new Date(s).toISOString().slice(0, 16))} className="rounded border border-accent-line px-1.5 py-0.5 text-[10px] text-accent hover:bg-accent-soft">
