...
export function MessagesCenter() {
  ...
  const { data, error, loading, refresh } = useNx<MsgData>(
    `/api/nx/messages?channel=${encodeURIComponent(channel)}`,
    { pollMs: 10000 },
  );
  ...
  async function send() {
    ...
    setSending(true);
    try {
      await nx("/api/nx/messages", {
        method: "POST",
        body: JSON.stringify({ channel, body: draft }),
      });
      setDraft("");
      refresh();
    } catch (e) {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[260px_1fr]">
        ...
        <div className="space-y-1">
          {data?.channels.map((c) => (
            ...
            >
              ...
              {c.kind === "care-team" && (
                <span className="ml-auto text-[9px] uppercase text-ink-4">care</span>
              )}
            </button>
          ))}
        </div>
      ...
      <Panel
        title={data?.channels.find((c) => c.key === channel)?.label || channel}
        subtitle="Messages carry clinical context without exposing more than needed"
      >
        <div className="flex h-[440px] flex-col">
          <div className="nx-scroll flex-1 space-y-2.5 overflow-y-auto pr-1">
            {loading ? (
              ...
              <Empty
                ...
                hint="Start the conversation — every message is audit-logged."
              />
            ) : (
              data.messages.map((m) => (
                <div key={m.id} className="max-w-[85%]">
                  <p className="text-[10px] text-ink-3">
                    {m.sender} · {m.role} · {fmtClock(m.at)}
                  </p>
                  <div
                    className={`mt-0.5 rounded-xl px-3 py-2 text-xs ${m.role === "system" ? "border border-vio-line bg-vio-soft text-vio" : "border border-line bg-panel text-ink"}`}
                  >
                    {m.body}
                  </div>
                </div>
          ...
          <div className="mt-3 flex items-center gap-2 border-t border-line pt-3">
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg bg-accent px-3 py-2 text-xs font-semibold text-accent-ink hover:bg-accent disabled:opacity-50"
            >
