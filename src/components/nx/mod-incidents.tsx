...
export function IncidentCenter() {
  const { data, error, loading, refresh } = useNx<IncidentData>("/api/nx/incidents", {
    pollMs: 30000,
  });
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Stat
          ...
          tone={data.counts.open ? "warn" : "good"}
          icon={<ShieldAlert className="h-4 w-4" />}
        />
        <Stat
          label="Critical"
          value={data.counts.critical}
          tone={data.counts.critical ? "critical" : "good"}
        />
      ...
      {data.incidents.length === 0 ? (
        ...
        <div className="space-y-2">
          {data.incidents.map((i) => (
            <div
              key={i.id}
              className={`rounded-xl border p-4 ${i.severity === "critical" ? "border-crit-line bg-crit-soft" : i.severity === "major" ? "border-accent-line bg-accent-soft" : "border-line bg-panel"}`}
            >
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-[11px] text-ink-3">
                    {i.location || "hospital-wide"} · reported by {i.reportedBy || "—"} ·{" "}
                    {timeAgo(i.createdAt)}
                    {i.acknowledgedAt && ` · ack ${timeAgo(i.acknowledgedAt)}`}
                  </p>
                </div>
                <div className="flex shrink-0 items-center gap-1.5">
                  {busyId === i.id ? (
                    ...
                    <>
                      ...
                      {(i.status === "acknowledged" || i.status === "open") && (
                        <button
                          onClick={() => patch(i.id, "investigating")}
                          className="rounded-md border border-line-2 px-2 py-1 text-[11px] text-ink-2 hover:bg-inset"
                        >
                          Investigate
                        </button>
                      )}
                      {["acknowledged", "investigating", "open"].includes(i.status) && (
                        <button
                          onClick={() => patch(i.id, "resolved")}
                          className="rounded-md border border-good-line px-2 py-1 text-[11px] text-good hover:bg-good-soft"
                        >
                          Resolve
                        </button>
                      )}
...
function ReportIncident({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  ...
  async function submit() {
    ...
    setBusy(true);
    try {
      await nx("/api/nx/incidents", {
        method: "POST",
        body: JSON.stringify({ title, description, severity, category, location }),
      });
  ...
  return (
    ...
    >
      <div>
        <label htmlFor="inc-title" className="sr-only">
          What happened?
        </label>
        <input
          id="inc-title"
      ...
      <div>
        <label htmlFor="inc-desc" className="sr-only">
          Details
        </label>
        <textarea
          id="inc-desc"
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label htmlFor="inc-sev" className="sr-only">
            Severity
          </label>
          <select
            id="inc-sev"
        ...
        <div>
          <label htmlFor="inc-cat" className="sr-only">
            Category
          </label>
          <select
            id="inc-cat"
      ...
      <div>
        <label htmlFor="inc-loc" className="sr-only">
          Location
        </label>
        <input
          id="inc-loc"
          ...
          placeholder="Location (e.g. ICU Bay 2)"
          className={nxField}
        />
      </div>
    </NxModal>
