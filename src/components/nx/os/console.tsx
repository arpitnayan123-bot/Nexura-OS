...
function railSnapshot(): boolean {
  try {
    return localStorage.getItem("nx-rail") === "1";
  } catch {
    return false;
  }
}
...
function RailContent({
  ...
  const grouped = useMemo(
    () =>
      GROUP_ORDER.map((g) => ({ group: g, items: apps.filter((a) => a.group === g) })).filter(
        (g) => g.items.length > 0,
      ),
    [apps],
  );
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      {/* nav */}
      <nav
        className="nx-scroll flex-1 overflow-y-auto px-2.5 py-3"
        aria-label="Hospital OS modules"
      >
        {grouped.map(({ group, items }) => (
          <div key={group} className="mb-3">
            {!collapsed && (
              <p className="mb-1 px-2.5 text-[9.5px] font-semibold uppercase tracking-[0.14em] text-ink-4">
                {group}
              </p>
            )}
              ...
              {items.map((a) => {
                ...
                return (
                  ...
                  >
                    {isActive && (
                      <span
                        className="absolute left-0 top-1/2 h-4.5 w-0.5 -translate-y-1/2 rounded-full bg-accent"
                        aria-hidden
                      />
                    )}
                    <a.icon
                    ...
                    {!collapsed && badge > 0 && (
                      <span className="ml-auto rounded-full bg-crit-soft px-1.5 py-0.5 text-[10px] font-semibold text-crit">
                        {badge}
                      </span>
                    )}
                    {collapsed && badge > 0 && (
                      <span
                        className="absolute right-1.5 top-1.5 h-1.5 w-1.5 rounded-full bg-crit"
                        aria-hidden
                      />
                    )}
                  </button>
      ...
      <div className="shrink-0 border-t border-line p-2.5">
        <button
          ...
          title="Lock (⌘L)"
          className={cn(
            "flex h-8 w-full items-center gap-2.5 rounded-lg px-2.5 text-[12.5px] text-ink-3 transition-colors hover:bg-panel-2 hover:text-ink",
            collapsed && "justify-center px-0",
          )}
        >
        ...
        <div
          className={cn(
            "mt-1 flex items-center gap-2.5 rounded-lg p-2",
            collapsed && "justify-center p-0 py-2",
          )}
        >
          <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-inset text-[11px] font-semibold text-ink-2">
            {initials}
          </span>
          {!collapsed && (
            <>
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-[10.5px] capitalize text-ink-4">
                  {user.role}
              ...
              <button
                ...
                title="Sign out"
                aria-label="Sign out"
                className="rounded-md p-1.5 text-ink-4 transition-colors hover:bg-panel-2 hover:text-crit"
              >
...
export function ConsoleShell({
  ...
  const toggleRail = () => {
    ...
    try {
      localStorage.setItem("nx-rail", next ? "1" : "0");
    } catch {
      /* private mode */
    }
  ...
  const fallbackKey = useMemo(
    () => APPS.find((a) => a.system || allowed.has(a.key))?.key ?? "command-center",
    [allowed],
  );
  const effectiveActive =
    active && (appFor(active)?.system || allowed.has(active)) ? active : fallbackKey;
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <header className="relative z-20 flex h-14 shrink-0 items-center gap-3 border-b border-line bg-panel px-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            {!isMobile && (
              <p className="truncate text-[11.5px] leading-tight text-ink-3">{app.desc}</p>
            )}
          </div>
          ...
          >
            ...
            <kbd className="hidden rounded border border-line-2 bg-panel px-1 py-0.5 text-[9.5px] text-ink-4 sm:inline">
              ⌘K
            </kbd>
          </button>
