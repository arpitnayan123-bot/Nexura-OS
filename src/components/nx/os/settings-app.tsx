...
const ACCENTS: Array<{ id: Accent; label: string; swatch: string }> = [
  {
    id: "amber",
    label: "Solar Amber",
    swatch: "linear-gradient(135deg, oklch(0.815 0.135 78), oklch(0.7 0.14 65))",
  },
  {
    id: "jade",
    label: "Jade",
    swatch: "linear-gradient(135deg, oklch(0.78 0.14 168), oklch(0.62 0.12 175))",
  },
  {
    id: "coral",
    label: "Coral",
    swatch: "linear-gradient(135deg, oklch(0.74 0.15 35), oklch(0.62 0.16 30))",
  },
  {
    id: "cyan",
    label: "Mineral Cyan",
    swatch: "linear-gradient(135deg, oklch(0.79 0.11 210), oklch(0.64 0.1 215))",
  },
  {
    id: "violet",
    label: "Iris",
    swatch: "linear-gradient(135deg, oklch(0.74 0.14 300), oklch(0.58 0.15 295))",
  },
];
...
const WALLPAPERS: Array<{ id: Wallpaper; label: string; hint: string; css: string }> = [
  {
    id: "aurora",
    label: "Aurora",
    hint: "Signature amber atmosphere",
  ...
  },
  {
    id: "dawn",
    label: "Dawn",
    hint: "Warm rose for morning rounds",
  ...
  },
  {
    id: "meadow",
    label: "Meadow",
    hint: "Calm sage, easy on long shifts",
  ...
  },
  {
    id: "mono",
    label: "Mono",
    hint: "Pure graphite, zero atmosphere",
        ...
        {section === "appearance" && (
          <div className="max-w-xl space-y-8">
            <Group
              title="Theme"
              icon={SunMoon}
              hint="Light and dark are designed independently — pick a mood."
            >
              <Segmented
                value={os.theme}
            ...
            <Group
              title="Accent"
              icon={Palette}
              hint="One signature color, applied across the system."
            >
            ...
            <Group
              ...
              icon={WallpaperIcon}
              hint="The ambient wash behind the console — a quiet backdrop, it never affects data colors."
            >
              <div className="flex flex-wrap gap-3">
                {WALLPAPERS.map((w) => (
                  <button
                  ...
                  >
                    ...
                    {os.wallpaper === w.id && (
                      <Check className="absolute right-1.5 top-1.5 h-3.5 w-3.5 text-white drop-shadow" />
                    )}
                  </button>
                ))}
              </div>
            ...
            <Group
              title="Density"
              icon={Type}
              hint="Compact tightens list spacing for data-heavy work."
            >
              <Segmented
                value={os.density}
            ...
            <Group
              title="Night warmth"
              icon={Moon}
              hint="Adds a warm tint for late shifts. Does not change data colors."
            >
        ...
        {section === "accessibility" && (
          <div className="max-w-xl space-y-8">
            <Group
              title="Motion"
              icon={PersonStanding}
              hint="Reduced motion removes transitions and ambient animation."
            >
              <Segmented
                value={os.motion}
            ...
            <Group
              title="Focus"
              icon={Moon}
              hint="While on duty, critical alerts always break through."
            >
              <Toggle
                checked={os.focus}
                onChange={os.setFocus}
                label="Focus mode — quiet non-critical interruptions"
              />
            </Group>
              ...
              <dl className="space-y-2 text-sm">
                {[
                  ...
                  <div key={k} className="flex items-center justify-between gap-4">
                    ...
                    <dd>
                      <kbd className="rounded-md border border-line-2 bg-inset px-2 py-0.5 font-mono text-[11px] text-ink-2">
                        {k}
                      </kbd>
                    </dd>
                  </div>
        ...
        {section === "account" && (
          <div className="max-w-xl space-y-6">
            <Group
              ...
              icon={UserRound}
              hint="Access is role-based and re-checked on every API call."
            >
              {session?.user ? (
            ...
            <Group title="Connection" icon={Wifi}>
              <div className="flex items-center gap-2 text-sm text-ink-2">
                ...
                {typeof navigator !== "undefined" && navigator.onLine
                  ? "Online — live data with 30s polling"
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-ink-3">
                  Nexura Hospital Operating System · Version 4.0 "Foundation"
                </p>
              </div>
            </div>
            <p className="text-sm leading-relaxed text-ink-2">
              One calm, real-time surface for the whole hospital — command center, clinical
              workflows, operations and governed AI. {APPS.filter((a) => !a.system).length} apps
              installed. Every action is written to a tamper-evident audit chain, and AI never makes
      ...
      {options.map((o) => (
        <button
          ...
          className={cn(
            ...
            value === o.value
              ? "bg-panel-3 font-medium text-ink shadow-[var(--nx-e1)]"
  ...
  return (
    ...
    >
      ...
      >
        <span
          className={cn(
            "absolute h-4.5 w-4.5 h-5 w-5 rounded-full bg-white shadow transition-transform",
...
export function LanguageSection() {
  ...
  const [mode, setMode] = useState<"desktop" | "tablet" | "kiosk">(
    () => readLS("nx-mode", "desktop") as "desktop" | "tablet" | "kiosk",
  );
  ...
  return (
    <div className="max-w-xl space-y-8">
      <Group
        title="Language"
        icon={Type}
        hint="Applies to OS chrome, actions and statuses. Clinical content stays English until terminology review."
      >
            ...
            <button
              ...
              className={cn(
                ...
                locale === l
                  ...
                  : "border-line-2 bg-inset text-ink-2 hover:bg-panel-3",
              )}
            >
              {LOCALE_LABELS[l]}
              <span className="block text-[10px] font-normal text-ink-5">
                {tr(l, "shell.modules")} · {tr(l, "action.complete")}
              </span>
            </button>
          ))}
        </div>
      ...
      <Group
        title="Device mode"
        icon={WallpaperIcon}
        hint="Tablet: 44px+ touch targets for ward rounds. Kiosk: locked fullscreen single-app mode for shared devices."
      >
        <Segmented
          value={mode}
