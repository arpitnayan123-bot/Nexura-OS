...
const DEMO_ACCOUNTS: DemoAccount[] = [
  {
    ...
    name: "Devika Rao",
    role: "Super Admin",
    desc: "Entire platform, every hospital, every setting",
  },
  {
    code: "ADM.SUNIL",
    email: "admin@demo.nexura.health",
    name: "Sunil Menon",
    role: "Hospital Administrator",
    desc: "Full hospital access: users, roles, settings, audit",
  },
  {
    code: "DR.RAJESH",
    email: "doctor@demo.nexura.health",
    name: "Dr. Rajesh Sharma",
    role: "Doctor",
    desc: "Clinical workspace, orders, notes, sign-off",
  },
  {
    code: "NS.PRIYA",
    email: "nurse@demo.nexura.health",
    name: "Nurse Priya Singh",
    role: "Nurse",
    desc: "Shift tasks, medication administration, vitals",
  },
  {
    code: "RC.MEERA",
    ...
    name: "Meera Iyer",
    role: "Receptionist",
    desc: "Appointments, check-in, patient registration",
  },
  {
    code: "RX.KAVITA",
    email: "pharmacy@demo.nexura.health",
    name: "Kavita Verma",
    role: "Pharmacist",
    desc: "Verification, dispensing, stock, MAR",
  },
  {
    code: "LAB.SURESH",
    email: "lab@demo.nexura.health",
    name: "Suresh Patel",
    role: "Lab Technician",
    desc: "Specimen queue, result entry, validation",
  },
  {
    code: "BILL.ARVIND",
    ...
    desc: "Charges, payments, claims, export",
  },
  {
    code: "PAT.ARBOR",
    email: "patient@demo.nexura.health",
    name: "Arbor Mehta (Patient)",
    role: "Patient",
    desc: "Own record only — appointments and demographics",
  },
  {
    code: "CEO.NEHA",
    email: "executive@demo.nexura.health",
    name: "Neha Kulkarni",
    role: "Operations Executive",
    desc: "Analytics, revenue, compliance overview",
  },
];
  ...
  function currentPayload(): Record<string, unknown> {
    if (mode === "password") {
      return {
        ...
        ...(mfaRequired ? { mfaToken: mfaRequired.mfaToken, mfaCode } : {}),
      };
    }
  ...
  return (
    <div className="relative flex min-h-dvh items-center justify-center overflow-hidden p-4">
      ...
      <div className="nx-sky" aria-hidden>
        <div
          className="nx-blob left-[-12%] top-[-22%] h-[60vh] w-[55vw]"
          style={{ background: "var(--nx-sky-1)" }}
        />
        <div
          className="nx-blob nx-blob-b right-[-18%] top-[6%] h-[55vh] w-[48vw]"
          style={{ background: "var(--nx-sky-3)" }}
        />
        <div
          className="nx-blob nx-blob-c bottom-[-30%] left-[20%] h-[50vh] w-[52vw]"
          style={{ background: "var(--nx-sky-2)" }}
        />
      </div>
      ...
      <div className="relative grid w-full max-w-4xl gap-10 py-10 md:grid-cols-[1fr_1.15fr] md:gap-12">
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <p className="text-[11px] font-medium uppercase tracking-[0.18em] text-ink-4">
                by Nexura · v4 “Foundation”
              </p>
            </div>
          </div>
          <p className="mt-7 max-w-sm text-[13.5px] leading-relaxed text-ink-2">
            One connected system for every person, department, workflow, room and decision — a calm
            command layer over the entire hospital.
          </p>
          <div className="mt-7 space-y-2.5 text-[12px] text-ink-3">
            <p className="flex items-center gap-2">
              <ShieldCheck className="h-4 w-4 text-good" /> Role-based access with audited actions
            </p>
            <p className="flex items-center gap-2">
              <ShieldCheck className="h-4 w-4 text-good" /> Break-glass policy enforced on every
              record
            </p>
            <p className="flex items-center gap-2">
              <ShieldCheck className="h-4 w-4 text-good" /> Governed AI — it assists, clinicians
              decide
            </p>
          </div>
        ...
        <form
          ...
          className="nx-glass-2 nx-sheen rounded-3xl border border-line p-6 shadow-[var(--nx-e4)]"
        >
            ...
            >
              <button
                ...
                className={cn(
                  ...
                  mode === "password" ? "bg-accent-soft text-ink" : "text-ink-3 hover:text-ink-2",
                )}
              >
                Email
              </button>
              <button
                ...
                className={cn(
                  "rounded-md px-2.5 py-1 text-[11px] font-medium transition",
                  mode === "pin" ? "bg-accent-soft text-ink" : "text-ink-3 hover:text-ink-2",
                )}
              >
                Staff code
              </button>
          ...
          {mode === "password" ? (
            <>
              <label
                htmlFor="nx-email"
                className="mt-5 block text-[11px] font-medium uppercase tracking-wider text-ink-3"
              >
                Email
              </label>
              ...
              <label
                htmlFor="nx-pw"
                className="mt-3.5 block text-[11px] font-medium uppercase tracking-wider text-ink-3"
              >
                Password
              </label>
              <div className="relative mt-1.5">
              ...
              <div className="mt-3 flex items-center justify-between text-[11.5px]">
                <label className="flex cursor-pointer items-center gap-1.5 text-ink-3">
                  <input
                    ...
                    className="h-3.5 w-3.5 accent-[var(--nx-accent)]"
                  />
          ...
          {mfaRequired && (
            <div className="mt-4 rounded-xl border border-accent-line bg-accent-soft p-3">
              <label
                htmlFor="nx-mfa"
                className="flex items-center gap-1.5 text-[11px] font-medium uppercase tracking-wider text-ink-2"
              >
          ...
          {error && (
            <p
              role="alert"
              className="mt-3.5 rounded-xl border border-crit-line bg-crit-soft px-3 py-2 text-xs text-crit"
            >
              {error}
            </p>
          )}
          {notice && !error && (
            <p
              role="status"
              className="mt-3.5 rounded-xl border border-accent-line bg-accent-soft px-3 py-2 text-xs text-ink-2"
            >
              {notice}
            </p>
          )}
          ...
          >
            ...
            {showDemo ? "Hide demo roles" : "Explore demo roles"}
          </button>

          {showDemo && (
            <div className="mt-4 border-t border-line pt-4">
              <p className="text-[10.5px] font-semibold uppercase tracking-[0.14em] text-ink-4">
                Demo identities · fills the form, you press Sign in
              </p>
              ...
              <p className="mt-2 text-[10px] leading-relaxed text-ink-4">
                Every role sees a different OS. Demo sign-ins use the same audited auth flow as
                production — nothing is bypassed. Legacy fast path: staff code + PIN 2468.
              </p>
            </div>
          )}
        </form>
