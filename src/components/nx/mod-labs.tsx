...
interface LabRow {
  ...
  stage: string;
  tatMins: number | null;
  results: Array<{ id: string; test: string; value: string | null; unit: string | null; flag: string; ref: unknown[]; reportedAt: string | null }>;
}
interface LabData {
  queue: LabRow[];
  specimenTasks: Array<{ id: string; title: string; priority: string; dueAt: string | null; location: string | null }>;
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
        <Stat label="STAT" value={data.stats.stat} tone={data.stats.stat ? "critical" : "good"} icon={<TestTube2 className="h-4 w-4" />} />
        ...
        <Stat label="Critical values" value={data.stats.critical} tone={data.stats.critical ? "critical" : "good"} icon={<FlaskConical className="h-4 w-4" />} />
      </div>

      {resultFor && <ResultEntry row={resultFor} onClose={() => setResultFor(null)} onDone={() => { setResultFor(null); refresh(); }} />}

      <Panel title="Specimen collection tasks" subtitle="Auto-routed by order automation with SLA timers">
        {data.specimenTasks.length === 0 ? <Empty title="No pending collections" /> : (
          <div className="flex flex-wrap gap-2">
            {data.specimenTasks.map((t) => (
              <div key={t.id} className="flex items-center gap-2 rounded-lg border border-line bg-panel px-3 py-2">
      ...
      <Panel title="Worklist" subtitle="Ordered → acknowledged → processing → validated">
        {data.queue.length === 0 ? <Empty title="No lab orders" /> : (
              ...
              <div key={o.id} className={`rounded-xl border p-3.5 ${o.priority === "stat" ? "border-crit-line bg-crit-soft" : "border-line bg-panel"}`}>
                <div className="flex flex-wrap items-start justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    {o.status === "ordered" && <button onClick={() => advance(o.id, "acknowledged")} disabled={busyId === o.id} className="rounded-md border border-line-2 px-2 py-1 text-[11px] text-ink-2 hover:bg-inset disabled:opacity-50">Acknowledge</button>}
                    {o.status === "acknowledged" && <button onClick={() => advance(o.id, "in_progress")} disabled={busyId === o.id} className="rounded-md border border-vio-line px-2 py-1 text-[11px] text-vio hover:bg-vio-soft disabled:opacity-50">Collect specimen</button>}
                    {o.status === "in_progress" && <button onClick={() => setResultFor(o)} className="rounded-md bg-accent px-2.5 py-1 text-[11px] font-semibold text-accent-ink hover:bg-accent">Enter result</button>}
                  </div>
                </div>
                {o.results.length > 0 && (
...
function ResultEntry({ row, onClose, onDone }: { row: LabRow; onClose: () => void; onDone: () => void }) {
  ...
  async function submit() {
    ...
    try {
      const created = await nx<{ result: { id: string } }>("/api/nx/labs", {
        method: "POST",
        body: JSON.stringify({ orderId: row.id, testName, value, unit, refMin: refMin ? Number(refMin) : undefined, refMax: refMax ? Number(refMax) : undefined, flag }),
      });
      // Validate immediately (completes the lifecycle)
      await nx("/api/nx/orders", { method: "PATCH", body: JSON.stringify({ id: row.id, to: "validate_result", resultId: created.result.id, flag, value }) });
      toast.success(flag === "critical" ? "Critical value recorded — escalation automation fired" : "Result validated");
      onDone();
    } catch (e) {
