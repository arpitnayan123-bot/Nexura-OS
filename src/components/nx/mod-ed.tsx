...
export function EdBoard() {
  ...
  const sorted = [...data.cases].sort(
    (a, b) => a.acuity.level - b.acuity.level || b.waitMins - a.waitMins,
  );
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
        ...
        />
        <Stat
          label="Resuscitation (L1)"
          value={data.stats.level1}
          tone={data.stats.level1 ? "critical" : "good"}
        />
        <Stat
          label="Awaiting space"
          value={data.stats.waitingSpace}
          tone={data.stats.waitingSpace ? "warn" : "good"}
        />
        <Stat
          label="Avg wait"
          value={`${data.stats.avgWaitMins}m`}
          tone={data.stats.avgWaitMins > 30 ? "warn" : "default"}
          icon={<Timer className="h-4 w-4" />}
        />
        <Stat
          label="Waiting >60m"
          value={data.stats.longWaits}
          tone={data.stats.longWaits ? "critical" : "good"}
        />
      </div>

      <Panel
        title="Arrival & treatment board"
        subtitle="Acuity-ranked · live wait times · time-to-treatment tracking"
      >
        {sorted.length === 0 ? (
          <Empty
            title="No ED cases in the last 24h"
            hint="Emergency admissions appear here the moment they are registered."
          />
        ) : (
          <div className="space-y-2">
            {sorted.map((c) => (
              <div
                key={c.id}
                className={`rounded-xl border p-4 ${c.acuity.level === 1 ? "border-crit-line bg-crit-soft" : "border-line bg-panel"}`}
              >
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      <p className="text-sm font-medium text-ink">{c.patient.fullName}</p>
                      <Pill tone={ACUITY_TONE[c.acuity.level - 1]}>
                        L{c.acuity.level} · {c.acuity.label}
                      </Pill>
                      {c.patient.allergy && (
                        <Pill tone="critical">allergy: {c.patient.allergy}</Pill>
                      )}
                      <StatusPill status={c.disposition} />
                    </div>
                    <p className="mt-0.5 text-[11px] text-ink-3">
                      {c.patient.uhid} · {c.patient.age ?? "?"}y {c.patient.gender} ·{" "}
                      {c.diagnosis || "undifferentiated"} · {c.location} ·{" "}
                      {c.doctor || "unassigned"}
                    </p>
                    <p className="mt-1 text-[11px] text-ink-4">Acuity: {c.acuity.reason}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <Pill tone={c.waitMins > 60 ? "critical" : c.waitMins > 30 ? "warn" : "good"}>
                      wait {c.waitMins}m
                    </Pill>
                    <button
                ...
                {c.latestVitals && (
                  <p className="mt-2 text-[11px] tabular-nums text-ink-3">
                    Vitals {timeAgo(c.latestVitals.at)}: BP {c.latestVitals.bp} · P
                    {c.latestVitals.pulse ?? "?"} · SpO₂ {c.latestVitals.spo2 ?? "?"}% ·{" "}
                    {c.latestVitals.temp ?? "?"}°C
                  </p>
                )}
              </div>
            ))}
...
function TriageNoteModal({
  ...
  async function submit() {
    ...
    try {
      await nx("/api/nx/ed", {
        method: "PATCH",
        body: JSON.stringify({ id: row.id, note: note.trim() }),
      });
    ...
    >
      <div>
        ...
        <textarea
          ...
          placeholder="Disposition note, observations, or instructions…"
          rows={4}
          autoFocus
          className={nxField}
        />
