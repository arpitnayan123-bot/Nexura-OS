    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <Stat
          ...
          sub={`${s.collectionPct}% collection rate`}
          tone="good"
        />
        <Stat
          label="Pending"
          value={inr(s.pending)}
          tone={s.pending > s.collected ? "warn" : "default"}
        />
        <Stat
          label="Claims"
          value={s.claimsSubmitted}
          sub={`${s.claimsApproved} approved · ${s.claimsQuery} queries · ${s.claimsRejected} rejected`}
          tone="info"
        />
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Panel
          title="Patient bills"
          subtitle="Clinical activity linked to financial workflow — patient-friendly statements"
        >
          {data.bills.length === 0 ? (
                ...
                >
                  <div className="min-w-0">
                    <p className="truncate text-xs font-medium text-ink">
                      {b.patient.fullName} <span className="text-ink-3">{b.patient.uhid}</span>
                    </p>
                    <p className="text-[11px] text-ink-3">
                      {b.paymentMode} · {timeAgo(b.createdAt)}
                    </p>
                  </div>
        ...
        <Panel
          ...
          subtitle="Pre-auth → query → approval pipeline (IRDAI-aligned)"
        >
          {data.claims.length === 0 ? (
            <Empty title="No claims" />
          ) : (
            <div className="nx-scroll max-h-[420px] space-y-2 overflow-y-auto pr-1">
              {data.claims.map((c) => (
                <div key={c.id} className="rounded-lg border border-line bg-panel px-3 py-2.5">
                  <div className="flex items-center justify-between gap-2">
                    <p className="truncate text-xs font-medium text-ink">
                      {c.patient.fullName} · {c.tpa}
                    </p>
                    <StatusPill status={c.status} />
                  </div>
                  <p className="text-[11px] text-ink-3">
                    {c.icd10 || "ICD-10 pending"} · est {inr(c.estimated)}
                    {c.approved > 0 && ` · approved ${inr(c.approved)}`} ·{" "}
