...
interface PatientRecord {
  ...
  orders?: Array<{
    id: string;
    type: string;
    details: { testName?: string; dose?: string };
    priority: string;
    status: string;
    at: string;
    doctor?: string;
...
export function PatientRegistry() {
  ...
  const { data, error, loading, refresh } = useNx<{ patients: PatientRow[] }>(
    "/api/nx/patients?take=24",
  );
  ...
  const { data: searched } = useNx<{ patients: PatientRow[] }>(
    dq.trim().length >= 2 ? `/api/nx/patients?q=${encodeURIComponent(dq.trim())}` : null,
  );
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-ink-3">
                      {p.uhid} · {p.age ?? "?"}y · {p.gender} · {p.bloodGroup || "?"}
                    </p>
                  </div>
                </div>
                {p.allergy ? (
                  <Pill tone="critical">allergy</Pill>
                ) : p.isAdmitted ? (
                  <Pill tone="info">IPD</Pill>
                ) : (
                  <Pill>OPD</Pill>
                )}
              </div>
              <div className="mt-3 flex flex-wrap gap-1.5">
                {p.isAdmitted && p.currentLocation && (
                  <Pill tone="violet">{p.currentLocation}</Pill>
                )}
                {p.chronicConditions &&
                  p.chronicConditions
                    .split(",")
...
function PatientDrawer({ id, onClose }: { id: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/60" onClick={onClose}>
      <div
        className="nx-scroll h-full w-full max-w-3xl overflow-y-auto border-l border-line bg-[#0a1120] p-5"
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-xl border border-line bg-panel p-5">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-ink-3">
                      {d.profile.uhid} · {d.profile.age ?? "?"}y {d.profile.gender} ·{" "}
                      {d.profile.bloodGroup || "?"} · speaks {d.profile.language}
                    </p>
                  </div>
                </div>
                <div className="flex flex-wrap items-center gap-1.5">
                  {d.status.isAdmitted ? (
                    <Pill tone="info">Admitted · {d.status.location}</Pill>
                  ) : (
                    <Pill>Not admitted</Pill>
                  )}
                  {d.allergies.map((a) => (
              ...
              <div className="mt-3 grid grid-cols-2 gap-2 text-[11px] text-ink-3 sm:grid-cols-4">
                <p>
                  Phone: <span className="text-ink-2">{d.profile.phone || "—"}</span>
                </p>
                <p>
                  Emergency:{" "}
                  <span className="text-ink-2">
                    {d.profile.emergencyContact.name || "—"}{" "}
                    {d.profile.emergencyContact.phone || ""}
                  </span>
                </p>
                <p>
                  Insurance:{" "}
                  <span className="text-ink-2">{d.profile.insurance.provider || "self-pay"}</span>
                </p>
                <p>
                  ABHA: <span className="text-ink-2">{d.profile.abhaId || "not linked"}</span>
                </p>
              </div>
              {d.status.attendingDoctor && (
                <p className="mt-2 text-xs text-ink-3">
                  Under care of{" "}
                  <span className="font-medium text-ink">{d.status.attendingDoctor}</span> —{" "}
                  {d.status.admissionDiagnosis}
                  {d.status.expectedDischarge && (
                    <>
                      {" "}
                      · expected discharge{" "}
                      {new Date(d.status.expectedDischarge).toLocaleDateString("en-IN")}
                    </>
                  )}
                </p>
              )}
                ...
                <div className="relative space-y-0 pl-5">
                  ...
                  {d.journey.map((s) => (
                    <div key={s.key} className="relative pb-4 last:pb-0">
                      <span
                        className={`absolute -left-5 top-1 h-[15px] w-[15px] rounded-full border-2 ${s.status === "active" ? "border-accent-line bg-accent-soft animate-pulse" : s.status === "completed" ? "border-good bg-good-soft" : "border-line-2 bg-panel"}`}
                      />
                      ...
                      <p className="text-[11px] text-ink-3">
                        {s.owner} · {s.at ? timeAgo(s.at) : "pending"} · {s.detail}
                      </p>
                    </div>
                  ))}
                </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Panel
                title="Open tasks & pending results"
                subtitle={`${d.whatChanged.orders} orders · ${d.whatChanged.vitals} vitals in last 48h`}
              >
                <div className="space-y-2">
                  {d.openTasks.map((t) => (
                    <div key={t.id} className="rounded-lg border border-line bg-panel px-3 py-2">
                      ...
                      <p className="text-[11px] text-ink-3">
                        {t.ownerName} · due {t.dueAt ? timeAgo(t.dueAt) : "—"}
                      </p>
                    </div>
                  ))}
                  {d.pendingResults.map((r) => (
                    ...
                    >
                      <p className="truncate text-xs text-ink-2">
                        {r.test} <span className="text-ink-3">({r.type})</span>
                      </p>
                      <StatusPill status={r.priority} />
                    </div>
                  ))}
                  {d.openTasks.length === 0 && d.pendingResults.length === 0 && (
                ...
                {d.vitals.length === 0 ? (
                  ...
                  <div className="space-y-1.5">
                    {d.vitals.map((v) => (
                      <div
                        key={v.id}
                        className="flex items-center justify-between gap-2 rounded-lg border border-line bg-panel px-3 py-1.5 text-xs"
                      >
                        <span className="tabular-nums text-ink-2">
                          {v.bp} · P{v.pulse ?? "?"} · SpO₂ {v.spo2 ?? "?"}% · {v.temp ?? "?"}°C
                        </span>
                        <span className="flex items-center gap-2 text-ink-3">
                          {v.news2 != null && (
                            <Pill tone={v.news2 >= 5 ? "critical" : v.news2 >= 3 ? "warn" : "good"}>
                              NEWS2 {v.news2}
                            </Pill>
                          )}
                          {fmtClock(v.at)}
                    ...
                    <div key={o.id} className="rounded-lg border border-line bg-panel px-3 py-2.5">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <p className="text-xs font-medium text-ink">
                          {o.details?.testName || o.type}{" "}
                          <span className="text-ink-3">· {o.type}</span>
                        </p>
            ...
            <Panel
              ...
              actions={
                <button
                  onClick={summarize}
                  disabled={aiBusy}
                  className="flex items-center gap-1.5 rounded-lg bg-vio-soft px-3 py-1.5 text-xs font-medium text-vio ring-1 ring-vio-line hover:bg-vio-soft disabled:opacity-50"
                >
                  {aiBusy ? (
              ...
              {ai ? (
                <div className="space-y-3">
                        ...
                        <div className="mt-1.5 flex flex-wrap gap-1.5">
                          {(items as string[])?.length ? (
                            (items as string[]).map((x, i) => (
                              <Pill key={i} tone={tone as "warn"}>
                                {x}
                              </Pill>
                            ))
                          ) : (
                            <p className="text-[11px] text-ink-4">none noted</p>
                          )}
                        </div>
                      </div>
                    ))}
                  ...
                  {ai.summary.suggestedNextSteps?.length > 0 && (
                    <div>
                      <p className="text-[11px] font-semibold uppercase tracking-wider text-ink-3">
                        Suggested coordination steps (not clinical orders)
                      </p>
                      <ul className="mt-1.5 list-inside list-disc space-y-0.5 text-xs text-ink-2">
                        {ai.summary.suggestedNextSteps.map((s, i) => (
                          <li key={i}>{s}</li>
                        ))}
                      </ul>
                    </div>
                ...
                <Empty
                  ...
                  title={aiBusy ? "Reading the full record…" : "One-click longitudinal summary"}
                  hint="AI condenses the timeline, results and notes into a clinician-friendly brief — clearly labeled, never a diagnosis."
                />
              )}
            </Panel>
