  ...
  return (
    ...
    >
        ...
        <div>
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="mt-0.5 text-xs text-[#988F81]">
                {m.relation} · {m.uhid}
              </div>
            </div>
            <span
              className={`rounded-full border px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.1em] ${
                m.score >= 85
                  ? "border-[#8FBF8F]/40 text-[#8FBF8F]"
                  : m.score >= 75
                    ? "border-[#E3C578]/40 text-[#E3C578]"
                    : "border-[#E58F7A]/40 text-[#E58F7A]"
              }`}
            >
              {m.scoreLabel}
            </span>
      ...
      <div className="rounded-3xl border border-[#2E2A20] bg-[#191611]/80 p-7">
        <div className="text-xs font-semibold uppercase tracking-[0.16em] text-[#C8A55B]">
          Health at a glance
        </div>
        <ul className="mt-4 space-y-3.5">
          {m.vitals.map((v) => (
            <li
              key={v.label}
              className="flex items-center justify-between gap-3 border-b border-dashed border-[#2E2A20] pb-3 last:border-b-0 last:pb-0"
            >
              <span className="text-sm text-[#9C927E]">{v.label}</span>
              <span
                className={`text-sm font-medium tabular-nums ${v.tone === "ok" ? "text-[#EFE7D3]" : "text-[#E3C578]"}`}
              >
                {v.value}
              </span>
            </li>
  ...
  return (
    <div className="nxc-root relative min-h-dvh bg-[#141210] text-[#EFE7D3]">
      ...
      <main>
        ...
        <section className="relative px-6 py-20 sm:py-24" aria-labelledby="gens-heading">
          <SectionHeading
            ...
            title={
              <span id="gens-heading">
                The little things,{" "}
                <em className="text-gold-gradient not-italic">never slipping.</em>
              </span>
            }
          ...
          <div className="mx-auto mt-14 grid max-w-6xl gap-5 lg:grid-cols-2">
              ...
              <ol className="mt-6 space-y-0">
                {VACCINES.map((v, i) => (
                  <li key={v.name} className="relative flex gap-4 pb-6 last:pb-0">
                    {i < VACCINES.length - 1 && (
                      <span
                        className="absolute left-[9px] top-6 h-full w-px bg-[#2E2A20]"
                        aria-hidden="true"
                      />
                    )}
                    <span
                      className={`relative mt-0.5 flex h-[19px] w-[19px] shrink-0 items-center justify-center rounded-full border ${
                        v.done
                          ? "border-[#A16207] bg-[#A16207]/20 text-[#D9B87C]"
                          : "border-[#3A3428] bg-[#241F16] text-transparent"
                      }`}
                    >
                    ...
                    <div className="min-w-0">
                      <div
                        className={`text-sm font-medium ${v.done ? "text-[#EFE7D3]" : "text-[#9C927E]"}`}
                      >
                        {v.name}
                      </div>
                      <div className="text-xs text-[#988F81]">
                        {v.when}
                        {!v.done && " · due later"}
                      </div>
                    </div>
            ...
            <SpotlightCard className="rounded-3xl border border-[#2E2A20] bg-[#191611]/80 p-7">
              <div className="flex items-center gap-2 text-[#C8A55B]">
                ...
                <span className="text-xs font-semibold uppercase tracking-[0.16em]">
                  Devi · medication adherence · 30 days
                </span>
              </div>
                ...
                {ADHERENCE.map((a) => (
                  <div key={a.med}>
                    <div className="flex items-baseline justify-between">
                      ...
                      <span
                        className={`text-sm font-semibold tabular-nums ${a.pct >= 90 ? "text-[#8FBF8F]" : a.pct >= 80 ? "text-[#E3C578]" : "text-[#E58F7A]"}`}
                      >
                        {a.pct}%
                      </span>
                    </div>
                    ...
                    <div className="mt-1.5 text-xs text-[#988F81]">
                      {a.when} · synced from Pharmacia
                    </div>
                  </div>
                ))}
              </div>
              <p className="mt-6 rounded-2xl border border-[#E3C578]/30 bg-[#E3C578]/5 px-4 py-3 text-xs leading-relaxed text-[#E3C578]">
                Amlodipine adherence at 82% — Connect can schedule a gentle daily reminder call with
                her pharmacist.
              </p>
            </SpotlightCard>
          </div>
          ...
          <SectionHeading
            ...
            title={
              <span id="trust-heading">
                Shared care, <em className="text-gold-gradient not-italic">clear boundaries.</em>
              </span>
            }
          />
          <div className="mx-auto mt-12 grid max-w-6xl gap-5 lg:grid-cols-2">
            ...
            <div className="rounded-3xl border border-[#2E2A20] bg-[#191611]/80 p-7">
              <div className="flex items-center gap-2 text-[#C8A55B]">
                ...
                <span className="text-xs font-semibold uppercase tracking-[0.16em]">
                  Family cover · Star Health · FY 2026
                </span>
              </div>
              <div className="mt-6">
                ...
                <ul className="mt-5 space-y-2 text-sm text-[#B3A892]">
                  <li className="flex justify-between border-b border-dashed border-[#2E2A20] pb-2">
                    <span>Devi · cardiology (May)</span>
                    <span className="tabular-nums text-[#EFE7D3]">₹2.6L</span>
                  </li>
                  <li className="flex justify-between border-b border-dashed border-[#2E2A20] pb-2">
                    <span>Aisha · tonsillitis (Jan)</span>
                    <span className="tabular-nums text-[#EFE7D3]">₹34,200</span>
                  </li>
                  <li className="flex justify-between">
                    <span>Arpit · Full Body Gold (Jun)</span>
                    <span className="tabular-nums text-[#EFE7D3]">₹24,900</span>
                  </li>
                </ul>
              </div>
            </div>
            ...
            <div className="rounded-3xl border border-[#2E2A20] bg-[#191611]/80 p-7">
              ...
              <ul className="mt-5 space-y-3">
                {CONSENTS.map((c, i) => (
                  <li
                    key={c.who}
                    className="flex items-center justify-between gap-4 rounded-2xl border border-[#2E2A20] px-4 py-3"
                  >
                    <div className="min-w-0">
              ...
              <p className="mt-4 text-[11px] leading-relaxed text-[#8B8476]">
                Consent changes are logged, timestamped and revocable — the audit trail ships with
                the DPDP compliance report.
              </p>
            </div>
          </div>
        </section>
      ...
      <footer className="border-t border-[#241F16] px-6 py-10 text-center text-xs leading-relaxed text-[#8B8476]">
        Nexura Care Circle — demo family, simulated data. Consent tools reflect DPDP Act 2023
        principles. A product of Nexura OS
      </footer>
    </div>
  );
}
