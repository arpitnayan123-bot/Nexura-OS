...
const BAND_STYLES: Record<RiskBandName, { chip: string; dot: string; label: string }> = {
  green: {
    chip: "text-emerald-700 bg-emerald-50 border-emerald-200 dark:text-emerald-300 dark:bg-emerald-950/40 dark:border-emerald-900",
    dot: "bg-emerald-500",
    label: "Stable",
  },
  yellow: {
    chip: "text-amber-700 bg-amber-50 border-amber-200 dark:text-amber-300 dark:bg-amber-950/40 dark:border-amber-900",
    dot: "bg-amber-500",
    label: "Watchlist",
  },
  red: {
    chip: "text-rose-700 bg-rose-50 border-rose-200 dark:text-rose-300 dark:bg-rose-950/40 dark:border-rose-900",
    dot: "bg-rose-500",
    label: "Critical",
  },
};
...
export function RiskBadge({
  ...
  return (
    ...
    >
      ...
      {uncertain && (
        <span className="text-[10px] leading-none" title="Uncertain — Manual Review Required">
          ?
        </span>
      )}
...
export function RiskSparkline({
  ...
  if (points.length < 2)
    return <div className="h-8 w-[120px] rounded bg-black/5 dark:bg-white/5" />;
