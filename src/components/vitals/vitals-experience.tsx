...
import {
  Activity, HeartPulse, Moon, Footprints, Wind, Watch, Smartphone,
  Bluetooth, Share2, TrendingDown, TrendingUp, AlertTriangle, Pause, Play,
} from "lucide-react";
import {
...
function Sparkline({
  data, width = 260, height = 56, stroke = "#D9B87C", fill = "rgba(161,98,7,0.18)", live = false,
}: {
  data: number[]; width?: number; height?: number; stroke?: string; fill?: string; live?: boolean;
}) {
...
function RingGauge({ value, max, label, sub }: { value: number; max: number; label: string; sub: string }) {
  const R = 52;
  const C = 2 * Math.PI * R;
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="relative h-36 w-36">
        <svg viewBox="0 0 120 120" className="h-full w-full -rotate-90" role="img" aria-label={`${label}: ${value} of ${max}`}>
          <circle cx="60" cy="60" r={R} fill="none" stroke="#2E2A20" strokeWidth="8" />
          <motion.circle
            cx="60" cy="60" r={R} fill="none"
            stroke="url(#goldRing)" strokeWidth="8" strokeLinecap="round"
            strokeDasharray={C}
...
function SleepBar() {
  ...
  return (
    <div>
      <div className="flex h-14 w-full overflow-hidden rounded-2xl border border-[#2E2A20]" role="img" aria-label={`Sleep stages: ${SLEEP.map((s) => `${s.stage} ${s.hrs} hours`).join(", ")}`}>
        {SLEEP.map((s, i) => (
          <motion.div
            key={s.stage}
      ...
      <div className="mt-3 flex flex-wrap gap-x-5 gap-y-1">
        {SLEEP.map((s) => (
          <span key={s.stage} className="inline-flex items-center gap-1.5 text-xs text-[#9C927E]">
            <span className={`inline-block h-2 w-2 rounded-full bg-gradient-to-b ${s.cls}`} aria-hidden="true" />
  ...
  return (
    <SpotlightCard className="relative rounded-3xl border border-[#2E2A20] bg-[#191611]/80 p-7">
      <div className="flex items-start justify-between">
        ...
        >
          {live ? <Pause className="h-4 w-4" aria-hidden="true" /> : <Play className="h-4 w-4" aria-hidden="true" />}
        </button>
      </div>
      <div className="mt-5">
...
const ALERTS = [
  { time: "Yesterday · 03:12", icon: Wind, title: "SpO₂ dipped to 93%", body: "Brief desaturation during sleep — within your pattern, but logged for your physician.", tone: "watch" },
  { time: "Mon · 18:40", icon: AlertTriangle, title: "Elevated resting HR (89 bpm)", body: "Correlated with a 2.1 km evening run. No action needed; hydration suggested.", tone: "info" },
  { time: "Sun · 23:58", icon: Moon, title: "Short REM night (1.1 h)", body: "Late screen time flagged. Sleep onset improved after — worth keeping the routine.", tone: "info" },
];
  ...
  return (
    <section className="relative px-6 py-20 sm:py-24" aria-labelledby="alerts-heading">
      <SectionHeading
        ...
        title={<span id="alerts-heading">Notified with <em className="text-gold-gradient not-italic">context, never panic.</em></span>}
            ...
            <StaggerItem key={a.title}>
              <div className="flex gap-4 rounded-3xl border border-[#2E2A20] bg-[#191611]/80 p-6 transition-colors hover:border-[#A16207]/50">
                <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border ${
                  a.tone === "watch" ? "border-[#E3C578]/40 bg-[#E3C578]/10" : "border-[#3A3428] bg-[#241F16]"
                }`}>
    ...
    <div className="nxt-root relative min-h-dvh bg-[#141210] text-[#EFE7D3]">
      ...
      <header className="relative overflow-hidden px-6 pt-36 pb-16 sm:pt-44">
        ...
        <div className="relative mx-auto max-w-5xl text-center">
          ...
          >
            Connect the watch and sensors you already own. Vitals turns their streams into one
            calm, physician-grade picture — with trends explained, not just plotted.
          </motion.p>
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            {[["Apple Watch", Watch], ["Fitbit / Wear OS", Watch], ["CGMs", Smartphone], ["BLE BP monitors", Bluetooth]].map(([name, Icon]) => {
              const I = Icon as React.ElementType;
              return (
                <span key={name as string} className="inline-flex items-center gap-1.5 rounded-full border border-[#3A3428] px-3.5 py-1.5">
      ...
      <section className="relative px-6 pb-8" aria-labelledby="dash-heading">
        <h2 id="dash-heading" className="sr-only">Live vitals dashboard</h2>
        <div className="mx-auto grid max-w-6xl gap-5 lg:grid-cols-3">
          <div className="lg:col-span-2"><LiveHeartRate /></div>
      ...
      <section className="relative px-6 pb-24" aria-labelledby="share-heading">
        <div className="mx-auto max-w-4xl rounded-3xl border border-[#A16207]/40 bg-gradient-to-b from-[#1D1810] to-[#15120C] p-8 text-center sm:p-10">
          ...
          <h2 id="share-heading" className="font-serif text-2xl font-semibold text-[#F5EDD8] sm:text-3xl">
            Your week, doctor-ready.
          </h2>
          <p className="mx-auto mt-3 max-w-xl text-sm leading-relaxed text-[#B3A892]">
            One tap compiles seven days of vitals, alerts and trends into a summary your doctor can
            read in ninety seconds — straight into Nexura Connect, or as a PDF.
          </p>
          <div className="mt-7 flex flex-wrap items-center justify-center gap-4">
            <Magnetic>
              <Link href="/connect" className="btn-gold inline-flex h-11 items-center gap-2 rounded-full px-6 text-[0.95rem] font-medium">
