...
const ADULT_ROWS: ScheduleRow[] = [
  ...
  {
    when: "35+",
    what: "HbA1c / fasting blood sugar",
    note: "yearly if you have risk factors or are overweight (Indian threshold: BMI 23)",
  },
  { when: "40+", what: "Lipid profile", note: "yearly from 40; earlier if you have risk factors" },
  {
    when: "Women",
    what: "Haemoglobin (Hb)",
    note: "yearly during reproductive age — anaemia is common",
  },
...
const PREGNANCY_ROWS: ScheduleRow[] = [
  {
    when: "First 12 weeks",
    what: "First ANC visit",
    note: "within 12 weeks of missing a period — register at the nearest PHC/CHC",
  },
  ...
  {
    when: "After birth",
    what: "Postnatal checks",
    note: "day 1, day 3, day 7 and a 6-week visit for mother + newborn",
  },
];
  ...
  return (
    <ul className="mt-2.5">
      {rows.map((r) => (
        <li
          key={r.when + r.what}
          className="flex items-start gap-2.5 border-t border-[#EFE9E0]/70 py-2 first:border-t-0"
        >
          <span className="mt-0.5 shrink-0 rounded-full bg-[#9DB89E]/15 px-2.5 py-1 text-[0.6rem] font-bold uppercase tracking-wider text-[#5A7A5B]">
            {r.when}
          </span>
          <div className="min-w-0">
            <p className="text-xs font-semibold text-[#1F1B17]">{r.what}</p>
            {r.note && (
              <p className="mt-0.5 text-[0.7rem] leading-relaxed text-[#5C544D]">{r.note}</p>
            )}
          </div>
        </li>
      ))}
...
export function PreventionSchedule() {
  return (
    <section aria-labelledby="kyh-prevention-title" className="glass-soft rounded-2xl p-5 sm:p-6">
      <h2
        id="kyh-prevention-title"
        className="flex items-center gap-2 font-serif text-base font-semibold tracking-tight text-[#1F1B17]"
      >
        <CalendarCheck className="h-4 w-4 text-[#5A7A5B]" aria-hidden="true" /> Prevention, on
        schedule
      </h2>
      <p className="mt-1.5 flex items-start gap-1.5 text-[0.65rem] leading-relaxed text-[#9A8F84]">
        ...
        General reference from India&rsquo;s public-health guidelines — your doctor personalises
        this for you.
      </p>
      <div className="mt-4 grid gap-3 lg:grid-cols-2">
        <div className="rounded-xl bg-[#FAF7F2]/60 p-3.5 sm:p-4">
          <p className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-[#5C544D]">
            <Baby className="h-3.5 w-3.5 text-[#B8893D]" aria-hidden="true" /> Child immunization
            highlights
          </p>
          <p className="mt-0.5 text-[0.6rem] text-[#B5A99E]">
            India&rsquo;s Universal Immunization Programme — key milestones
          </p>
          <RowList rows={CHILD_ROWS} />
        </div>
        <div className="rounded-xl bg-[#FAF7F2]/60 p-3.5 sm:p-4">
          <p className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-[#5C544D]">
            <Stethoscope className="h-3.5 w-3.5 text-[#5A7A5B]" aria-hidden="true" /> Adult
            screening cadence
          </p>
          <p className="mt-0.5 text-[0.6rem] text-[#B5A99E]">
            Common check-up rhythm for adults in India
          </p>
          <RowList rows={ADULT_ROWS} />
        </div>
        <div className="rounded-xl bg-[#FAF7F2]/60 p-3.5 sm:p-4">
          <p className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-[#5C544D]">
            <HeartHandshake className="h-3.5 w-3.5 text-[#B8860B]" aria-hidden="true" /> Pregnancy
            care (ANC) highlights
          </p>
          <p className="mt-0.5 text-[0.6rem] text-[#B5A99E]">
            Standard India / WHO guidance for every pregnancy
          </p>
