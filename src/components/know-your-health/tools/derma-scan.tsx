...
export function DermaScan() {
  ...
  const run = async () => {
    if (!image) {
      showError("Please upload a skin photo first");
      return;
    }
    setLoading(true);
    setResult(null);
        ...
        >
          <div>
            <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-[#5C544D]">
              Upload a close-up of the skin concern
            </label>
            <ImageUploader
              image={image}
              onPick={setImage}
              ...
              accent={accent}
              label="Drop or click to upload skin photo"
            />
          </div>
          <div>
            <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-[#5C544D]">
              Describe the concern (optional)
            </label>
            <ToolTextarea
              value={concern}
              onChange={setConcern}
              placeholder="e.g. Itchy, red patch on the right forearm that appeared 3 days ago and is slowly spreading…"
              rows={3}
            />
          ...
          >
            ...
            <ResultCard accent={accent} title="When to See a Dermatologist">
              <div className="flex items-start gap-2.5">
                ...
                <p className="text-sm leading-relaxed text-[#1F1B17]">
                  {result.whenToSeeDermatologist}
                </p>
              </div>
            </ResultCard>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="flex items-center gap-1 text-[0.65rem] text-[#9A8F84]">
                ...
                {result.disclaimer || "Informational only — not a diagnosis"}
              </span>
            </div>
            <Disclaimer />
          </motion.div>
