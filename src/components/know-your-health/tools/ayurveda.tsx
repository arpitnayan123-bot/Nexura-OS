...
const QUESTIONS: { q: string; opts: [string, string, string] }[] = [
  {
    ...
    opts: [
      ...
      "Heavy, broad, easy to gain weight",
    ],
  },
  {
    q: "Skin type",
    opts: ["Dry, rough, thin", "Warm, reddish, sensitive", "Cool, oily, thick, smooth"],
  },
  {
    q: "Hair texture",
    opts: ["Dry, frizzy, thin", "Fine, early greying, balding", "Thick, lustrous, oily"],
  },
  {
    q: "Appetite",
    opts: ["Irregular, varies daily", "Sharp, must eat on time", "Steady but slow"],
  },
  {
    q: "Digestion",
    opts: [
      "Gas, bloating, constipation",
      "Strong, acid reflux when delayed",
      "Slow but comfortable, heavy feeling",
    ],
  },
  {
    q: "Energy pattern",
    opts: [
      ...
      "Strong, steady, competitive",
      "Steady, slow to start, enduring",
    ],
  },
  {
    q: "Sleep",
    opts: [
      "Light, interrupted, vivid dreams",
      "Sound, moderate, wake fresh",
  ...
  },
  {
    ...
    opts: [
      ...
      "Warm, sweats easily",
      "Cool, comfortable, dislikes damp",
    ],
  },
  {
    q: "Speech",
    opts: [
      "Fast, talkative, jumps topics",
      "Sharp, articulate, convincing",
      "Slow, measured, melodious",
    ],
  },
  {
    q: "Memory",
    opts: [
      ...
      "Sharp, organised, focused",
      "Slow to learn, never forget",
    ],
  },
  {
    q: "Emotions under stress",
    opts: ["Anxious, fearful, worried", "Irritable, angry, critical", "Calm, attached, withdrawn"],
  },
  {
    q: "Temperament",
    opts: [
      "Enthusiastic, creative, restless",
      "Ambitious, driven, perfectionist",
      "Calm, content, steady",
    ],
  },
  ...
  {
    q: "Decision making",
    opts: ["Quick but changes mind", "Decisive, sharp, firm", "Slow, deliberate, steady"],
  },
  {
    q: "Weather preference",
    opts: ["Loves warm, humid", "Loves cool, dislikes heat", "Loves warm, dry, dislikes cold/damp"],
  },
  {
    q: "Spending habits",
    opts: [
      "Impulsive, on small things",
      "Calculated, on quality items",
      "Saves, buys for the long term",
    ],
  },
  {
    q: "Friendship style",
    opts: [
      "Many friends, brief bonds",
      "Loyal to few, demanding",
      "Long-lasting, slow to make friends",
    ],
  },
  {
    q: "Reaction to change",
    opts: ["Adapts fast but anxious", "Plans and pushes through", "Resists, prefers routine"],
  },
];
...
export function Ayurveda() {
  ...
  const run = async () => {
    ...
    if (cleaned.some((c) => !c)) {
      showError("Please answer all 20 questions");
      return;
    }
    setLoading(true);
    setResult(null);
        ...
        >
          <div className="rounded-2xl glass-soft p-4 shadow-depth">
            <div className="mb-3 flex items-center justify-between">
              <p className="text-xs font-semibold uppercase tracking-wider text-[#5C544D]">
                Question {step + 1} of 20
              </p>
              <span className="rounded-full glass-chip px-2.5 py-0.5 text-[0.65rem] font-medium text-[#5A7A5B]">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <p className="mb-3 font-serif text-lg font-semibold text-[#1F1B17]">
                  {QUESTIONS[step].q}
                </p>
                <div className="space-y-2">
                  {QUESTIONS[step].opts.map((opt, i) => (
                    <button
                      key={i}
                      onClick={() => pick(step, i)}
                      className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm transition-all ${answers[step] === i ? "glass-input shadow-depth" : "glass-chip hover:scale-[1.01]"}`}
                      style={answers[step] === i ? { borderColor: `${accent}80` } : {}}
                    >
                      <span
                        className="grid h-6 w-6 shrink-0 place-items-center rounded-full text-[0.65rem] font-bold"
                        style={{
                          background: `${DOSHA_COLOR[["Vata", "Pitta", "Kapha"][i]]}20`,
                          color: DOSHA_COLOR[["Vata", "Pitta", "Kapha"][i]],
                        }}
                      >
                        {["V", "P", "K"][i]}
                      </span>
            ...
            <div className="mt-3 flex items-center justify-between">
              <button
                ...
                disabled={step === 0}
                className="rounded-full glass-chip px-3 py-1.5 text-xs text-[#5C544D] disabled:opacity-30"
              >
                ← Back
              </button>
              {step < 19 && (
                <button
                  onClick={() => setStep(Math.min(19, step + 1))}
                  className="rounded-full glass-chip px-3 py-1.5 text-xs text-[#5C544D]"
                >
                  Next →
                </button>
              )}
          ...
          <div className="flex items-center gap-3">
            <RunButton
              ...
              accent={accent}
              label="Reveal my Prakriti"
            />
            <span className="text-[0.65rem] text-[#9A8F84]">
              Based on NIIMH Ayurveda principles
            </span>
          </div>
          <Disclaimer />
        </motion.div>
          ...
          >
            <ResultCard accent={accent} title="Your Prakriti">
              <div className="mb-4 flex flex-col items-center">
                <motion.div
                  ...
                  className="grid h-20 w-20 place-items-center rounded-full shadow-depth"
                  style={{
                    background: `linear-gradient(135deg, ${DOSHA_COLOR[result.dominantDosha] || accent}, ${DOSHA_COLOR[result.dominantDosha] || accent}cc)`,
                  }}
                >
                  <Leaf className="h-9 w-9 text-white" />
                </motion.div>
                <p className="mt-2 font-serif text-2xl font-bold text-[#1F1B17]">
                  {result.dominantDosha} Prakriti
                </p>
                    ...
                    <div key={d}>
                      ...
                      <div className="h-2 overflow-hidden rounded-full bg-[#EFE9E0]">
                        <motion.div
                          ...
                          className="h-full rounded-full"
                          style={{
                            background: `linear-gradient(90deg, ${DOSHA_COLOR[name]}, ${DOSHA_COLOR[name]}cc)`,
                          }}
                        />
                      </div>
                    </div>
              ...
              <ResultCard accent="#7A9A7B" title="Personality Traits">
                <div className="flex flex-wrap gap-2">
                  {result.personalityTraits.map((t, i) => (
                    <span
                      key={i}
                      className="rounded-full glass-chip px-2.5 py-1 text-[0.65rem] text-[#5C544D]"
                    >
                      {t}
                    </span>
                  ))}
              ...
              <ResultCard accent="#5A7A5B" title="Recommended Foods">
                <ul className="space-y-1.5">
                  {result.recommendedFoods?.map((r, i) => (
                    <li key={i} className="flex items-start gap-2 text-xs text-[#1F1B17]">
                      <Utensils className="mt-0.5 h-3 w-3 shrink-0 text-[#5A7A5B]" />
                      <span>{r}</span>
                    </li>
                  ))}
                </ul>
              ...
              <ResultCard accent="#B8860B" title="Foods to Avoid">
                <ul className="space-y-1.5">
                  {result.foodsToAvoid?.map((r, i) => (
                    <li key={i} className="flex items-start gap-2 text-xs text-[#1F1B17]">
                      <span className="mt-0.5 h-3 w-3 shrink-0 text-[#8A5A04]">✕</span>
                      <span>{r}</span>
                    </li>
                  ))}
                </ul>
                ...
                <ul className="space-y-2">
                  {result.lifestyleRecommendations.map((r, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-[#1F1B17]">
                      <Activity className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#C9962E]" />
                      <span>{r}</span>
                    </li>
                  ))}
                </ul>
              ...
              <ResultCard accent="#A16207" title="Dinacharya (Daily Routine)">
                <ul className="space-y-2">
                  {result.dinacharya.map((r, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-[#1F1B17]">
                      <Sun className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#A16207]" />
                      <span>{r}</span>
                    </li>
                  ))}
                </ul>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="flex items-center gap-1 text-[0.65rem] text-[#9A8F84]">
                <Sparkles className="h-3 w-3" /> Ayurvedic guidance, informational only
              </span>
            </div>
            <Disclaimer />
          </motion.div>
