...
function MacroStat({
  ...
  return (
    <motion.div
      ...
      className="rounded-xl glass-soft p-3 text-center shadow-depth"
    >
      <span
      ...
      <p className="text-lg font-bold leading-none text-[#1F1B17]">
        {value}
        <span className="ml-0.5 text-[0.6rem] font-medium text-[#9A8F84]">{unit}</span>
      </p>
...
export function FoodScan() {
  ...
  const run = async () => {
    if (!image) {
      showError("Please upload a photo of your meal first");
      return;
    }
    setLoading(true);
    setResult(null);
        ...
        >
          <div>
            ...
            <ImageUploader
              ...
              accent={accent}
              label="Drop or click to upload food photo"
            />
          </div>
          <div>
            <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-[#5C544D]">
          ...
          >
            <ResultCard accent={accent} title="Identified Food">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="flex-1">
                  ...
                  <p className="font-serif text-xl font-semibold text-[#1F1B17]">
                    {result.identifiedFood}
                  </p>
                  {result.portionEstimate && (
                    <p className="mt-1 text-xs text-[#5C544D]">
                ...
                <div
                  className="rounded-2xl px-4 py-3 text-center shadow-depth"
            ...
            <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4">
              <MacroStat
                icon={Beef}
                label="Protein"
                value={result.macros?.protein_g ?? 0}
                unit="g"
                accent="#B8860B"
              />
              <MacroStat
            ...
            {typeof result.macros?.sugar_g === "number" && (
              <div className="flex items-center gap-2 rounded-xl glass-chip px-3 py-2 text-xs text-[#5C544D]">
                ...
                <span>
                  Estimated sugar:{" "}
                  <strong className="font-semibold text-[#1F1B17]">
                    {result.macros.sugar_g} g
                  </strong>
                </span>
              </div>
            )}
                ...
                <ul className="space-y-1.5">
                  {result.ingredients.map((ing, i) => (
                    <motion.li
                      ...
                      className="flex items-center justify-between gap-2 rounded-lg bg-[#FAF7F2]/60 px-3 py-2"
                    >
                      <span className="flex items-center gap-2 text-sm text-[#1F1B17]">
                        <Apple className="h-3 w-3" style={{ color: accent }} />
                        {ing.name}
                      </span>
                      <span className="text-[0.7rem] font-medium text-[#9A8F84]">
            ...
            <ResultCard accent={accent} title="Health Score">
              <div className="flex items-center gap-4">
                <div
                  className="grid h-16 w-16 shrink-0 place-items-center rounded-full shadow-depth"
                  style={{
                    background: `linear-gradient(135deg, ${sc.bg}, ${sc.bg})`,
                    border: `2px solid ${sc.text}`,
                  }}
                >
                  <div className="text-center">
                    <p className="text-xl font-bold leading-none" style={{ color: sc.text }}>
                      {score}
                    </p>
                    <p
                ...
                <div className="flex-1">
                  <div className="mb-1.5 h-2 w-full overflow-hidden rounded-full bg-[#EFE9E0]">
                    <motion.div
                      ...
                      style={{ background: `linear-gradient(90deg, ${sc.text}, ${sc.text}cc)` }}
                    />
                  </div>
                  {result.healthNotes?.length > 0 && (
                    <ul className="space-y-1">
                      {result.healthNotes.map((n, i) => (
                        <li
                          key={i}
                          className="flex items-start gap-1.5 text-[0.7rem] text-[#5C544D]"
                        >
                          <Flame className="mt-0.5 h-3 w-3 shrink-0" style={{ color: sc.text }} />
                          <span>{n}</span>
                        </li>
                      ))}
                    </ul>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="flex items-center gap-1 text-[0.65rem] text-[#9A8F84]">
                ...
                {result.disclaimer || "Nutrition values are estimates"}
              </span>
            </div>
            <Disclaimer />
          </motion.div>
