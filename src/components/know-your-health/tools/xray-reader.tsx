...
export function XrayReader() {
  ...
  const run = async () => {
    if (!image) {
      showError("Please upload an X-ray image first");
      return;
    }
    if (!bodyPart) {
      showError("Please select the body part");
      return;
    }
    setLoading(true);
    setResult(null);
        ...
        >
          <div>
            ...
            <ImageUploader
              ...
              accent={accent}
              label="Drop or click to upload X-ray"
            />
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="flex items-end">
              <div className="rounded-xl glass-chip px-3 py-2 text-[0.65rem] leading-snug text-[#9A8F84]">
                <ImageIcon className="mb-1 h-3 w-3 text-[#7A9A7B]" /> Educational analysis only —
                not a radiologist's report.
              </div>
            </div>
          </div>
          <div>
            ...
            <ToolTextarea
              ...
              placeholder="e.g. 45-year-old male with persistent cough for 3 weeks, smoker…"
              rows={3}
            />
          </div>
          <div className="flex items-center gap-3">
            <RunButton
              onClick={run}
              loading={loading}
              disabled={!image || !bodyPart}
              accent={accent}
              label="Analyze X-ray with AI"
            />
          ...
          >
            {result.requiresRadiologistReview && (
              <motion.div
                ...
                className="flex items-start gap-3 rounded-2xl p-3.5 shadow-depth"
                style={{ background: "linear-gradient(135deg, #A1620718, #A1620708)" }}
              >
                <ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-[#8A5A04]" />
                <div>
                  <p className="text-sm font-semibold text-[#7A4A3A]">
                    A qualified radiologist should review this image
                  </p>
                  <p className="mt-0.5 text-[0.7rem] text-[#8A5A04]">
                    This AI overview is educational. It does not replace a formal radiology report.
                  </p>
                </div>
              </motion.div>
            )}
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="flex items-center gap-1 text-[0.65rem] text-[#9A8F84]">
                ...
                {result.disclaimer || "Educational analysis — not a radiology report"}
              </span>
            </div>
            <Disclaimer />
          </motion.div>
