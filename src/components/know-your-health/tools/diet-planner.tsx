...
const SWAPS: { condition: string; swap: string; bg: string; text: string }[] = [
  {
    ...
    swap: "white rice → millets / whole-wheat roti — portion first",
    bg: "#C9962E15",
    text: "#B8893D",
  },
  {
    condition: "Blood pressure",
    swap: "pickle + papad daily → twice a week; salt added at table only",
    bg: "#A1620715",
    text: "#8A5A04",
  },
  {
    condition: "Fatty liver",
    swap: "sugary chai / mithai daily → unsweetened chai, weekend mithai",
    bg: "#A1620715",
    text: "#8A5A04",
  },
  {
    condition: "Anaemia",
    swap: "chai with meals → chai an hour before / after; add vitamin-C foods with meals",
    bg: "#7A9A7B15",
    text: "#4A6A4B",
  },
  {
    condition: "Cholesterol",
    swap: "fried snacks → roasted chana / makhana",
    bg: "#9DB89E15",
    text: "#5A7A5B",
  },
];
...
export function DietPlanner() {
  ...
  const run = async () => {
    if (!f.age || !f.weightKg || !f.heightCm || !f.calorieTarget) {
        ...
        >
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-[0.65rem] text-[#9A8F84]">
              Indian dishes · grocery list included
            </span>
          </div>
          <Disclaimer />
        </motion.div>
          ...
          >
            <div className="grid gap-3 sm:grid-cols-3">
              ...
              <StatCard
                ...
                label="Hydration"
                value={result.hydrationTarget || "2.5+ L"}
                accent="#7A9A7B"
              />
            </div>
            ...
            <ResultCard accent={accent} title="Macro Split">
              <div className="grid grid-cols-3 gap-3">
                {(
                  [
                    ["Protein", result.macroSplit?.protein, "#A16207"],
                    ["Carbs", result.macroSplit?.carbs, "#C9962E"],
                    ["Fat", result.macroSplit?.fat, "#9DB89E"],
                  ] as const
                ).map(([label, g, c]) => (
                  <div key={label} className="rounded-xl bg-[#FAF7F2]/60 p-2.5 text-center">
                    <p className="font-serif text-xl font-bold text-[#1F1B17]">
                      {g || 0}
                      <span className="ml-0.5 text-[0.6rem] text-[#9A8F84]">g</span>
                    </p>
                    <p className="text-[0.6rem] uppercase tracking-wider" style={{ color: c }}>
                      {label}
                    </p>
                  </div>
                ))}
              ...
              <div className="space-y-3">
                {result.mealPlan?.map((d, i) => (
                  <motion.div
                    ...
                    className="rounded-xl bg-[#FAF7F2]/60 p-3"
                  >
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  {result.groceryList.map((c, i) => (
                    <div key={i} className="rounded-xl bg-[#FAF7F2]/60 p-2.5">
                      <p className="mb-1.5 flex items-center gap-1.5 text-[0.65rem] font-semibold uppercase tracking-wider text-[#5A7A5B]">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {c.items?.map((it, j) => (
                          <span
                            key={j}
                            className="rounded-full glass-chip px-2 py-0.5 text-[0.6rem] text-[#5C544D]"
                          >
                            {it}
                          </span>
                        ))}
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="flex items-center gap-1 text-[0.65rem] text-[#9A8F84]">
                <UtensilsCrossed className="h-3 w-3" /> Personalised plan — adjust portions to your
                needs
              </span>
            </div>
            <Disclaimer />
...
function TherapeuticSwaps() {
  return (
    <ResultCard accent="#7A9A7B" title="Therapeutic swaps — Indian kitchens">
      <p className="mb-2.5 text-[0.65rem] italic text-[#9A8F84]">
        General guidance — a dietitian personalises this for you.
      </p>
      <div className="space-y-1.5">
        {SWAPS.map((s) => {
          ...
          return (
            <div
              key={s.condition}
              className="flex items-start gap-2 rounded-lg bg-[#FAF7F2]/60 p-2"
            >
              <span
              ...
              <p className="text-xs leading-relaxed text-[#5C544D]">
                ...
                <span className="font-medium text-[#1F1B17]">{to?.trim()}</span>
              </p>
            </div>
          );
        })}
...
function MealRow({ label, meal, accent }: { label: string; meal: Meal; accent: string }) {
  ...
  return (
    <div className="rounded-lg bg-white/40 p-2">
      <p className="text-[0.6rem] font-semibold uppercase tracking-wider" style={{ color: accent }}>
        {label}
      </p>
      <p className="text-xs font-medium text-[#1F1B17]">{meal.meal}</p>
      <p className="mt-0.5 text-[0.6rem] text-[#9A8F84]">
