      ...
      {!result && !loading && (
        ...
        >
          <div className="rounded-2xl glass-soft p-4 shadow-depth">
            <div className="mb-3 flex items-center justify-between">
              <p className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-[#5C544D]">
                <Heart className="h-3.5 w-3.5" aria-hidden="true" /> PHQ-9 · Depression screen
              </p>
            ...
            <div className="space-y-3">
              {PHQ9_Q.map((q, i) => (
                <div key={i} className="rounded-xl bg-[#FAF7F2]/60 p-2.5">
                  <p className="mb-2 text-xs text-[#1F1B17]">
                    {i + 1}. {q}
                  </p>
                  <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-4">
                    {OPTIONS.map((o) => (
                      <button
                        key={o.v}
                        onClick={() => setP(i, o.v)}
                        className={`rounded-lg px-2 py-1.5 text-[0.65rem] font-medium transition-all ${phq9[i] === o.v ? "text-white shadow-depth" : "glass-chip text-[#5C544D] hover:scale-[1.02]"}`}
                        style={
                          phq9[i] === o.v
          ...
          <div className="rounded-2xl glass-soft p-4 shadow-depth">
            <div className="mb-3 flex items-center justify-between">
              <p className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-[#5C544D]">
                <Wind className="h-3.5 w-3.5" /> GAD-7 · Anxiety screen
              </p>
              <span className="rounded-full glass-chip px-2.5 py-0.5 text-[0.65rem] font-medium text-[#5A7A5B]">
                Score: {gad7Total}/21
              </span>
                  ...
                  <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-4">
                    {OPTIONS.map((o) => (
                      <button
                        key={o.v}
                        onClick={() => setG(i, o.v)}
                        className={`rounded-lg px-2 py-1.5 text-[0.65rem] font-medium transition-all ${gad7[i] === o.v ? "text-white shadow-depth" : "glass-chip text-[#5C544D] hover:scale-[1.02]"}`}
                        style={
                          gad7[i] === o.v
                            ? { background: `linear-gradient(135deg, #7A9A7B, #7A9A7Bcc)` }
                            : {}
                        }
                      >
                        {o.label}
          ...
          <div className="rounded-2xl glass-soft p-4 shadow-depth">
            ...
            <textarea
              ...
              placeholder="Anything on your mind, recent events, what you're feeling…"
            />
          </div>
        ...
        {result && (
          ...
          >
                ...
                <ul className="space-y-2">
                  {result.groundingExercises.map((g, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-[#1F1B17]">
                      <Wind className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#9DB89E]" />
                      <span>{g}</span>
                    </li>
                  ))}
                </ul>
            ...
            <ResultCard accent="#7A9A7B" title="What helps next">
              <p className="mb-3 text-[0.65rem] text-[#9A8F84]">
                Small, evidence-informed starters — pick one and keep it easy.
              </p>
              <ul className="space-y-2">
                <li className="flex items-start gap-2 text-sm text-[#1F1B17]">
                  <Moon className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#5A7A5B]" />
                  <span>
                    Keep a <strong>regular sleep window</strong> — same bedtime and wake-up time,
                    even on weekends. Sleep is the fastest lever on mood and worry.
                  </span>
                </li>
                <li className="flex items-start gap-2 text-sm text-[#1F1B17]">
                  <Sun className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#5A7A5B]" />
                  <span>
                    Get <strong>20–30 minutes of movement</strong> each day — a brisk walk in
                    morning light counts. Daylight plus movement reliably lifts mood.
                  </span>
                </li>
                <li className="flex items-start gap-2 text-sm text-[#1F1B17]">
                  <MessageCircle className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#5A7A5B]" />
                  <span>
                    <strong>Tell one trusted person</strong> how you've been feeling — a friend,
                    family member, or a counsellor (Tele-MANAS 14416 is free and 24×7).
                  </span>
                </li>
              </ul>
              <div className="mt-3 flex items-start gap-2.5 rounded-xl bg-[#A1620715] p-3">
                ...
                <p className="text-xs leading-relaxed text-[#1F1B17]">
                  <strong className="text-[#7A4A3A]">When to reach out now:</strong> if you have
                  thoughts of harming yourself or feel unsafe, call{" "}
                  <a
                    href="tel:14416"
                    className="font-semibold text-[#8A5A04] underline underline-offset-2"
                  >
                    Tele-MANAS 14416
                  </a>{" "}
                  (also 1-800-891-4416) right away (free, 24×7) or dial{" "}
                  <a
                    href="tel:108"
                    className="font-semibold text-[#8A5A04] underline underline-offset-2"
                  >
                  ...
                  in an emergency. Reaching out early is a strength — you don't have to wait.
                </p>
              </div>
            </ResultCard>
            ...
            <ResultCard accent="#B8860B" title="Crisis Resources — Available Now">
              <div className="grid gap-2 sm:grid-cols-3">
                {(result.crisisResources?.length
                  ? result.crisisResources
                  : [
                      {
                        name: "Tele-MANAS (Govt of India)",
                        phone: "14416",
                        hours: "24x7 · Free · All Indian languages",
                      },
                  ...
                  <a
                    key={i}
                    href={`tel:${c.phone}`}
                    className="block rounded-xl bg-[#FAF7F2]/60 p-3 transition-all hover:scale-[1.02]"
                  >
                    <p className="flex items-center gap-1 text-xs font-semibold text-[#1F1B17]">
                      <Phone className="h-3 w-3 text-[#8A5A04]" /> {c.name}
                    </p>
              ...
              <p className="mt-2 text-[0.65rem] text-[#9A8F84]">
                If you're in immediate danger, please call Tele-MANAS 14416 (free, 24×7) or 112
                (India emergency), or go to your nearest hospital.
              </p>
            </ResultCard>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="flex items-center gap-1 text-[0.65rem] text-[#9A8F84]">
                <Activity className="h-3 w-3" /> Not a diagnosis — please consult a mental health
                professional
              </span>
            </div>
            <Disclaimer />
    ...
    >
      ...
      >
        <div>
          ...
          <p className="text-[0.55rem] opacity-90">{subtitle}</p>
        </div>
      ...
      <div className="p-3">
        <p className="font-serif text-2xl font-bold text-[#1F1B17]">
          {score?.score || 0}
          <span className="ml-1 text-[0.6rem] text-[#9A8F84]">/ {label === "PHQ-9" ? 27 : 21}</span>
        </p>
        <p className="text-[0.65rem] uppercase tracking-wider" style={{ color: accent }}>
          {score?.level || "—"}
        </p>
