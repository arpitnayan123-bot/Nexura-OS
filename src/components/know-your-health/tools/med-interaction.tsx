...
export function MedInteraction() {
  ...
  const updateCond = (i: number, v: string) => {
    const n = [...conditions];
    n[i] = v;
    setConditions(n);
  };
  ...
  const suggestions = useMemo(() => {
    ...
    return INDIAN_BRANDS.filter((b) => b.toLowerCase().includes(q) && !meds.includes(b)).slice(
      0,
      8,
    );
  }, [activeIdx, meds]);
  ...
  const run = async () => {
    ...
    if (cleanMeds.length < 2) {
      showError("Add at least 2 medications to check interactions");
      return;
    }
        ...
        >
          <div className="rounded-2xl glass-soft p-4 shadow-depth">
            <div className="mb-2 flex items-center justify-between">
              <p className="text-xs font-semibold uppercase tracking-wider text-[#5C544D]">
                Medications ({meds.filter((m) => m.trim()).length})
              </p>
              <button
                onClick={addMed}
              ...
              {meds.map((m, i) => (
                <div key={i} className="relative">
                  <div className="flex items-center gap-2">
                    <span className="grid h-10 w-9 shrink-0 place-items-center rounded-lg glass-chip">
                      <Pill className="h-3.5 w-3.5" style={{ color: accent }} />
                    </span>
                    <input
                      value={m}
                      onChange={(e) => {
                        updateMed(i, e.target.value);
                        setActiveIdx(i);
                      }}
                    ...
                    <button
                      ...
                      className="grid h-10 w-9 shrink-0 place-items-center rounded-lg glass-chip text-[#8A5A04] disabled:opacity-30 hover:bg-[#B8860B]/10"
                    >
          ...
          <div className="grid grid-cols-2 gap-3 rounded-2xl glass-soft p-4 shadow-depth sm:grid-cols-4">
            ...
            <div>
              ...
              <select value={liver} onChange={(e) => setLiver(e.target.value)} className={inputCls}>
                <option>Normal</option>
                <option>Mild impairment</option>
                <option>Moderate</option>
                <option>Severe</option>
          ...
          <div className="flex items-center gap-3">
            <RunButton
              onClick={run}
              loading={loading}
              disabled={meds.filter((m) => m.trim()).length < 2}
              accent={accent}
              label="Check interactions"
            />
            <span className="text-[0.65rem] text-[#9A8F84]">
              Indian brand-aware · drug-drug & drug-condition
            </span>
          </div>
          <Disclaimer />
        </motion.div>
          ...
          >
              ...
              <div className="flex items-center gap-2">
                {result.requiresPharmacistConsult ? (
                  <span
                    ...
                    style={{ background: "#A1620715", color: "#8A5A04" }}
                  >
                    ⚠️ Pharmacist consult advised
                  </span>
                ) : (
                  <span
                    className="rounded-full px-2.5 py-0.5 text-[0.6rem] font-bold uppercase tracking-wider"
                  ...
                  >
                    <ShieldCheck className="mr-1 inline h-3 w-3" /> No major concerns
                  </span>
                )}
                <span className="rounded-full glass-chip px-2.5 py-0.5 text-[0.65rem] font-medium text-[#5C544D]">
                  {result.interactions?.length || 0} interactions found
                </span>
              </div>
            ...
            {result.interactions?.length > 0 ? (
                  ...
                  .map((it, i) => (
                    ...
                    >
                      <div
                        ...
                        style={{
                          background: `linear-gradient(135deg, ${SEV_COLOR[it.severity]}15, ${SEV_COLOR[it.severity]}08)`,
                        }}
                      >
                        <div className="flex items-center gap-2">
                          <AlertTriangle
                            className="h-3.5 w-3.5"
                            style={{ color: SEV_COLOR[it.severity] }}
                          />
                          <p className="text-sm font-semibold text-[#1F1B17]">
                            {it.drug1} <span className="text-[#9A8F84]">+</span> {it.drug2}
                          </p>
                        </div>
                      ...
                      <div className="space-y-1.5 px-4 py-2.5">
                        <p className="text-xs">
                          <span className="font-medium text-[#5C544D]">Mechanism:</span>{" "}
                          <span className="text-[#1F1B17]">{it.mechanism}</span>
                        </p>
                        <p className="text-xs">
                          <span className="font-medium text-[#5C544D]">Effect:</span>{" "}
                          <span className="text-[#1F1B17]">{it.clinicalEffect}</span>
                        </p>
                        <p className="text-xs">
                          <span className="font-medium text-[#5C544D]">Action:</span>{" "}
                          <span className="text-[#1F1B17]">{it.recommendation}</span>
                        </p>
                      </div>
                    </motion.div>
                  ))}
              ...
              <ResultCard accent="#9DB89E" title="No Interactions Found">
                <p className="text-sm text-[#1F1B17]">
                  No clinically significant drug-drug interactions detected for the medicines you
                  listed. Continue as prescribed and consult your doctor if you notice side effects.
                </p>
              </ResultCard>
            )}
                ...
                <ul className="space-y-2">
                  {result.conditionWarnings.map((c, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-[#1F1B17]">
                      <AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#C9962E]" />
                      <span>{c}</span>
                    </li>
                  ))}
                </ul>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="flex items-center gap-1 text-[0.65rem] text-[#9A8F84]">
                <Activity className="h-3 w-3" /> Never stop prescribed medication without your
                doctor's advice
              </span>
            </div>
            <Disclaimer />
