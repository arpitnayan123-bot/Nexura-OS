...
export function DiabetesCare() {
  ...
  const run = async () => {
    if (!f.fastingSugar && !f.postMealSugar && !f.hba1c) {
      showError("Enter at least one sugar value or HbA1c");
      return;
    }
    setLoading(true);
    setResult(null);
    try {
      const res = await fetch("/api/know-your-health/diabetes-care", {
        ...
        body: JSON.stringify({
          ...f,
          fastingSugar: Number(f.fastingSugar) || 0,
          postMealSugar: Number(f.postMealSugar) || 0,
          hba1c: f.hba1c ? Number(f.hba1c) : undefined,
        }),
      });
      if (!res.ok) {
        ...
        >
          <div className="grid grid-cols-1 gap-3 rounded-2xl glass-soft p-4 shadow-depth sm:grid-cols-3">
            ...
            <div>
              ...
              <input
                type="number"
                value={f.postMealSugar}
                onChange={(e) => set("postMealSugar", e.target.value)}
                className={inputCls}
                placeholder="<180"
              />
            </div>
          ...
          <div className="grid grid-cols-1 gap-3 rounded-2xl glass-soft p-4 shadow-depth sm:grid-cols-2">
            <div>
              ...
              <input
                value={f.lastMeal}
                onChange={(e) => set("lastMeal", e.target.value)}
                className={inputCls}
                placeholder="2 roti, dal, sabzi"
              />
            </div>
            <div>
              <label className="mb-1 block text-[0.65rem] font-semibold uppercase tracking-wider text-[#9A8F84]">
            ...
            <div>
              ...
              <input
                ...
                className={inputCls}
                placeholder="felt shaky in afternoon"
              />
            </div>
          </div>
          ...
          >
            <ResultCard accent={accent} title="Diabetes Status">
              <div className="flex flex-col items-center gap-3 py-3">
                <div
                  className="relative grid h-24 w-24 place-items-center rounded-full shadow-depth"
                  style={{
                    background: `linear-gradient(135deg, ${statusOf(result.status).accent}, ${statusOf(result.status).accent}cc)`,
                  }}
                >
              ...
              <ResultCard accent="#A16207" title="Indian Diet Tips">
                <ul className="space-y-2">
                  {result.indianDietTips.map((r, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-[#1F1B17]">
                      <Utensils className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#A16207]" />
                      <span>{r}</span>
                    </li>
                  ))}
                </ul>
