...
const OPTIONS_PHYSICAL = [
  "Sedentary (desk job, no exercise)",
  "Light (occasional walks)",
  "Moderate (30 min most days)",
...
const OPTIONS_DIET = [
  "Mostly junk / fried",
  "Mixed veg / non-veg",
  "Mostly vegetarian home-cooked",
  "Balanced with fruits & veg",
  "Strict healthy / Mediterranean",
];
...
export function DiseaseRisk() {
  ...
  const [f, setF] = useState({
    ...
    smoker: false,
    alcohol: "Occasional (1-2/week)",
    physicalActivity: "Light (occasional walks)",
  ...
  const run = async () => {
    const payload = {
      ...
      waistCm: Number(f.waistCm),
      bpSystolic: Number(f.bpSystolic),
      bpDiastolic: Number(f.bpDiastolic),
    ...
    if (!payload.age || !payload.heightCm || !payload.weightKg) {
      showError("Please fill age, height and weight");
      return;
    }
    setLoading(true);
    setResult(null);
      ...
      {!result && !loading && (
        ...
        >
          <div className="grid grid-cols-2 gap-3 rounded-2xl glass-soft p-4 shadow-depth sm:grid-cols-4">
            ...
            <Field label="Fasting Sugar (mg/dL)">
              <input
                type="number"
                value={f.fastingSugar}
                onChange={(e) => set("fastingSugar", e.target.value)}
                placeholder="optional"
                className={inputCls}
              />
            </Field>
          ...
          <div className="grid grid-cols-1 gap-3 rounded-2xl glass-soft p-4 shadow-depth sm:grid-cols-2">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <label className="flex items-center gap-2 text-xs text-[#1F1B17]">
                <input
                  type="checkbox"
                  checked={f.familyHistoryDiabetes}
                  onChange={(e) => set("familyHistoryDiabetes", e.target.checked)}
                  className="accent-[#B8860B]"
                />{" "}
              ...
              <label className="flex items-center gap-2 text-xs text-[#1F1B17]">
                <input
                  type="checkbox"
                  checked={f.familyHistoryHeart}
                  onChange={(e) => set("familyHistoryHeart", e.target.checked)}
                  className="accent-[#B8860B]"
                />{" "}
          ...
          <div className="flex items-center gap-3">
            <RunButton
              ...
              disabled={loading}
              accent={accent}
              label="Estimate 10-year risk"
            />
            <span className="text-[0.65rem] text-[#9A8F84]">
              ICMR-INDIAB · ASCVD · KFRE · Indian context
            </span>
          </div>
          <Disclaimer />
        </motion.div>
          ...
          >
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              <RiskCard
                ...
                risk={result.diabetesRisk}
                accent="#B8860B"
                icon={<Droplet className="h-4 w-4" />}
              />
              <RiskCard
                label="Cardiovascular (CVD)"
                risk={result.cvdRisk}
                accent="#A16207"
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="flex items-center gap-1 text-[0.65rem] text-[#9A8F84]">
                <Activity className="h-3 w-3" /> Risk estimate, not a diagnosis
              </span>
            </div>
            <Disclaimer />
          </motion.div>
    ...
    >
      ...
      <div className="p-4">
        <div className="mb-2 flex items-end justify-between">
          ...
          <SeverityBadge
            level={risk.level === "low" ? "low" : risk.level === "moderate" ? "moderate" : "high"}
          />
        </div>
