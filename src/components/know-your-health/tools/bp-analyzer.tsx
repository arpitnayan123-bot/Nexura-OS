...
export function BpAnalyzer() {
  ...
  const [readings, setReadings] = useState<Reading[]>([
    { systolic: "", diastolic: "", date: "", time: "", arm: "Left", position: "Sitting" },
  ]);
  ...
  const update = (i: number, k: keyof Reading, v: string) =>
    setReadings(readings.map((r, idx) => (idx === i ? { ...r, [k]: v } : r)));

  const run = async () => {
    const clean = readings
      .filter((r) => r.systolic && r.diastolic)
      .map((r) => ({ ...r, systolic: Number(r.systolic), diastolic: Number(r.diastolic) }));
    if (clean.length === 0) {
      showError("Add at least one valid BP reading");
      return;
    }
    setLoading(true);
    setResult(null);
        ...
        >
          ...
          <div className="rounded-2xl glass-soft p-4 shadow-depth">
            ...
            <div className="space-y-2">
              {readings.map((r, i) => (
                <div
                  key={i}
                  className="grid grid-cols-2 gap-2 rounded-xl bg-[#FAF7F2]/60 p-2 sm:grid-cols-[0.8fr_0.8fr_0.8fr_0.8fr_0.6fr_0.7fr_auto]"
                >
                  <input
                    type="number"
                    value={r.systolic}
                    onChange={(e) => update(i, "systolic", e.target.value)}
                    placeholder="Sys"
                    className={inputCls}
                  />
                  <input
                    type="number"
                    value={r.diastolic}
                    onChange={(e) => update(i, "diastolic", e.target.value)}
                    placeholder="Dia"
                    className={inputCls}
                  />
                  <input
                  ...
                  <select
                    value={r.arm}
                    onChange={(e) => update(i, "arm", e.target.value)}
                    className={inputCls}
                  >
                    <option>Left</option>
                    <option>Right</option>
                  ...
                  <button
                    ...
                    className="grid h-10 w-9 place-items-center rounded-lg glass-chip text-[#8A5A04] disabled:opacity-30 hover:bg-[#B8860B]/10"
                  >
          ...
          <div className="flex items-center gap-3">
            <RunButton
              onClick={run}
              loading={loading}
              disabled={!readings.some((r) => r.systolic && r.diastolic)}
              accent={accent}
              label="Classify & analyse"
            />
          ...
          >
            <ResultCard accent={accent} title="BP Classification">
              <div className="flex flex-col items-center py-3">
                <motion.div
                  ...
                  style={{
                    background: `linear-gradient(135deg, ${CLASS_COLOR[result.classification] || accent}, ${CLASS_COLOR[result.classification] || accent}cc)`,
                  }}
                >
                  <Droplet className="h-9 w-9 text-white" />
                </motion.div>
                <p className="mt-2 font-serif text-xl font-bold text-center text-[#1F1B17]">
                  {result.classification}
                </p>
                <p className="mt-1 font-serif text-3xl font-bold text-[#1F1B17]">
                  {result.averageSystolic}
                  <span className="text-[#9A8F84]">/</span>
                  {result.averageDiastolic}
                  <span className="ml-1 text-sm text-[#9A8F84]">mmHg</span>
                </p>
                <p className="text-[0.65rem] text-[#9A8F84]">
                  Pulse pressure: {result.pulsePressure} mmHg
                </p>
                {result.whiteCoatSuspected && (
                  <span
                    className="mt-2 rounded-full px-2.5 py-0.5 text-[0.6rem] font-bold uppercase tracking-wider"
                    style={{ background: "#C9962E15", color: "#B8893D" }}
                  >
                    ⚠️ White-coat suspected
                  </span>
                )}
              </div>
            </ResultCard>
              ...
              <ResultCard accent="#9DB89E" title="Pattern & Trend">
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <TrendingUp className="h-3.5 w-3.5 text-[#9DB89E]" />
                    <span className="text-xs font-semibold uppercase tracking-wider text-[#5A7A5B]">
                      Trend
                    </span>
                    <span className="rounded-full glass-chip px-2 py-0.5 text-[0.6rem] capitalize text-[#5C544D]">
                      {result.trend}
                    </span>
                  </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="flex items-center gap-1 text-[0.65rem] text-[#9A8F84]">
                <Activity className="h-3 w-3" /> Track BP at the same time daily for best results
              </span>
            </div>
            <Disclaimer />
          </motion.div>
