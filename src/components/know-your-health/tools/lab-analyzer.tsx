    ...
    <div className="rounded-2xl glass-soft p-3 shadow-depth">
      <div className="mb-2 flex items-center gap-1.5">
        ...
        <p className="text-xs font-semibold uppercase tracking-wider text-[#5C544D]">
          Quick reference — common Indian lab ranges
        </p>
      </div>
      <div className="space-y-1.5">
        {QUICK_RANGES.map((r) => (
          <div key={r.test} className="flex items-start gap-2 rounded-lg bg-[#FAF7F2]/60 p-2">
            <span
              className="mt-0.5 shrink-0 rounded-full px-2 py-0.5 text-[0.6rem] font-bold uppercase tracking-wider"
              style={{ background: "#9DB89E15", color: "#5A7A5B" }}
            >
              {r.test}
            </span>
      ...
      <p className="mt-2 text-[0.65rem] italic text-[#9A8F84]">
        Ranges vary by lab and context — always compare with your report's own reference column.
      </p>
    </div>
  );
}
...
export function LabAnalyzer() {
  ...
  const update = (i: number, k: keyof LabTest, v: string) =>
    setTests(tests.map((t, idx) => (idx === i ? { ...t, [k]: v } : t)));
      ...
      const payload =
        mode === "photo"
          ...
          : (() => {
              ...
              return clean.length ? { tests: clean } : null;
            })();
      if (!payload) {
        showError(
          mode === "photo"
            ? "Upload a photo of your lab report first"
            : "Add at least one test with name and value",
        );
        setLoading(false);
        return;
      }
  ...
  const reset = () => {
    setResult(null);
    setTests([{ name: "Hemoglobin", value: "", unit: "g/dL" }]);
    setImage(null);
  };
        ...
        >
          <div className="flex gap-2">
            {(["photo", "manual"] as const).map((m) => (
              <button
                key={m}
                onClick={() => setMode(m)}
                className={`flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-xs font-semibold transition-all ${mode === m ? "bg-[#1F1B17] text-white shadow-depth" : "glass-chip text-[#5C544D]"}`}
              >
                {m === "photo" ? (
          ...
          {mode === "photo" ? (
            <ImageUploader
              ...
              accent={accent}
              label="Snap or upload your lab report"
            />
          ) : (
            <div className="rounded-2xl glass-soft p-3 shadow-depth">
              <div className="mb-2 flex items-center justify-between">
                ...
                <button
                  onClick={addRow}
                  className="flex items-center gap-1 rounded-full glass-chip px-2.5 py-1 text-[0.65rem] font-medium text-[#5A7A5B] hover:scale-105"
                >
              ...
              <div className="space-y-2">
                {tests.map((t, i) => (
                  <div
                    key={i}
                    className="grid grid-cols-1 gap-2 sm:grid-cols-[1.4fr_1fr_0.8fr_auto]"
                  >
                    <select
                      value={t.name}
                      onChange={(e) => update(i, "name", e.target.value)}
                      className="glass-input h-10 w-full rounded-lg px-2 text-xs outline-none"
                    >
                      ...
                      {t.name && !COMMON_TESTS.find((c) => c.name === t.name) && (
                    ...
                    <button
                      ...
                      className="grid h-10 w-10 place-items-center rounded-lg glass-chip text-[#8A5A04] disabled:opacity-30 hover:bg-[#B8860B]/10"
                    >
          ...
          <div className="flex items-center gap-3">
            <RunButton
              onClick={run}
              loading={loading}
              disabled={mode === "photo" ? !image : !tests.some((t) => t.name && t.value)}
              accent={accent}
              label={mode === "photo" ? "Read report & interpret" : "Interpret against ICMR ranges"}
            />
            <span className="text-[0.65rem] text-[#9A8F84]">
              AI-processed · Indian reference ranges
            </span>
          </div>
          <QuickReference />
          <Disclaimer />
          ...
          >
            <ResultCard accent={accent} title="Overall Summary">
              <div className="mb-3 flex flex-wrap items-center gap-2">
                <span className="rounded-full glass-chip px-2.5 py-0.5 text-[0.65rem] font-semibold text-[#5C544D]">
                  {result.tests.length} tests
                </span>
                <span
                  className="rounded-full px-2.5 py-0.5 text-[0.65rem] font-bold uppercase tracking-wider"
                  style={{
                    background: result.abnormalCount > 0 ? "#A1620715" : "#9DB89E15",
                    color: result.abnormalCount > 0 ? "#8A5A04" : "#5A7A5B",
                  }}
                >
                  {result.abnormalCount} abnormal
                </span>
                {result.extractedFromImage && (
                  <span
                    className="rounded-full px-2.5 py-0.5 text-[0.65rem] font-semibold"
                    style={{ background: "#C9962E15", color: "#B8893D" }}
                  >
                    Read from photo — verify values
                  </span>
                )}
                {result.requiresDoctorFollowUp && (
                  <span
                    className="rounded-full px-2.5 py-0.5 text-[0.65rem] font-bold uppercase tracking-wider"
                    style={{ background: "#A1620720", color: "#7A4A3A" }}
                  >
                    Doctor follow-up advised
                  </span>
                )}
              </div>
              ...
              {result.tests.map((t, i) => (
                ...
                >
                  <div
                    ...
                    style={{
                      background: `linear-gradient(135deg, ${statusColor[t.status]}15, ${statusColor[t.status]}08)`,
                    }}
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-bold text-[#1F1B17]">
                        {t.value} {t.unit}
                      </span>
                      <span
                  ...
                  <div className="space-y-1.5 px-4 py-2.5">
                    <p className="text-[0.7rem] text-[#9A8F84]">
                      <span className="font-medium text-[#5C544D]">Reference:</span>{" "}
                      {t.referenceRange}
                    </p>
            ...
            <div className="flex items-center gap-3">
              ...
              {result.requiresDoctorFollowUp && (
                <span className="flex items-center gap-1 text-[0.65rem] text-[#8A5A04]">
                  <AlertCircle className="h-3 w-3" /> Please consult a physician
                </span>
              )}
            </div>
            <Disclaimer />
