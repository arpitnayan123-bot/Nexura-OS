...
export function CustomersModule() {
  ...
  return (
    <div className="space-y-4">
      <div><h1 className="font-serif text-2xl font-semibold text-white">Customer Accounts</h1><p className="text-sm text-[#6B7280]">Credit accounts &amp; outstanding · total due ₹{totalOutstanding.toLocaleString("en-IN")}</p></div>
        ...
        <div className="max-h-[65vh] overflow-auto">
          {customers.map(c => (
            <div key={c.id} className="flex flex-wrap items-center gap-3 border-b border-[#1A1D22] px-4 py-3 last:border-0 hover:bg-[#0D0F12]">
              <span className="grid h-9 w-9 place-items-center rounded-full bg-[#1E2228] text-xs font-bold text-[#6B7280]">{c.name.split(" ").map((x:string)=>x[0]).join("").slice(0,2)}</span>
              <div className="min-w-0 flex-1"><p className="text-sm font-medium text-white">{c.name}</p><p className="text-[0.65rem] text-[#6B7280]">{c.phone||"No phone"} · {c.sales.length} credit bills</p></div>
          ...
          {customers.length === 0 && <p className="py-10 text-center text-sm text-[#6B7280]">No customers found.</p>}
        </div>
      </div>
    </div>
  );
