...
export const BillingModule = memo(function BillingModule() {
  ...
  const [pendingScheduleH, setPendingScheduleH] = useState<{
    item: InventoryItem;
    batchId: string;
  } | null>(null);
  ...
  const stockBadge = (p: InventoryItem) => {
    ...
    if (p.stockStrips <= 5)
      return { label: "Low stock", color: "text-yellow-400 bg-yellow-500/10" };
  ...
  const addToCart = (item: InventoryItem, batchId?: string) => {
    ...
    if (isScheduleH) {
      setPendingScheduleH({ item, batchId: batch.id });
      return;
    }
  ...
  const commitToCart = (item: InventoryItem, batchId: string, scheduleH?: any) => {
    ...
    setCart((c) => {
      ...
      if (existing)
        return c.map((x) =>
          x.key === key
            ? { ...x, qtyStrips: x.qtyStrips + 1, scheduleH: scheduleH || x.scheduleH }
            : x,
        );
      return [
        ...c,
  ...
  const updateDiscount = (key: string, pct: number) =>
    setCart((c) =>
      c.map((x) => (x.key === key ? { ...x, discountPct: Math.max(0, Math.min(100, pct)) } : x)),
    );
  ...
  useEffect(() => {
    if (cart.length >= 3) {
      ...
      const hasNSAID2 =
        names.filter(
          (n) => n.includes("ibuprofen") || n.includes("diclofenac") || n.includes("aspirin"),
        ).length >= 2;
      const found: any[] = [];
      if (hasNSAID2)
        found.push({
      ...
      if (hasAspirin && hasNSAID)
        found.push({
          ...
          type: "Pharmacodynamic",
          severity: "moderate",
          effect: "Increased bleeding time",
        });
  ...
  const submit = async () => {
    ...
    if (missing) {
      toast.error(`${missing.name} needs Schedule H details`);
      return;
    }
    setBilling(true);
    try {
  ...
  const fetchEinvoice = async () => {
    ...
    setEinvLoading(true);
    try {
      const r = await fetch("/api/pharmacy/e-invoice", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ saleId: lastSale.id }),
      });
        ...
        JSON.stringify(
          {
            ...
            eInvoice: d.eInvoice,
            ewayBill: d.ewayBill,
            eligibleEwayBill: d.eligibleEwayBill,
          },
          null,
          2,
        ),
    ...
    <div className="space-y-4">
      ...
      <div className="relative">
        ...
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter" && results[0]) addToCart(results[0]);
          }}
        ...
        <div className="absolute right-3 top-1/2 flex -translate-y-1/2 items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl bg-[#F59E0B]/10 px-3 py-2 text-xs font-medium text-[#F59E0B] transition-colors hover:bg-[#F59E0B]/20"
            title="AI Prescription Camera"
          >
            <Camera className="h-4 w-4" /> <span className="hidden sm:inline">AI Rx</span>
          </button>
          <button
            onClick={() => toast.info("AI Substitute Finder — search an out-of-stock medicine")}
            className="grid h-9 w-9 place-items-center rounded-xl bg-[#1E2228] text-[#828894] hover:bg-[#2A2E35] hover:text-white"
            title="AI Substitute"
          >
        ...
        <AnimatePresence>
          {showResults && results.length > 0 && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -4 }}
              className="absolute z-30 mt-2 w-full overflow-hidden rounded-xl border border-[#1E2228] bg-[#111418] shadow-2xl"
            >
              {results.map((p) => {
                ...
                return (
                  <button
                    ...
                    className="flex w-full items-center gap-3 border-b border-[#1E2228] px-4 py-3 text-left transition-colors last:border-0 hover:bg-[#1A1D22]"
                  >
                    <span
                      className={cn(
                        "grid h-9 w-9 shrink-0 place-items-center rounded-lg",
                        isH ? "bg-red-500/10 text-red-400" : "bg-green-500/10 text-green-400",
                      )}
                    >
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-medium text-white">
                        {p.name}{" "}
                        {isH && (
                          <span className="ml-1 rounded bg-red-500/15 px-1 text-[0.55rem] font-bold text-red-400">
                            {p.schedule}
                          </span>
                        )}
                      </p>
                      <p className="truncate text-[0.65rem] text-[#828894]">
                        {p.genericName} · {p.brand} · {p.salts}
                      </p>
                    </div>
                    <div className="text-right">
                      ...
                      <span
                        className={cn(
                          "rounded-full px-2 py-0.5 text-[0.55rem] font-medium",
                          badge.color,
                        )}
                      >
                        {badge.label}
        ...
        {interactions.length > 0 && (
          ...
          >
            ...
            <span className="text-xs font-medium text-yellow-400">
              AI Drug Interaction detected — {interactions.length} interaction(s) found. Click to
              review.
            </span>
          </motion.button>
        )}
        ...
        <div className="rounded-2xl border border-[#1E2228] bg-[#111418] p-4">
          <div className="mb-3 flex items-center justify-between">
            ...
            {cart.length > 0 && (
              <button
                onClick={() => setCart([])}
                className="text-xs text-[#828894] hover:text-red-400"
              >
                Clear
              </button>
            )}
          ...
          {cart.length === 0 ? (
            <div className="grid h-48 place-items-center text-center">
              <div className="flex flex-col items-center gap-3">
                <span
                  className="grid h-12 w-12 place-items-center rounded-2xl border border-dashed border-[#2A2E35] bg-[#0D0F12]/60"
                  aria-hidden="true"
                >
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-[0.65rem] text-[#828894]">
                    AI Rx reading · interaction checks · GST built in
                  </p>
                </div>
              </div>
            </div>
                      ...
                      return (
                        ...
                        >
                          <td className="py-2.5 pr-2">
                            ...
                            {it.scheduleH && (
                              <p className="mt-0.5 text-[0.55rem] text-green-400">
                                ✓ Rx: {it.scheduleH.doctorName} · {it.scheduleH.patientName}
                              </p>
                            )}
                          </td>
                          <td className="px-2 py-2.5 text-center tabular-nums text-[#828894]">
                            ₹{it.mrp.toFixed(0)}
                          </td>
                          <td className="px-2 py-2.5">
                            <div className="mx-auto flex w-fit items-center gap-1">
                              <button
                                ...
                                className="grid h-6 w-6 place-items-center rounded-full bg-[#1E2228] text-[#828894] hover:bg-[#2A2E35]"
                              >
                          ...
                          <td className="px-2 py-2.5">
                            <div className="mx-auto flex w-fit items-center gap-1">
                              ...
                              <span className="w-5 text-center tabular-nums text-white">
                                {it.qtyLoose}
                              </span>
                              <button
                                onClick={() => updateQty(it.key, 1, true)}
                          ...
                          <td className="px-2 py-2.5 text-center">
                            <input
                              ...
                              onChange={(e) => updateDiscount(it.key, +e.target.value || 0)}
                              className="h-7 w-12 rounded-md bg-[#0D0F12] text-center text-xs text-white ring-1 ring-[#1E2228] outline-none focus:ring-[#F59E0B]/40"
                              placeholder="0"
                            />
                          </td>
                          <td className="px-2 py-2.5 text-right font-medium tabular-nums text-white">
                            ₹{amt.toFixed(2)}
                          </td>
                          <td className="py-2.5 pl-2">
                            <button
                              onClick={() => removeItem(it.key)}
                              className="grid h-7 w-7 place-items-center rounded-full text-[#828894] hover:bg-red-500/10 hover:text-red-400"
                            >
          ...
          <div className="mt-4 flex flex-col gap-3 border-t border-[#1E2228] pt-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-1 rounded-xl bg-[#0D0F12] p-0.5">
              {(["cash", "upi", "card", "credit"] as const).map((m) => (
                <button
                  ...
                  className={cn(
                    "rounded-lg px-2.5 py-1.5 text-xs font-medium uppercase transition-colors",
                    payMode === m ? "bg-[#F59E0B] text-black" : "text-[#828894] hover:text-white",
                  )}
                >
                  {m}
                </button>
            ...
            <button
              ...
              className="btn-gold flex items-center justify-center gap-2 rounded-xl bg-[#F59E0B] px-6 py-2.5 text-sm font-bold text-black shadow-lg shadow-[#F59E0B]/20 transition-all hover:bg-[#D97706] disabled:opacity-50"
            >
          ...
          >
            ...
            <div className="mb-3 space-y-1">
              {GST_SLABS.filter((s) => totals.slabData[s].taxable > 0).map((s) => (
                <div
                  key={s}
                  className="flex items-center justify-between rounded-lg bg-[#0D0F12] px-2.5 py-1.5"
                >
                  <span className="text-xs text-[#828894]">{s}% slab</span>
                  <div className="text-right">
                    <span className="text-xs tabular-nums text-white">
                      ₹{totals.slabData[s].taxable.toFixed(0)}
                    </span>
                    <span className="ml-2 text-[0.6rem] tabular-nums text-[#828894]">
                      CGST ₹{totals.slabData[s].cgst.toFixed(0)} · SGST ₹
                      {totals.slabData[s].sgst.toFixed(0)}
                    </span>
                  </div>
                </div>
              ))}
            ...
            <dl className="space-y-1.5 text-sm">
              <div className="flex justify-between">
                <dt className="text-[#828894]">Subtotal</dt>
                <dd className="tabular-nums text-white">₹{totals.subtotal.toFixed(2)}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-[#828894]">Discount</dt>
                <dd className="tabular-nums text-red-400">-₹{totals.discount.toFixed(2)}</dd>
              </div>
              {lastSale && (
                <div className="pt-2">
                  <button
                    onClick={fetchEinvoice}
                    disabled={einvLoading}
                    className="flex w-full items-center justify-center gap-1.5 rounded-lg border border-[#F59E0B]/30 px-3 py-2 text-xs font-semibold text-[#F59E0B] transition hover:bg-[#F59E0B]/10 disabled:opacity-50"
                  >
                    {einvLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : null}
                    Generate e-Invoice JSON — {lastSale.invoiceNo}
                  </button>
                  {eInvoice && (
                    <pre className="nxf-scroll mt-2 max-h-48 overflow-auto rounded-lg bg-[#0D0F12] p-2.5 text-[10px] leading-relaxed text-[#9CA3AF]">
                      {eInvoice}
                    </pre>
                  )}
                </div>
              ...
              <div className="flex justify-between">
                <dt className="text-[#828894]">CGST</dt>
                <dd className="tabular-nums text-white">₹{totals.totalCgst.toFixed(2)}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-[#828894]">SGST</dt>
                <dd className="tabular-nums text-white">₹{totals.totalSgst.toFixed(2)}</dd>
              </div>
              <div className="flex justify-between text-xs">
                <dt className="text-[#828894]">Round off</dt>
                <dd className="tabular-nums text-[#828894]">
      ...
      <BatchPanel
        ...
        onSelect={(item, batchId) => {
          addToCart(item, batchId);
          setBatchPanel(null);
        }}
      />
      ...
      <ScheduleHModal
        pending={pendingScheduleH}
        onClose={() => setPendingScheduleH(null)}
        onConfirm={(d) => {
          if (pendingScheduleH) commitToCart(pendingScheduleH.item, pendingScheduleH.batchId, d);
          setPendingScheduleH(null);
        }}
      />
      ...
      <PrescriptionModal
        ...
        onMapped={(items) => {
          ...
          toast.success(
            `${items.filter((i) => i.matched).length} medicines added from prescription`,
          );
        }}
        inventory={inventory}
      />
...
function BatchPanel({
  ...
  onClose: () => void;
  onSelect: (item: InventoryItem, batchId: string) => void;
}) {
  return (
    <AnimatePresence>
      {item && (
        ...
        >
          <motion.div
            ...
            className="w-full max-w-md rounded-t-2xl border border-[#1E2228] bg-[#111418] p-5 shadow-2xl sm:rounded-2xl"
          >
            <div className="mb-3 flex items-center justify-between">
              <div>
                <h3 className="font-serif text-base font-semibold text-white">{item.name}</h3>
                <p className="text-[0.65rem] text-[#828894]">
                  {item.genericName} · {item.salts} · {item.brand}
                </p>
              </div>
              <button
                onClick={onClose}
            ...
            <p className="mb-3 text-[0.6rem] uppercase tracking-wider text-[#828894]">
            ...
            <div className="space-y-2">
              {[...item.batches]
                ...
                .map((b) => {
                  ...
                  const urgency =
                    daysToExp <= 30
                      ? { label: "Expiring <30d", color: "text-red-400 bg-red-500/10" }
                      : daysToExp <= 90
                        ? { label: "Expiring <90d", color: "text-yellow-400 bg-yellow-500/10" }
                        : { label: "Good", color: "text-green-400 bg-green-500/10" };
                  const isOut = b.stockStrips === 0;
                  return (
                    <div
                      key={b.id}
                      className="flex items-center gap-3 rounded-xl border border-[#1E2228] bg-[#0D0F12] p-3"
                    >
                      <div className="flex-1">
                        <p className="text-sm font-medium text-white">Batch: {b.batchNo}</p>
                        <p className="text-[0.65rem] text-[#828894]">
                          MRP ₹{b.mrp.toFixed(0)} · Stock: {b.stockStrips} strips
                        </p>
                        <p className="text-[0.65rem] text-[#828894]">
                          Exp:{" "}
                      ...
                      {isOut ? (
                        <button
                          onClick={() => {
                            toast.info(
                              "Promise Order saved — customer will be notified on WhatsApp when stock arrives",
                            );
                            onClose();
                          }}
                          className="rounded-full bg-[#1E2228] px-3 py-1.5 text-[0.65rem] font-medium text-[#F59E0B] hover:bg-[#2A2E35]"
                        >
                          Promise Order
                        </button>
                      ) : (
                        <button
                          onClick={() => onSelect(item, b.id)}
                          className="rounded-full bg-[#F59E0B] px-3 py-1.5 text-[0.65rem] font-bold text-black hover:bg-[#D97706]"
                        >
                          Add
                        </button>
                      )}
...
function ScheduleHModal({
  pending,
  onClose,
  onConfirm,
}: {
  pending: { item: InventoryItem; batchId: string } | null;
  onClose: () => void;
      ...
      {pending && (
        ...
        >
          <motion.div
            ...
            onClick={(e) => e.stopPropagation()}
            className="relative w-full max-w-md overflow-hidden rounded-2xl border-2 border-red-500/30 bg-[#111418] shadow-2xl"
          >
            ...
            <div className="relative flex items-center justify-between border-b border-[#1E2228] bg-red-500/5 px-5 py-4">
              <div className="flex items-center gap-2.5">
                <span className="grid h-9 w-9 place-items-center rounded-xl bg-red-500/15 text-red-400">
                  <ShieldAlert className="h-4 w-4" />
                </span>
                <div>
                  <h3 className="font-serif text-base font-semibold text-white">
                    Schedule {pending.item.schedule} — Mandatory
                  </h3>
                  <p className="text-[0.65rem] text-[#828894]">
                    Drug Inspector compliance — Drugs &amp; Cosmetics Rules 1945
                  </p>
                </div>
              </div>
              <button
...
function ScheduleHForm({
  pending,
  onClose,
  onConfirm,
}: {
  pending: { item: InventoryItem; batchId: string };
  onClose: () => void;
  onConfirm: (d: any) => void;
}) {
  ...
  return (
    <>
      <div className="space-y-3 p-5">
        <div className="rounded-lg bg-red-500/5 px-3 py-2 text-xs">
          <span className="text-[#828894]">Adding: </span>
          <strong className="text-white">{pending.item.name}</strong>
          <span className="ml-1 rounded bg-red-500/15 px-1 text-[0.55rem] font-bold text-red-400">
            {pending.item.schedule}
          </span>
        </div>
        <Field
          label="Patient name *"
          value={d.patientName}
          onChange={(v) => setD({ ...d, patientName: v })}
        />
        <Field
          label="Patient address"
          value={d.patientAddress}
          onChange={(v) => setD({ ...d, patientAddress: v })}
        />
        <Field
          label="Prescribing doctor name *"
          value={d.doctorName}
          onChange={(v) => setD({ ...d, doctorName: v })}
        />
        <div className="grid grid-cols-2 gap-3">
          <Field
            label="Doctor reg. no. *"
            value={d.doctorRegNo}
            onChange={(v) => setD({ ...d, doctorRegNo: v })}
          />
          <Field
            label="Rx date *"
            value={d.prescriptionDate}
            onChange={(v) => setD({ ...d, prescriptionDate: v })}
            type="date"
          />
        </div>
      </div>
      <div className="flex gap-2 border-t border-[#1E2228] p-4">
        ...
        <button
          onClick={() => valid && onConfirm(d)}
          disabled={!valid}
          className="flex-[2] rounded-xl bg-[#F59E0B] py-2.5 text-sm font-bold text-black disabled:opacity-40"
        >
          Confirm &amp; Add to Bill
        </button>
      </div>
...
function Field({
  ...
  return (
    <div>
      <label className="text-xs text-[#828894]">{label}</label>
      <input
        type={type}
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="mt-1 h-10 w-full rounded-lg border border-[#1E2228] bg-[#0D0F12] px-3 text-sm text-white outline-none focus:border-[#F59E0B]/50"
      />
    </div>
  );
}
...
function PrescriptionModal({
  ...
  const analyze = async () => {
    if (!preview) {
      toast.error("Upload a prescription photo");
      return;
    }
    setLoading(true);
    try {
      ...
      const mapped = (data.items || []).map((m: any) => {
        ...
        const confidence = product
          ? product.name.toLowerCase() === spoken
            ? 98
            : product.name.toLowerCase().includes(spoken) ||
                spoken.includes(product.name.toLowerCase())
              ? 92
              : 84
          : 40;
        return {
      ...
      setItems(mapped);
      toast.success(
        `${mapped.filter((i: any) => i.matched).length} medicines matched from AI read`,
      );
    } catch {
      ...
      {open && (
        ...
        >
          <motion.div
            ...
            exit={{ scale: 0.96, y: 16 }}
            onClick={(e) => e.stopPropagation()}
            className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-[#1E2228] bg-[#111418] shadow-2xl"
          >
            <div className="flex items-center justify-between border-b border-[#1E2228] bg-[#F59E0B]/5 px-5 py-4">
              <div className="flex items-center gap-2.5">
                <span className="grid h-9 w-9 place-items-center rounded-xl bg-[#F59E0B]/15 text-[#F59E0B]">
                ...
                <div>
                  ...
                  <p className="text-[0.65rem] text-[#828894]">
                    Nexura AI Vision — reads handwriting
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="space-y-4 p-5">
              {!preview ? (
                <button
                  ...
                  className="flex aspect-[4/3] w-full flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-[#1E2228] text-[#828894] transition-colors hover:border-[#F59E0B]/30 hover:bg-[#0D0F12]"
                >
                  <Camera className="h-8 w-8" />
                  <span className="text-sm font-medium">Take / upload prescription photo</span>
                </button>
              ) : (
                <div className="relative">
                  <img
              ...
              {preview && !items && (
                <button
                  onClick={analyze}
                  disabled={loading}
                  className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#F59E0B] py-2.5 text-sm font-bold text-black disabled:opacity-50"
                >
                  {loading ? (
                    <Loader2 className="h-4 w-4 animate-spin" />
                  ) : (
              ...
              {items && (
                <div className="space-y-2">
                  <p className="text-xs font-semibold uppercase tracking-wider text-[#828894]">
                    AI Extracted Medicines ({items.length})
                  </p>
                  {items.map((m, i) => (
                    <div
                      key={i}
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-[0.6rem] text-[#828894]">
                          {m.matched ? `₹${m.mrp.toFixed(0)} · Matched` : "Not in inventory"}
                        </p>
                      </div>
                      <span
                        className={cn(
                  ...
                  <button
                    onClick={() => onMapped(items)}
                    className="mt-2 w-full rounded-xl bg-[#F59E0B] py-2.5 text-sm font-bold text-black"
                  >
                    Add matched to cart
                  </button>
                </div>
      ...
      {data && (
        ...
        >
          <motion.div
            ...
            exit={{ scale: 0.96, y: 16 }}
            onClick={(e) => e.stopPropagation()}
            className="w-full max-w-md rounded-2xl border border-yellow-500/30 bg-[#111418] p-5 shadow-2xl"
          >
            ...
            <div className="space-y-3">
              <div className="rounded-lg bg-[#0D0F12] p-3">
                <p className="text-xs text-[#828894]">Interacting pair</p>
                <p className="text-sm font-medium text-white">{data.pair}</p>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div className="rounded-lg bg-[#0D0F12] p-3">
                  <p className="text-xs text-[#828894]">Type</p>
                  <p className="text-sm text-white">{data.type}</p>
                </div>
                <div className="rounded-lg bg-[#0D0F12] p-3">
                  <p className="text-xs text-[#828894]">Severity</p>
                  <p
                    className={cn(
                      "text-sm font-medium",
                      data.severity === "severe" ? "text-red-400" : "text-yellow-400",
                    )}
                  >
                    {data.severity}
                  </p>
              ...
              <div className="rounded-lg bg-[#0D0F12] p-3">
                <p className="text-xs text-[#828894]">Clinical effect</p>
                <p className="text-sm text-white">{data.effect}</p>
              </div>
            </div>
            <p className="mt-3 text-[0.6rem] text-[#828894]">
              AI-as-decision-support — does not block sale. Pharmacist acknowledgment logged with
              timestamp per US PioneerRx standard.
            </p>
            <button
              onClick={onClose}
              className="mt-4 w-full rounded-xl bg-[#F59E0B] py-2.5 text-sm font-bold text-black"
            >
