...
export function ReportsModule() {
  ...
  if (loading || !data) return <div className="grid h-40 place-items-center"><Loader2 className="h-5 w-5 animate-spin text-[#6B7280]" /></div>;
  return (
    <div className="space-y-5">
      <div><h1 className="font-serif text-2xl font-semibold text-white">Reports &amp; Business Intelligence</h1><p className="text-sm text-[#6B7280]">Day closing, GSTR-1, BI dashboard &amp; AI Query</p></div>
      {/* AI Query Bar */}
      <div className="rounded-2xl border border-[#F59E0B]/20 bg-[#F59E0B]/5 p-4">
        ...
        <div className="flex gap-2">
          <div className="relative flex-1"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#6B7280]" /><input value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>e.key==="Enter"&&runQuery()} placeholder="e.g. which medicines are expiring in 60 days?" className="h-10 w-full rounded-lg border border-[#1E2228] bg-[#0D0F12] pl-10 pr-3 text-sm text-white outline-none focus:border-[#F59E0B]/50" /></div>
        ...
        {answer && <div className="mt-3 rounded-lg bg-[#0D0F12] p-3"><p className="text-[0.6rem] text-[#6B7280]">Query: {answer.query}</p><p className="mt-1 text-sm text-white">{answer.text}</p></div>}
      </div>
      {/* Day closing summary */}
      <div><h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-[#6B7280]">Day Closing Summary</h3>
      ...
      <div className="rounded-2xl border border-[#1E2228] bg-[#111418] p-5">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="text-center"><p className="text-xs text-[#6B7280]">CGST</p><p className="font-serif text-xl font-bold text-white">₹{data.cgstCollected.toFixed(2)}</p></div>
          <div className="text-center"><p className="text-xs text-[#6B7280]">SGST</p><p className="font-serif text-xl font-bold text-white">₹{data.sgstCollected.toFixed(2)}</p></div>
          <div className="border-l border-[#1E2228] text-center"><p className="text-xs text-[#6B7280]">Total GST</p><p className="font-serif text-xl font-bold text-[#F59E0B]">₹{data.totalGst.toFixed(2)}</p></div>
        </div>
      </div>
    </div>
  );
...
function Kpi({ label, value, sub, color }: { label: string; value: string; sub?: string; color: string }) {
  return <div className="rounded-2xl border border-[#1E2228] bg-[#111418] p-4"><span className="h-2 w-2 rounded-full block" style={{background:color}} /><p className="mt-1 font-serif text-xl font-bold text-white">{value}</p><p className="text-xs text-[#6B7280]">{label}</p>{sub && <p className="text-[0.6rem] text-[#6B7280]">{sub}</p>}</div>;
}
