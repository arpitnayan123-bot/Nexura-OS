...
export function InventoryModule() {
  ...
  return (
    <div className="space-y-4">
      <div><h1 className="font-serif text-2xl font-semibold text-white">Inventory</h1><p className="text-sm text-[#828894]">{items.length} products · {low} low stock · {exp} near expiry</p></div>
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {[{l:"Products",v:items.length,c:"#F59E0B"},{l:"Low stock",v:low,c:"#EF4444"},{l:"Near expiry",v:exp,c:"#EAB308"},{l:"Total units",v:items.reduce((s,i)=>s+i.stockStrips,0),c:"#22C55E"}].map(k => (
          <div key={k.l} className="rounded-2xl border border-[#1E2228] bg-[#111418] p-4"><span className="h-2 w-2 rounded-full block" style={{background:k.c}} /><p className="mt-1 font-serif text-xl font-bold text-white">{k.v}</p><p className="text-[0.6rem] text-[#828894]">{k.l}</p></div>
        ))}
      </div>
      <div className="overflow-hidden rounded-2xl border border-[#1E2228] bg-[#111418]">
        <div className="max-h-[65vh] overflow-auto">
          <table className="w-full text-sm"><thead className="sticky top-0 bg-[#0D0F12]"><tr className="text-left text-[0.6rem] uppercase tracking-wider text-[#828894]">
            ...
            {items.map(p => (<tr key={p.id} className="border-b border-[#1A1D22] hover:bg-[#0D0F12]">
              <td className="p-3"><p className="font-medium text-white">{p.name}</p><p className="text-[0.6rem] text-[#828894]">{p.genericName} · {p.brand}</p></td>
              ...
              <td className="p-3 text-center">{p.nearExpiry ? <span className="flex items-center justify-center gap-1 text-yellow-400"><AlertTriangle className="h-3 w-3" />{p.earliestExpiry?.slice(0,7)}</span> : <span className="text-[#828894]">{p.earliestExpiry?.slice(0,7)}</span>}</td>
            </tr>))}
          </tbody></table>
        </div>
      </div>
