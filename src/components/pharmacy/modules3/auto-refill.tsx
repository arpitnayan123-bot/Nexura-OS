...
export function AutoRefillModule() {
  ...
  return (
    <div className="space-y-4">
      <div><h1 className="font-serif text-2xl font-semibold text-white">Auto-Refill</h1><p className="text-sm text-[#6B7280]">AI tracks prescriptions → 3 days before run-out → WhatsApp reminder to patient</p></div>
      {refills.length === 0 ? <div className="grid place-items-center rounded-2xl border border-[#1E2228] bg-[#111418] py-16"><RefreshCw className="mb-2 h-10 w-10 text-[#1E2228]" /><p className="text-sm text-[#6B7280]">No refills due yet.</p></div> :
      <div className="space-y-2">{refills.map((r,i) => (
        <div key={i} className="flex items-center gap-3 rounded-2xl border border-[#F59E0B]/20 bg-[#F59E0B]/5 p-3">
          ...
          <div className="flex-1"><p className="text-sm font-semibold text-white">{r.customer}</p><p className="text-[0.65rem] text-[#6B7280]">{r.meds.join(", ")} · {r.daysSince}d since last purchase</p></div>
