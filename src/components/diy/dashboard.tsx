...
export function Dashboard({ onNewGoals }: { onNewGoals: () => void }) {
  ...
  const withConsent = useCallback(
    async (run: () => Promise<unknown>, spec: Omit<ConsentSheetSpec, "onAllow">) => {
      try {
        await run();
      } catch (e) {
  ...
  const patchTask = (id: string, status: string, note: string | null) => {
    setData((d) => {
      ...
      if (inToday) {
        ...
        if (prev === "SKIPPED" && status !== "SKIPPED")
  ...
  const act = async (
    task: DashboardTask | DashboardWeekly,
    action: "done" | "skipped" | "not_feasible",
    note?: string,
  ) => {
    setBusyTask(task.id);
    ...
    await withConsent(
      () =>
        diyFetch(`/api/diy/tasks/${task.id}`, {
          method: "POST",
  ...
  const undo = async (task: DashboardTask | DashboardWeekly) => {
    ...
    await withConsent(() => diyFetch(`/api/diy/tasks/${task.id}`, { method: "DELETE" }), {
      title: "Allow & record my progress",
  ...
  if (error) {
    return (
      <div className="mx-auto max-w-md px-6 pb-28 pt-20 text-center">
        ...
        <button onClick={load} className="diy-btn-primary mt-4 text-sm">
          Retry
        </button>
      </div>
    );
  ...
  const hasLog = !!(
    logged &&
    (logged.mood != null || logged.energy != null || logged.sleep != null)
  );
      ...
      <section className="relative h-44 overflow-hidden sm:h-52">
        ...
        <div className="absolute inset-x-0 bottom-4 z-10 mx-auto w-[min(94%,680px)]">
          <div className="nx-glass-deep rounded-2xl px-5 py-4">
            <p className="text-xs font-medium text-[#8A7454]">
              {allDone ? "Day complete" : greeting()}
            </p>
            ...
            <div className="mt-2.5 h-2 overflow-hidden rounded-full bg-[#EBDCC2]">
              <div
                className={`h-full rounded-full transition-all ${allDone ? "bg-[#7A9A7B]" : "bg-gradient-to-r from-[#B8860B] to-[#7A9A7B]"}`}
                style={{ width: `${pct}%` }}
              />
            </div>
      ...
      <div className="mx-auto w-[min(94%,680px)] space-y-6 pt-6">
        ...
        {total === 0 ? (
          <div className="nx-glass-deep rounded-3xl p-8 text-center">
            ...
            <p className="mx-auto mt-1.5 max-w-xs text-sm leading-relaxed text-[#6B5D4E]">
              Tell the chat what you want to change and a plan lands here.
            </p>
            <button onClick={onNewGoals} className="diy-btn-primary mt-5 text-sm">
              <Plus size={15} aria-hidden /> New goals
            </button>
          </div>
        ) : (
          <section aria-label="Today's tasks" className="space-y-2.5">
            {data.todayTasks.map((t) => {
              ...
              return (
                <div
                  key={t.id}
                  className={`rounded-2xl border p-4 transition ${done ? "border-[#CBDCc4] bg-[#F2F7EC]" : off ? "border-[#E4D5C2] bg-[#F8F1E4] opacity-80" : "border-[#EADDC7] bg-[#FFFDF8]"}`}
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p
                        className={`text-sm font-medium ${done ? "text-[#4E6845] line-through decoration-[#A8C096]" : "text-[#2E2A26]"}`}
                      >
                        {t.title}
                      </p>
                    ...
                    <div className="flex shrink-0 items-center gap-1.5">
                      ...
                      {!done && (
                        <button
                          aria-label={skipped ? `Undo skip: ${t.title}` : `Skip: ${t.title}`}
                          disabled={busyTask === t.id}
                          onClick={() => (skipped ? undo(t) : act(t, "skipped"))}
                          className={`flex h-10 w-10 items-center justify-center rounded-full border transition ${skipped ? "border-[#B8860B] bg-[#F3E8CF] text-[#A16207]" : "border-[#D9C8AC] text-[#6B5D4E] hover:border-[#B8860B] hover:text-[#A16207]"}`}
                        >
                          {skipped ? (
                            <RotateCcw size={15} aria-hidden />
                          ) : (
                      ...
                      <button
                        aria-label={`Add note: ${t.title}`}
                        onClick={() => {
                          setNoteFor(noteFor === t.id ? null : t.id);
                          setNoteText(t.note ?? "");
                        }}
                        className="flex h-10 w-10 items-center justify-center rounded-full border border-[#D9C8AC] text-[#6B5D4E] transition hover:border-[#B8860B] hover:text-[#A16207]"
                      >
                  ...
                  {noteFor === t.id && (
                    <div className="mt-3 flex gap-2">
                      <input
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter" && noteText.trim()) saveNote(t);
                        }}
                      ...
                      <button
                        onClick={() => saveNote(t)}
                        className="diy-btn-primary shrink-0 px-4 text-xs"
                      >
                        Save
                      </button>
                    </div>
        ...
        {data.weekly.length > 0 && (
          <section aria-label="Weekly rhythm" className="nx-glass-deep rounded-3xl p-5">
            <div className="flex items-baseline justify-between gap-2">
              ...
              {(data.weeklyDone ?? 0) > 0 && (
                <span className="text-[11px] font-medium text-[#4E6845]">
                  {data.weeklyDone} of {data.weekly.length} done
                </span>
              )}
            </div>
            <p className="mt-0.5 text-xs text-[#6B5D4E]">
              Once-a-week anchors — slower signals, honest numbers.
            </p>
            <ul className="mt-3 space-y-2">
              {data.weekly.map((w) => {
                ...
                return (
                  <li
                    key={w.id}
                    className={`flex items-start justify-between gap-3 rounded-2xl border p-3.5 transition ${done ? "border-[#CBDCc4] bg-[#F2F7EC]" : "border-[#EADDC7] bg-[#FFFDF8]"}`}
                  >
                    <div className="min-w-0">
                      <p
                        className={`text-sm font-medium ${done ? "text-[#4E6845] line-through decoration-[#A8C096]" : "text-[#2E2A26]"}`}
                      ...
                      <p className="mt-1 text-[10.5px] uppercase tracking-wide text-[#A08D74]">
                        {w.category.replace(/_/g, " ").toLowerCase()}
                      </p>
                    </div>
                    <button
        ...
        <section className="nx-glass-deep rounded-3xl p-5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-0.5 text-xs text-[#6B5D4E]">
                Two minutes of honesty — mood, energy, sleep.
              </p>
            </div>
            <button
              onClick={() => {
                setCheckinOpen((o) => {
                  if (!o && hasLog) {
                    setCheckin({
                      mood: logged?.mood ?? 3,
                      energy: logged?.energy ?? 3,
                      sleep: logged?.sleep ?? 3,
                    });
                  }
                  return !o;
                });
              ...
              className="diy-btn-icon-terra"
              aria-label="Toggle weekly check-in"
            >
          ...
          {hasLog && (
            <div
              ...
              aria-label="Today's logged check-in"
            >
              <span className="text-[11px] text-[#8A7454]">Logged today:</span>
              {logged?.mood != null && (
                <span className="rounded-full bg-[#F1E6D4] px-2 py-0.5 text-[11px] font-medium text-[#7A5C36]">
                  mood {logged.mood}/5
                </span>
              )}
              {logged?.energy != null && (
                <span className="rounded-full bg-[#F1E6D4] px-2 py-0.5 text-[11px] font-medium text-[#7A5C36]">
                  energy {logged.energy}/5
                </span>
              )}
              {logged?.sleep != null && (
                <span className="rounded-full bg-[#F1E6D4] px-2 py-0.5 text-[11px] font-medium text-[#7A5C36]">
                  sleep {logged.sleep}/5
                </span>
              )}
            </div>
          )}
            ...
            <div className="mt-4 space-y-3">
              {(["mood", "energy", "sleep"] as const).map((k) => (
                <div key={k}>
                  <label
                    htmlFor={`ci-${k}`}
                    className="text-xs font-medium capitalize text-[#6B5D4E]"
                  >
                    {k} (1–5)
                  </label>
                  <input
                    ...
                    max={5}
                    value={checkin[k]}
                    onChange={(e) => setCheckin((c) => ({ ...c, [k]: Number(e.target.value) }))}
                    className="w-full accent-[#A16207]"
                  />
                </div>
              ))}
              <button onClick={saveCheckin} className="diy-btn-primary w-full text-sm">
                {hasLog ? "Update check-in" : "Save check-in"}
              </button>
            </div>
          )}
          {(checkinSaved || hasLog) && !checkinOpen && (
            <p className="mt-3 text-xs text-[#4E6845]">
              {checkinSaved ? "Saved — see you next week." : "Saved — you can update it anytime."}
            </p>
          )}
        </section>
        ...
        {data.plans.map((p) => {
          ...
          const posPct = totalDays
            ? Math.min(Math.round(((dayNumber - 1) / Math.max(totalDays - 1, 1)) * 100), 100)
            : 0;
          return (
            <section key={p.id} className="rounded-3xl border border-[#EADDC7] bg-[#FFFDF8] p-5">
              ...
              {totalDays != null && (
                <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-[#F0E4CE]" aria-hidden>
                  <div
                    className="h-full rounded-full bg-gradient-to-r from-[#B8860B] to-[#7A9A7B]"
                    style={{ width: `${posPct}%` }}
                  />
                </div>
              )}
                ...
                {p.milestones.map((m) => {
                  ...
                  return (
                    <div key={m.id} className="flex items-start gap-2.5 text-xs">
                      <BookOpenCheck
                        size={13}
                        className={`mt-0.5 shrink-0 ${reached ? "text-[#4E6845]" : "text-[#C9BBA2]"}`}
                        aria-hidden
                      />
                      <p className="leading-relaxed text-[#6B5D4E]">
                        <strong className={reached ? "text-[#4E6845]" : "text-[#4E4237]"}>
                          Day {m.targetDay}:
                        </strong>{" "}
                        {m.title}
                        {m.detail ? ` — ${m.detail}` : ""}
                        {reached && (
                          <span className="ml-1.5 rounded-full bg-[#EAF1E0] px-1.5 py-0.5 text-[9.5px] font-semibold uppercase tracking-wide text-[#4E6845]">
                            here
                          </span>
                        )}
                      </p>
          ...
          <section className="rounded-3xl border border-[#EAD9C0] bg-[#FBF3E4] p-5">
            ...
            <ul className="mt-2 space-y-1.5 text-xs leading-relaxed text-[#8A7454]">
              {data.conflicts.map((c) => (
                <li key={c.id}>
                  • {c.explanation}{" "}
                  <span className="text-[#A08D74]">({c.resolution.toLowerCase()})</span>
                </li>
              ))}
            </ul>
          </section>
