...
export function ConsentSheet({ sheet, onClose }: { sheet: ConsentSheetSpec | null; onClose: () => void }) {
  if (!sheet) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center" role="dialog" aria-modal="true" aria-label="Consent">
      <button aria-label="Close consent sheet" onClick={onClose} className="absolute inset-0 bg-[#2E2A26]/45 backdrop-blur-[2px]" />
      <div className="nx-glass-deep relative mx-4 mb-6 w-full max-w-md rounded-3xl p-6 pb-8">
        ...
        <div className="mt-5 flex gap-2.5">
          ...
          <button onClick={onClose} className="flex-1 rounded-full border border-[#E0D0B8] px-4 py-3 text-sm font-medium text-[#6B5D4E] hover:bg-[#F7EEDD]">
            Not now
          </button>
        </div>
      </div>
