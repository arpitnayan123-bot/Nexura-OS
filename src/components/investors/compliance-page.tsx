  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white">
      ...
      <section className="relative overflow-hidden pt-32 pb-16 px-4 sm:px-6 lg:px-8">
        <div
          aria-hidden
          className="aurora-gold -top-32 left-1/2 h-[24rem] w-[40rem] -translate-x-1/2 opacity-35"
        />
        <div className="relative mx-auto max-w-4xl text-center">
          ...
          >
            ...
            <p className="lede-lux mx-auto mt-6 max-w-2xl text-white/60">
              Indian healthcare is one of the most regulated sectors in the world. Nexura OS is
              built compliant from the architecture up — not as a feature, but as a principle.
            </p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-6xl space-y-6">
          {REGULATIONS.map((reg, i) => (
            ...
            >
              ...
              <span
                aria-hidden
                className="pointer-events-none absolute inset-x-12 top-0 h-px bg-gradient-to-r from-transparent via-[#D9B87C]/60 to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100"
              />
                ...
                <div className="lg:w-1/3">
                  <div className="flex items-center gap-2 mb-2">
                    <span
                      ...
                      style={{ background: `${reg.statusColor}20`, color: reg.statusColor }}
                    >
                  ...
                  <span
                    className="inline-block text-[0.6rem] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full"
                    style={{ background: `${reg.statusColor}15`, color: reg.statusColor }}
                  >
                    {reg.status}
                  </span>
            ...
            {[
              {
                ...
                desc: "TLS/HTTPS for all traffic today. At-rest encryption is on the roadmap and not yet enabled on the current database.",
              },
              {
                icon: FileCheck,
                title: "Tamper-evident audit trail",
                desc: "Every API action is logged with timestamp, user, and role into a SHA-256 hash chain — retroactive edits break the chain, and Merkle block roots can be shared with auditors for independent verification.",
              },
              {
                icon: Building2,
                title: "Hosting & residency",
                desc: "Single-region deployment today; Mumbai-region India hosting planned. No cross-border replication is built into the architecture.",
              },
              {
                icon: Shield,
                title: "Role-based access",
                desc: "Session-based auth with role + attribute permission checks enforced on every /api route — doctors see clinical data, admins see financials, patients see their own records.",
              },
              {
                icon: CheckCircle2,
                title: "Consent-first",
                desc: "Consent is captured as granted/denied per purpose (treatment, data share, research, telemedicine) and every capture is audit-logged. Withdrawal requests are handled via the clinic desk.",
              },
              {
                icon: Clock,
                title: "Breach response",
                desc: "Breach-response playbook defined; automated detection planned. Incidents are reviewed manually today through the incident module.",
              },
            ].map((p, i) => (
              <motion.div
                key={i}
      ...
      <section className="px-4 sm:px-6 lg:px-8 pb-20 border-t border-white/5">
        <div className="mx-auto max-w-5xl pt-16">
          ...
          <p className="text-sm text-white/50 mt-2 mb-8">
            How to exercise them today — self-serve where the product supports it, clinic-desk
            assisted where it is manual.
          </p>
            ...
            {[
              {
                ...
                desc: "Foresight Settings → “Download my data (JSON)” exports everything the engine stores about you. For hospital records, ask at the clinic desk.",
              },
              {
                icon: PencilLine,
                title: "Correct inaccurate data",
                desc: "Raise corrections with your doctor or the clinic desk. Fixes are made on the source record, and every change lands in the audit trail.",
              },
              {
                icon: Trash2,
                title: "Withdraw consent & delete",
                desc: "Settings → “Delete all my runs” wipes your Foresight data instantly. Hospital consents (treatment, data share, research) are withdrawn via the clinic desk — and audit-logged.",
              },
              {
                icon: LifeBuoy,
                title: "Grievance redressal",
                desc: "Start at the clinic desk, or email compliance@nexuraai.in. Every request is logged and tracked to closure.",
              },
            ].map((p, i) => (
              <motion.div
                key={i}
      ...
      <section className="px-4 sm:px-6 lg:px-8 pb-20">
        <div className="mx-auto max-w-3xl rounded-3xl bg-gradient-to-br from-[#A16207]/10 to-[#7A9A7B]/5 border border-white/10 p-8 text-center">
          ...
          <a
            href="mailto:compliance@nexuraai.in"
            className="inline-flex items-center gap-1.5 rounded-full bg-white text-[#0A0A0A] px-5 py-2.5 text-sm font-semibold hover:bg-white/90"
          >
