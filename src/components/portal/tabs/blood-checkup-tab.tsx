...
import {
  Droplet, Calendar, Clock, MapPin, Star, Phone, FileText, CheckCircle2,
  ChevronRight, Loader2, Truck, FlaskRound, Microscope, X, Plus, Sparkles,
} from "lucide-react";
        ...
        {shown.length === 0 ? (
          <div className="rounded-3xl border border-[#E7E5E4] bg-white p-10 text-center shadow-sm">
            ...
            <p className="mt-1 text-sm text-stone-500">
              {filter === "active" ? "Book your first at-home blood test." : `No ${filter} bookings to show.`}
            </p>
            {filter === "active" && (
              <button
                onClick={() => onBookTest()}
          ...
          <div className="grid gap-4">
            {shown.map((b, i) => (
              <BookingCard key={b.id} booking={b} delay={i * 0.05} onViewReport={() => onViewReport(b)} />
            ))}
          </div>
        )}
      </section>
...
function BookingCard({ booking, delay, onViewReport }: { booking: BloodBooking; delay: number; onViewReport: () => void }) {
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-[#E7E5E4] p-4 sm:p-5">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <p className="font-display text-base font-semibold text-stone-800">{booking.testPanelName}</p>
        ...
        <div className="text-right">
          ...
          <p className="text-sm font-semibold text-stone-800">{booking.phlebotomistName ?? "Auto-assigned"}</p>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={cn("rounded-full px-2 py-0.5 text-[0.6rem] font-semibold capitalize", s.bg, s.text)}>
      {s.label}
    </span>
  );
}
