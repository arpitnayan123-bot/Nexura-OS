...
const KIND_STYLE: Record<Milestone["kind"], { icon: typeof Activity; rail: string; dot: string; label: string }> = {
  screening: { icon: Stethoscope, rail: "border-violet-300/30", dot: "border-violet-300/60 bg-violet-300/15 text-violet-200", label: "screening" },
  rescan: { icon: Activity, rail: "border-teal-300/30", dot: "border-teal-300/60 bg-teal-300/15 text-teal-200", label: "re-scan" },
  threshold: { icon: TrendingDown, rail: "border-amber-300/30", dot: "border-amber-300/60 bg-amber-300/15 text-amber-200", label: "projection" },
  protective: { icon: CalendarClock, rail: "border-emerald-300/30", dot: "border-emerald-300/60 bg-emerald-300/15 text-emerald-200", label: "protective" },
};
  ...
  return (
    <motion.section {...fadeUp} aria-labelledby="nxf-timeline-title">
      ...
      {milestones.length === 0 ? (
        ...
        <ol className="nxf-glass relative space-y-0 rounded-3xl p-5 sm:p-6" aria-label="Upcoming milestones">
          {milestones.map((m, i) => {
            ...
            return (
              ...
              >
                ...
                <div className="relative flex flex-col items-center">
                  <span className={cn("z-10 grid h-8 w-8 shrink-0 place-items-center rounded-full border", style.dot)}>
                    <Icon aria-hidden="true" className="h-4 w-4" />
                  </span>
                  {!last && <span aria-hidden="true" className={cn("absolute top-8 bottom-0 w-px border-l border-dashed", style.rail)} />}
                </div>
                ...
                <div className="mt-1.5 sm:mt-0">
                  <p className="flex flex-wrap items-center gap-x-2 gap-y-1 text-[13.5px] font-semibold leading-snug nxf-hi">
                    ...
                    <span
                      className={cn(
                        ...
                        m.certainty === "illustrative" ? "border-amber-300/40 text-amber-200" : "border-teal-300/40 text-teal-200"
                      )}
                    >
                      {m.certainty}
                    </span>
                  ...
                  <p className="mt-1 text-[10.5px] uppercase tracking-[0.12em] nxf-mute">{style.label}</p>
                </div>
              </motion.li>
            );
          })}
