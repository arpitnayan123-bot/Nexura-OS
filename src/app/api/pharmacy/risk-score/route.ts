...
const RISK_FACTORS: { factor: string; weight: number; source: string }[] = [
  {
    factor: "Diabetes medication (Metformin/Glimepiride/Insulin)",
    weight: 25,
    source: "ICMR-INDIAB",
  },
  { factor: "Hypertension medication (2+ agents)", weight: 20, source: "ICMR HTN Study" },
  {
    factor: "Cardiac medication (Aspirin+Clopidogrel+Statin)",
    weight: 30,
    source: "ICMR CAD Registry",
  },
  ...
  {
    factor: "High purchase frequency (>10 purchases/month)",
    weight: 10,
    source: "Adherence Pattern",
  },
...
const CHRONIC_MED_MAP: { salts: string[]; disease: string; weight: number; source: string }[] = [
  {
    salts: ["metformin", "glimepiride", "vildagliptin", "insulin", "sitagliptin"],
    disease: "Diabetes Type 2",
    weight: 25,
    source: "ICMR-INDIAB",
  },
  {
    salts: ["amlodipine", "telmisartan", "ramipril", "metoprolol", "atenolol", "losartan"],
    disease: "Hypertension",
    weight: 20,
    source: "ICMR HTN",
  },
  {
    salts: ["aspirin", "clopidogrel", "ecosprin", "clopitab"],
    disease: "Coronary Artery Disease",
    weight: 30,
    source: "ICMR CAD",
  },
  {
    salts: ["atorvastatin", "rosuvastatin", "fenofibrate"],
    disease: "Dyslipidemia",
    weight: 10,
    source: "Indian Lipid Guidelines",
  },
  {
    salts: ["levothyroxine", "thyronorm", "eltroxin"],
    disease: "Hypothyroidism",
    weight: 10,
    source: "ICMR Thyroid",
  },
  {
    salts: ["sertraline", "escitalopram", "fluoxetine", "amitriptyline"],
    disease: "Depression/Anxiety",
    weight: 15,
    source: "NMHS India",
  },
  {
    salts: ["budesonide", "formoterol", "salbutamol", "foracort"],
    disease: "Asthma/COPD",
    weight: 15,
    source: "GAN India",
  },
  {
    salts: ["shelcal", "calcium", "vitamin d"],
    disease: "Calcium/Vit D Deficiency",
    weight: 5,
    source: "NFHS-5 Nutrition",
  },
  {
    salts: ["iron", "folic acid", "ferrous"],
    disease: "Anemia",
    weight: 15,
...
async function GET_impl(req: NextRequest) {
  try {
    ...
    const scored = customers.map((c) => {
      ...
      for (const cm of CHRONIC_MED_MAP) {
        const matching = allMeds.filter((m) =>
          cm.salts.some((sp) => m.salt.includes(sp) || m.name.toLowerCase().includes(sp)),
        );
      ...
      const monthlyPurchases =
        ...
        Math.max(
          1,
          Math.floor(
            (Date.now() - (c.sales[c.sales.length - 1]?.createdAt || new Date()).getTime()) /
              (30 * 86400000),
          ),
        );
      ...
      // Risk level
      const riskLevel =
        riskScore >= 60 ? "high" : riskScore >= 30 ? "moderate" : riskScore > 0 ? "low" : "none";
      const riskColor =
        riskLevel === "high"
          ? "#EF4444"
          : riskLevel === "moderate"
      ...
      if (detectedDiseases.some((d) => d.disease === "Diabetes Type 2"))
        recommendations.push("Schedule HbA1c check every 3 months (ICMR-INDIAB)");
      if (detectedDiseases.some((d) => d.disease === "Hypertension"))
        recommendations.push("Monitor BP weekly — target <140/90 (ICMR)");
      if (detectedDiseases.some((d) => d.disease === "Coronary Artery Disease"))
        recommendations.push("Annual ECG + Echo — refer cardiologist");
      if (detectedDiseases.length >= 3)
        recommendations.push("Medication Therapy Management (MTM) review recommended");
      if (riskScore >= 60)
        recommendations.push(
          "High-risk patient — prioritise refill reminders & adherence counseling",
        );
    ...
    log.error("pharmacy", "risk_score_failed", {
      err: err instanceof Error ? err.message : String(err),
    });
    return NextResponse.json(
      { error: "risk_score_failed", detail: "Risk scores could not be generated. Please retry." },
      { status: 500 },
    );
  }
}
