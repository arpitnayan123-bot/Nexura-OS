...
async function GET_impl(req: NextRequest) {
  try {
    ...
    if (q) {
      const results = Object.values(NPPA_DATA).filter(
        (d) => d.brandName.toLowerCase().includes(q) || d.salt.toLowerCase().includes(q),
      );
    ...
    return NextResponse.json({
      ...
      categories: {
        ...
        semiEssential: Object.values(NPPA_DATA).filter((d) => d.category === "semi-essential")
          .length,
        nonControlled: Object.values(NPPA_DATA).filter((d) => d.category === "non-controlled")
          .length,
      },
    });
  } catch (err) {
    log.error("pharmacy", "nppa_prices_failed", {
      err: err instanceof Error ? err.message : String(err),
    });
    return NextResponse.json(
      { error: "nppa_failed", detail: "Price data could not be loaded. Please retry." },
      { status: 500 },
    );
  }
}
