  ...
  try {
    ...
    const withLedger = customers.map((c) => {
      ...
      return {
        ...
        account: c.account ? toRupees(c.account, CREDIT_LIMIT_PAISE) : null,
      };
    });
    return NextResponse.json({ customers: withLedger.filter((c) => c.name !== "Walk-in") });
  } catch (err) {
    log.error("pharmacy", "customers_list_failed", {
      err: err instanceof Error ? err.message : String(err),
    });
    return NextResponse.json(
      {
        error: "customers_accounts_failed",
        detail: "Customer accounts could not be loaded. Please retry.",
      },
      { status: 500 },
    );
  }
...
async function POST_impl(req: NextRequest) {
  try {
    ...
    if (!parsed.success)
      return NextResponse.json(
        { error: "invalid_request", detail: "Invalid customer payment payload." },
        { status: 400 },
      );
    ...
    log.error("pharmacy", "customer_payment_failed", {
      err: err instanceof Error ? err.message : String(err),
    });
    return NextResponse.json(
      {
