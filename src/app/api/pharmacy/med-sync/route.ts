...
const CHRONIC_MAP: Record<string, { disease: string; saltPatterns: string[]; typicalCycleDays: number }> = {
  "Diabetes Type 2": { disease: "Diabetes Type 2", saltPatterns: ["metformin", "glimepiride", "vildagliptin", "insulin", "sitagliptin"], typicalCycleDays: 30 },
  "Hypertension": { disease: "Hypertension", saltPatterns: ["amlodipine", "telmisartan", "ramipril", "metoprolol", "atenolol", "losartan"], typicalCycleDays: 30 },
  "Cholesterol": { disease: "Dyslipidemia", saltPatterns: ["atorvastatin", "rosuvastatin", "fenofibrate"], typicalCycleDays: 30 },
  "Hypothyroidism": { disease: "Hypothyroidism", saltPatterns: ["levothyroxine", "thyronorm", "eltroxin"], typicalCycleDays: 30 },
  "Cardiac": { disease: "Coronary Artery Disease", saltPatterns: ["aspirin", "clopidogrel", "ecosprin"], typicalCycleDays: 30 },
  "Asthma": { disease: "Asthma/COPD", saltPatterns: ["budesonide", "formoterol", "salbutamol", "foracort", "asthalin"], typicalCycleDays: 30 },
  "Mental Health": { disease: "Depression/Anxiety", saltPatterns: ["sertraline", "escitalopram", "fluoxetine", "amitriptyline"], typicalCycleDays: 30 },
};
...
export async function GET() {
  try {
    ...
    const chronicPatients = customers.map(c => {
      ...
      const detectedDiseases: { disease: string; medicines: string[]; lastPurchase: string; cycleDays: number }[] = [];
      const seenSalts = new Set<string>();

      for (const [key, pattern] of Object.entries(CHRONIC_MAP)) {
        const matchingMeds = allMeds.filter(m =>
          pattern.saltPatterns.some(sp => m.salt.toLowerCase().includes(sp) || m.name.toLowerCase().includes(sp))
        );
      ...
      const nextRefillDates = detectedDiseases.map(d => {
        ...
        return { disease: d.disease, nextRefill: nextDate.toISOString().slice(0, 10), daysUntil: Math.ceil((nextDate.getTime() - Date.now()) / 86400000) };
      });
      ...
      const earliestRefill = nextRefillDates.length > 0
