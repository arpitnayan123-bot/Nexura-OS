...
export const GET = withRoute("nx.pharmacy.queue", async (req: NextRequest) => {
  ...
  const expiringSoon = stock
    .filter((m) => m.expiryDate && new Date(m.expiryDate) <= soon)
    .slice(0, 6);
...
export const PATCH = withRoute("nx.pharmacy.dispense", async (req: NextRequest) => {
  ...
  await db.nxOrderEvent.create({
    data: {
      ...
      note:
        body.note || (body.to === "acknowledged" ? "Pharmacist verification passed" : undefined),
    },
  });
  await audit({
    hospitalId: order.hospitalId,
    ...
    action: `medication.${body.to === "acknowledged" ? "verify" : body.to}`,
