  ...
  paths: {
    "/api/nx/predict/radar": {
      get: {
        ...
        responses: { "200": { description: "Radar rows with band, confidence, top driver" }, "401": { description: "Unauthorized" } },
      },
    },
    ...
    "/api/nx/predict/twin/{patientId}": {
      ...
      post: {
        ...
        requestBody: {
          required: true,
          content: { "application/json": { schema: { type: "object", required: ["interventionIds"], properties: { interventionIds: { type: "array", items: { type: "string" }, maxItems: 4 } } } } },
        },
    ...
    "/api/nx/predict/protocols": {
      get: { summary: "Protocol approval queue", responses: { "200": { description: "Protocols with parsed actions + patient labels" } } },
    },
          ...
          content: {
            "application/json": {
              schema: {
                type: "object",
                required: ["action"],
                properties: { action: { type: "string", enum: ["approve", "reject"] }, reason: { type: "string" }, approvedBy: { type: "string" } },
              },
            },
          },
        },
        responses: { "200": { description: "Decision recorded; approval creates WorkQueue tasks" }, "409": { description: "Protocol not actionable" } },
      },
    },
    "/api/nx/bio/{deviceId}": {
      post: {
            ...
            "application/json": {
              schema: {
                ...
                properties: {
                  ...
                  samples: { type: "array", items: { type: "object", required: ["metric", "value"], properties: { metric: { type: "string" }, value: { type: "number" }, unit: { type: "string" }, quality: { type: "number" }, capturedAt: { type: "string", format: "date-time" } } }, maxItems: 500 },
                },
              },
            },
          },
        },
        responses: { "200": { description: "Ingest report: accepted/rejected per metric" }, "404": { description: "Unknown device" } },
      },
    },
    "/api/nx/predict/explain/{assessmentId}": {
      get: {
        ...
        responses: { "200": { description: "Headline, top factors, protective factors, data quality" } },
      },
    },
    "/api/nx/predict/federation": {
      get: { summary: "Federated learning status", responses: { "200": { description: "Pending deltas + recent aggregations" } } },
      post: {
            ...
            "application/json": {
              schema: {
                ...
                properties: {
                  ...
                  modelId: { type: "string", enum: ["sepsis_early_warning", "readmission_trajectory", "chronic_organ_decay"] },
        ...
        responses: { "200": { description: "Accepted; aggregates when ≥2 tenants pending" }, "422": { description: "Privacy validation rejected the delta" } },
      },
    },
    "/api/nx/predict/governance": {
      get: { summary: "SaMD registry + bias audits + fail-safe posture", responses: { "200": { description: "Governance surface" } } },
    },
  },
  components: {
