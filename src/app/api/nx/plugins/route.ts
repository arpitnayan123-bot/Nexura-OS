  ...
  return ok(
    {
      ...
      sandboxPolicy:
        "plugins render panels via scoped APIs only; no direct DB access; every permission grant is listed here and audited on change",
    },
    { requestId },
  );
});
...
export const POST = withRoute("plugins.upsert", async (req: NextRequest, { requestId }) => {
  ...
  if (badSlot)
    return fail("unknown_slot", 422, `Slot "${badSlot}" not offered by the shell.`, requestId);
  ...
  const plugin = await db.nxPlugin.upsert({
    ...
    create: {
      ...
      version: d.version,
      vendor: d.vendor,
      permissionsJson: JSON.stringify(d.permissions),
  ...
  await audit({
    ...
    action: `plugin.${d.enabled ? "enable" : "register"}`,
    entityType: "nx_plugin",
    entityId: plugin.id,
    detail: { code: d.code, permissions: d.permissions },
  });
