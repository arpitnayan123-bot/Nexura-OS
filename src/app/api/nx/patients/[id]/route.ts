...
export const GET = withRoute<{ id: string }>(
  ...
  async (req: NextRequest, { params }) => {
    const gate = await requirePermission(req, "patient.demographics.view", {
      patientId: (await params).id,
    });
    if ("error" in gate)
      return NextResponse.json({ error: gate.error, detail: gate.detail }, { status: gate.status });
    const { id } = await params;
    const { session, perms } = gate;

    // Patient-role scoping: can only ever load their own linked record
    if (session.role === "patient") {
      const self = await db.nxStaffUser.findUnique({
        where: { id: session.userId },
        select: { linkedPatientId: true },
      });
      if (!self?.linkedPatientId || self.linkedPatientId !== id) {
        return NextResponse.json(
          { error: "forbidden", detail: "patient_scoped_only" },
          { status: 403 },
        );
      }
    }
    ...
    const openTasks = await db.nxTask.findMany({
      where: {
        patientId: patient.id,
        status: {
          in: ["new", "open", "assigned", "in_progress", "blocked", "waiting", "escalated"],
        },
      },
    ...
    const accessHistory = session.hospitalId
      ? await db.nxAuditEvent.findMany({
          where: {
            ...
            patientId: patient.id,
            action: {
              in: ["patient.view", "patient.view.breakglass", "note.sign", "breakglass.invoke"],
            },
          },
          orderBy: { createdAt: "desc" },
          take: 15,
    ...
    type TEvent = {
      ...
      title: string;
      sub?: string;
      state?: "critical" | "pending" | "verified" | "draft" | "signed" | "restricted";
    };
    ...
    for (const a of patient.admissions) {
      timeline.push({
        ...
        kind: "admission",
        icon: "hospital",
        title: a.dischargeStatus === "active" ? "Admitted" : "Admission",
        sub: `${a.admissionType}${a.admissionDiagnosis ? ` · ${a.admissionDiagnosis}` : ""}`,
        state: a.dischargeStatus === "active" ? "pending" : "verified",
      });
      if (a.actualDischargeDate)
        timeline.push({
          at: a.actualDischargeDate.toISOString(),
          kind: "discharge",
          icon: "logout",
          title: "Discharged",
          sub: a.dischargeStatus ?? undefined,
          state: "verified",
        });
    }
    for (const ap of patient.appointments) {
      timeline.push({
        at: ap.date.toISOString(),
        kind: "appointment",
        icon: "calendar",
        title: "Appointment",
        sub: `${ap.doctor?.name ?? ""}${ap.chiefComplaint ? ` · ${ap.chiefComplaint}` : ""}`,
        state:
          ap.status === "completed" ? "verified" : ap.status === "no_show" ? "critical" : "pending",
      });
    }
    if (canClinical) {
      for (const v of patient.vitals) {
        ...
        timeline.push({
          ...
          kind: "vitals",
          icon: "heart",
          title: `Vitals ${v.bpSystolic ?? "?"}/${v.bpDiastolic ?? "?"} · ${v.pulseRate ?? "?"}bpm · SpO₂ ${v.spo2 ?? "?"}%`,
          sub: v.news2Score != null ? `NEWS2 ${v.news2Score}` : undefined,
          state: crit ? "critical" : "verified",
        });
      }
      for (const n of patient.clinicalNotes) {
        timeline.push({
          ...
          kind: "note",
          icon: "file",
          title: `${n.noteType.toUpperCase()} note`,
          sub: n.signedByName ? `signed by ${n.signedByName}` : "draft",
          state: n.locked ? "signed" : n.restricted && !canRestricted ? "restricted" : "draft",
        });
      }
      for (const o of patient.admissions.flatMap((a) => a.orders)) {
        for (const r of o.labResults) {
          timeline.push({
            at: (r.reportedAt ?? r.createdAt).toISOString(),
            kind: "lab",
            icon: "flask",
            title: `${r.testName}: ${r.resultValue ?? r.resultText ?? "—"}`,
            sub: `${r.verificationStatus}${r.abnormalFlag !== "normal" ? ` · ${r.abnormalFlag}` : ""}`,
            state:
        ...
        for (const img of o.imagingReports) {
          timeline.push({
            at: img.createdAt.toISOString(),
            kind: "imaging",
            icon: "scan",
            title: `${img.modality.toUpperCase()} report`,
            sub: `${img.status}${img.radiologistName ? ` · ${img.radiologistName}` : ""}`,
            state: img.status === "final" ? "signed" : "pending",
          });
        }
      }
      for (const m of (
        ...
        timeline.push({
          ...
          kind: "mar",
          icon: "pill",
          title: `${m.medicineName} — ${m.status}`,
          sub: m.administeredBy ?? undefined,
          state:
            m.status === "given" ? "verified" : m.status === "pending" ? "pending" : "critical",
        });
      }
      for (const c of (
        patient as unknown as {
          consents?: Array<{ grantedAt: Date; type: string; status: string; recordedBy: string }>;
        }
      ).consents ?? []) {
        timeline.push({
          ...
          kind: "consent",
          icon: "shield",
          title: `Consent ${c.type} — ${c.status}`,
          sub: `by ${c.recordedBy}`,
          state: c.status === "granted" ? "verified" : "restricted",
        });
      }
    }
    ...
    const billingSummary = canBilling
      ? await (async () => {
          const [bills, payments, charges] = await Promise.all([
            db.hospitalBill.aggregate({
              where: { patientId: patient.id },
              _sum: { totalPayable: true },
              _count: true,
            }),
            db.nxPayment.aggregate({
              where: { patientId: patient.id, refundOfId: null },
              _sum: { amount: true },
              _count: true,
            }),
            db.nxCharge.aggregate({
    ...
    const recentOrders = patient.admissions
      .flatMap((a) => a.orders)
    ...
    return NextResponse.json({
      profile: {
        ...
        insurance: {
          ...
          pmjay: patient.pmjayBeneficiary,
        },
        allergies,
        chronic,
        emergencyContact: {
      ...
      status: {
        ...
        location: activeAdmission
          ? `${activeAdmission.bed?.ward?.name || "Ward"} · ${activeAdmission.bed?.bedNumber || "—"}`
          : null,
      ...
      consents: canClinical
        ? ((patient as unknown as { consents?: unknown[] }).consents ?? [])
        : [],
      mar: canClinical
        ? (
            (
      ...
      orders: canClinical
        ? patient.admissions.flatMap((a) =>
            a.orders.map((o) => ({
              ...
              imaging: o.imagingReports.map((i) => ({
                id: i.id,
                modality: i.modality,
                status: i.status,
                findings: i.findings,
                impression: i.impression,
                radiologist: i.radiologistName,
                at: i.createdAt,
              })),
            })),
          )
      ...
      notes: canClinical
        ? patient.clinicalNotes.map((n) => ({
            ...
            signedByRole: n.signedByRole,
            version: n.currentVersion,
            restricted: n.restricted,
            body:
              canRestricted || !n.restricted
                ? n.fullText || n.assessment || n.subjective || ""
                : null,
