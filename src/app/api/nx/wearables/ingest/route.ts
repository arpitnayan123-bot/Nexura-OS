...
export const POST = withRoute("wearables.ingest", async (req: NextRequest, { requestId }) => {
  ...
  let device = await db.nxWearableDevice.findFirst({
    where: { hospitalId, patientId: patient.id, source: body.data.source, active: true },
  });
  if (!device) {
    device = await db.nxWearableDevice.create({
      data: { hospitalId, patientId: patient.id, source: body.data.source, model: body.data.model },
    });
  }
  await db.nxWearableSample.createMany({
    data: body.data.samples.map((s) => ({
      deviceId: device.id,
      metric: s.metric,
      value: s.value,
      unit: s.unit,
  ...
  publish({
    event: "wearable.ingested",
    hospitalId,
    toUsers: [g.session.userId],
    data: { patientId: patient.id, count: body.data.samples.length },
  });
  return ok(
    { accepted: body.data.samples.length, deviceId: device.id },
