...
export const POST = withRoute("identity.vc.issue", async (req: NextRequest, { requestId }) => {
  ...
  const vc = await db.nxVerifiableCredential.create({
    data: {
      ...
      issuerId: `did:nexura:hospital:${hospitalId}`,
      signature: signClaims(claims),
      holderHint: patient.phone?.slice(-4),
    },
  });
  await audit({
    hospitalId,
    ...
    action: "vc.issue",
    entityType: "nx_verifiable_credential",
    entityId: vc.id,
    patientId: patient.id,
  });
  return ok(
    {
      ...
      proof: {
        type: "HmacSha256Proof2026",
        created: vc.issuedAt,
        verificationMethod: vc.issuerId,
        jws: vc.signature,
      },
...
export const GET = withRoute("identity.vc.verify", async (req: NextRequest, { requestId }) => {
  ...
  if (vc.revokedAt)
    return ok({ valid: false, reason: "revoked", revokedAt: vc.revokedAt }, { requestId });
  ...
  return ok(
    {
      ...
      subject: claims.credentialSubject,
    },
    { requestId },
  );
});
