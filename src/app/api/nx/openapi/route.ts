...
function op(
  summary: string,
  tag: string,
  opts?: { params?: string[]; body?: boolean; errors?: number[] },
) {
    ...
    paths: {
      "/api/health": {
        get: {
          ...
          tags: ["observability"],
          responses: { 200: okResp("Process up") },
        },
      },
      "/api/ready": {
        get: {
          summary: "Readiness probe (DB, seed, env)",
          tags: ["observability"],
          responses: { 200: okResp("Ready"), 503: errResp("Degraded") },
        },
      },
      "/api/nx/auth": {
        post: op(
          "Sign in (email+password or staffCode+PIN; MFA step-up; progressive lockout)",
          "auth",
          { body: true, errors: [401, 423, 429] },
        ),
      ...
      "/api/nx/auth/password": {
        ...
        post: op("Request password reset (console email transport in demo)", "auth", {
          body: true,
        }),
      ...
      "/api/nx/auth/break-glass": {
        post: op("Invoke emergency access (mandatory reason + TTL, audited)", "auth", {
          body: true,
          errors: [403],
        }),
      ...
      "/api/nx/notifications": {
        get: op("Inbox (targeted + role broadcasts) with unread count", "notifications", {
          params: ["page", "perPage"],
        }),
      ...
      "/api/nx/stream": {
        get: {
          summary: "Real-time SSE stream (auth at subscribe, heartbeat, seq dedupe)",
          tags: ["realtime"],
          security: [{ sessionCookie: [] }],
          responses: { 200: { description: "text/event-stream" }, 401: errResp("Unauthenticated") },
        },
      },
      "/api/nx/search": {
        get: op("Global cross-entity search (hospital-scoped)", "search", { params: ["q"] }),
      },
      "/api/nx/prefs": {
      ...
      "/api/nx/tasks": {
        get: op(
          "Work queue (filters: status/priority/mine/type/view/page; legacy counts included)",
          "tasks",
          { params: ["status", "priority", "mine", "type", "view", "page", "perPage", "q"] },
        ),
        post: op("Create task (validated, audited, live event)", "tasks", {
          body: true,
          errors: [400],
        }),
        patch: op(
          "Transition task / comment / checklist / handoff (illegal transitions → 422)",
          "tasks",
          { body: true, errors: [404, 422] },
        ),
      ...
      "/api/nx/notes/{id}": {
        ...
        patch: op("save | sign | addendum (signed notes immutable → 423)", "clinical-notes", {
          body: true,
          errors: [403, 409, 423],
        }),
      },
      "/api/nx/patients": {
        get: op("Patient directory (hospital-scoped)", "patients", {
          params: ["page", "perPage", "q"],
        }),
      },
      "/api/nx/patients/{id}": {
        get: op(
          "Patient record v2: permission-aware profile, timeline, consents, MAR, access history (view-audited)",
          "patients",
          { errors: [403, 404] },
        ),
      },
      "/api/nx/schedule": {
        ...
        post: op("Book appointment (conflict-checked, automation-fired)", "scheduling", {
          body: true,
          errors: [409],
        }),
        patch: op("Status transition / check-in / reschedule / no-show→waitlist", "scheduling", {
          body: true,
          errors: [409],
        }),
      },
      "/api/nx/schedule/waitlist": {
        ...
        patch: op("Promote waitlist entry to a slot (conflict-checked)", "scheduling", {
          body: true,
          errors: [409],
        }),
      },
      "/api/nx/beds": {
        ...
        post: op("Bed lifecycle transitions (cleaning/isolation/maintenance flows)", "beds", {
          body: true,
          errors: [422],
        }),
      },
      "/api/nx/labs/verify": {
        ...
        patch: op(
          "verify | reject | notify_critical (critical → clinician + command notified, task created)",
          "labs",
          { body: true, errors: [422] },
        ),
      },
      "/api/nx/pharmacy/mar": {
        get: op("MAR entries (pending/all by patient)", "medications", {
          params: ["patientId", "status"],
        }),
        post: op("Schedule administration (allergy cross-check: clear/warn/block)", "medications", {
          body: true,
        }),
        patch: op(
          "Given / held / refused / missed (controlled substances need witness)",
          "medications",
          { body: true, errors: [409, 422] },
        ),
      },
      "/api/nx/billing/v2": {
        get: op("Summary | charges | payments | CSV export (reports.export)", "billing", {
          params: ["kind", "patientId"],
        }),
        post: op("Create charge", "billing", { body: true }),
        put: op(
          "Record payment/refund (idempotent via x-idempotency-key; receipt returned)",
          "billing",
          { body: true },
        ),
      },
      "/api/nx/supply/procurement": {
        get: op("Vendors, POs, stock txns, low-stock/expiry alerts", "inventory", {
          params: ["kind"],
        }),
        post: op("Create vendor", "inventory", { body: true }),
        put: op("Create purchase order", "inventory", { body: true }),
        patch: op("Stock transaction (transactional onHand; alerts on low stock)", "inventory", {
          body: true,
          errors: [422],
        }),
      },
      ...
      "/api/nx/messages": {
        get: op("Channel directory + messages (patient channels privacy-gated)", "communication", {
          params: ["channel", "q"],
        }),
        post: op(
          "Post message (mentions → notifications; urgent severity; live event)",
          "communication",
          { body: true },
        ),
      ...
      "/api/nx/analytics": {
        get: op(
          "Role-aware analytics with ?days window; ?format=csv export (reports.export)",
          "analytics",
          { params: ["days", "format"] },
        ),
      },
      "/api/nx/audit": {
        get: op("Tamper-evident audit chain (audit.view)", "audit", {
          params: ["page", "perPage", "q"],
        }),
      },
      ...
      "/api/nx/tenants": {
        ...
        post: op("Create/update tenant: branding, modules, domains, hospital binding", "tenancy", {
          body: true,
          errors: [403, 409],
        }),
      },
      ...
      "/api/nx/gateway/v1/patients": {
        get: op("Partner sandbox read: patients (scope patients.read)", "gateway", {
          params: ["page", "perPage", "q"],
        }),
      },
      "/api/nx/gateway/v1/observations": {
        get: op("Partner sandbox read: lab observations (scope observations.read)", "gateway", {
          params: ["patientId"],
        }),
      },
      "/api/nx/fhir/metadata": { get: op("FHIR R4 CapabilityStatement", "fhir") },
      "/api/nx/fhir/{resource}": {
        get: op(
          "FHIR R4 search/read: Patient | Encounter | Observation | MedicationRequest",
          "fhir",
          { params: ["_id", "identifier", "name", "patient", "status"] },
        ),
      },
      "/api/nx/hl7": {
        post: op("HL7 v2 inbound: ADT^A01/A08 + ORU^R01", "hl7", {
          body: true,
          errors: [400, 403, 422],
        }),
      ...
      "/api/nx/webhooks/inbound": {
        post: op("Partner push with HMAC signature verification", "integrations", {
          body: true,
          errors: [401, 422],
        }),
      },
      ...
      "/api/nx/auth/stepup": {
        post: op("Step-up second factor (PIN/TOTP) for privileged actions", "security", {
          body: true,
          errors: [401],
        }),
      },
      ...
      "/api/nx/pathways": {
        ...
        post: op("create_def | start | advance (critical skip blocked)", "clinical", {
          body: true,
          errors: [422],
        }),
      },
      ...
      "/api/nx/prescriptions/sign": {
        post: op("DPCO two-person e-prescription signing", "clinical", {
          body: true,
          errors: [401, 422],
        }),
      ...
      "/api/nx/docs/versions": {
        get: op("Document versions with tracked edits", "clinical", {
      ...
      "/api/nx/automations/explain": {
        get: op("Explainable automations", "automations", { params: ["ruleId"] }),
      },
      "/api/nx/identity/vc": {
      ...
      "/api/nx/wearables/ingest": {
        post: op("Ingest wearable samples", "patient360", { body: true }),
      },
      "/api/nx/wearables/insights": {
        get: op("Deterministic wearable insights", "patient360", { params: ["patientId"] }),
      },
      "/api/nx/twin/simulate": {
        post: op("Digital twin projection (education only)", "patient360", { body: true }),
      },
      "/api/nx/telehealth": {
        get: op("Teleconsult queue", "telehealth"),
        post: op("Doctor-on-demand routing + bandwidth mode", "telehealth", {
          body: true,
          errors: [503],
        }),
      },
      ...
      "/api/nx/journey/predicted": {
        get: op("Predicted care journey nodes", "clinical", { params: ["patientId"] }),
      },
      "/api/nx/templates": { get: op("Hospital-as-a-Service templates", "platform") },
      "/api/nx/onboard": {
        post: op("Apply HaaS template to provision hospital", "platform", { body: true }),
      },
      "/api/nx/plugins": {
      ...
      "/api/nx/events": {
        get: op("Event log replay per aggregate", "audit", {
          params: ["aggregateType", "aggregateId"],
        }),
      },
      "/api/nx/offline/sync": {
        post: op("Flush offline write buffer (idempotent)", "offline", { body: true }),
      },
    },
  };
