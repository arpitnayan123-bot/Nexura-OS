...
export const GET = withRoute("nx.overview.command-center", async (req: NextRequest) => {
  ...
  const [
    ...
    db.hospitalAdmission.findMany({
      ...
      include: { patient: true, bed: { include: { ward: true } }, admittingDoctor: true },
    }),
    db.hospitalAdmission.count({ where: { hospitalId, admissionDate: { gte: startOfDay } } }),
    db.oTSurgery.findMany({
      where: { hospitalId, plannedStartTime: { gte: new Date(Date.now() - 12 * 3600000) } },
      include: { surgeon: true },
      orderBy: { plannedStartTime: "asc" },
    }),
    db.nxIncident.findMany({
    ...
    db.nxTask.findMany({
      where: { hospitalId, type: "cleaning", status: { in: ["open", "in_progress"] } },
    }),
  ...
  const now = new Date();
  const delayedDischarges = admissions.filter(
    (a) => a.expectedDischargeDate && new Date(a.expectedDischargeDate) < now,
  );
  ...
  return NextResponse.json({
    hospital: {
      id: hospitalId,
      name: (await db.hospital.findUnique({ where: { id: hospitalId } }))?.name,
    },
    census: {
      ...
      delayedDischarges: delayedDischarges.map((a) => ({
        ...
        uhid: a.patientUhid,
        expected: a.expectedDischargeDate,
        diagnosis: a.admissionDiagnosis,
        daysOver: Math.floor(
          (now.getTime() - new Date(a.expectedDischargeDate!).getTime()) / 86400000,
        ),
      })),
    },
    beds: {
      ...
      wards: wards.map((w) => ({
        ...
        occupied: beds.filter(
          (b) => b.wardId === w.id && ["occupied", "discharge_pending"].includes(b.status),
        ).length,
      })),
    },
    ed: {
    ...
    equipmentOutages: equipment.map((e) => ({
      id: e.id,
      name: e.name,
      status: e.status,
      location: e.location,
    ...
    revenueToday: {
      ...
      collected:
        billsToday
          ...
          .reduce((s, b) => s + b.totalPayable, 0) / 100,
      pending:
        billsToday
          .filter((b) => b.paymentStatus !== "paid")
          .reduce((s, b) => s + b.totalPayable, 0) / 100,
    ...
    bottlenecks: [
      ...(delayedDischarges.length > 0
        ? [
            {
              area: "Discharge",
              detail: `${delayedDischarges.length} patient(s) past expected discharge date`,
              severity: "high",
            },
          ]
        : []),
      ...(bedCounts.cleaning_required
        ? [
            {
              area: "Bed turnaround",
              detail: `${bedCounts.cleaning_required} bed(s) awaiting cleaning`,
              severity: "medium",
            },
          ]
        : []),
      ...(equipment.length
        ? [
            {
              area: "Equipment",
              detail: `${equipment.length} asset(s) in fault/maintenance`,
              severity: "medium",
            },
          ]
        : []),
        ...
        ? [
            {
              ...
              detail: `${orBoard.planned} cases scheduled — watch turnover`,
              severity: "low",
            },
          ]
        : []),
