...
export const GET = withRoute("messages.list", async (req: NextRequest) => {
  ...
  const unread = await db.nxMessage.count({
    where: { hospitalId, channelKey: channel, createdAt: { gt: new Date(Date.now() - 24 * 3600_000) }, senderName: { not: session.name }, reads: { none: { userId: session.userId } } },
  });
  ...
  return NextResponse.json({
      ...
      channels: ([
        ...
        { key: "emergency", label: "Emergency", kind: "emergency" as const, severity: "urgent" as const },
        { key: "dept-internal-medicine", label: "Internal Medicine", kind: "department" },
        ...departments.filter((d) => !["Internal Medicine"].includes(d.name)).map((d) => ({ key: `dept:${d.code}`, label: d.name, kind: "department" })),
        ...admissions.map((a) => ({
          key: `care-team:${a.id}`,
...
const PostSchema = z.object({
  ...
  attachments: z.array(z.object({ name: z.string().max(160), size: z.number().int().max(10_000_000), mime: z.string().max(80) })).max(5).optional(),
...
export const POST = withRoute("messages.post", async (req: NextRequest) => {
  ...
  if (d.channel.startsWith("care-team:") && !hasPermission(g.perms, "patient.clinical.view")) {
    return NextResponse.json({ error: "forbidden", detail: "patient_channel_requires_clinical_view" }, { status: 403 });
  }
  ...
  const message = await db.nxMessage.create({
    data: {
      ...
      patientId: d.patientId || (d.channel.startsWith("care-team:") ? d.channel.split(":")[1] : null),
    },
  });
  ...
  if (d.mentions?.length) {
    for (const code of d.mentions.slice(0, 5)) {
      const target = await db.nxStaffUser.findFirst({ where: { hospitalId, staffCode: code.toUpperCase() }, select: { id: true, name: true } });
      if (target) {
        await db.nxNotification.create({
          data: { hospitalId, userId: target.id, title: `${session.name} mentioned you`, body: d.body.slice(0, 140), level: severity === "urgent" ? "warning" : "info", category: "message", link: `messages:${d.channel}` },
        });
      }
    }
  }
  ...
  await audit({
    hospitalId, actorName: session.name, actorRole: session.role,
    action: severity === "urgent" ? "message.post.urgent" : "message.post",
    entityType: "NxMessage", entityId: message.id, patientId: message.patientId || undefined,
  ...
  publish({
    ...
    hospitalId,
    channelKey: d.channel,
    toRoles: ["super_admin", "org_admin", "hospital_admin", "dept_admin", "doctor", "nurse", "care_coordinator", "receptionist", "pharmacist", "lab_tech", "radiology_tech", "billing_officer", "inventory_manager", "hr_manager", "leadership", "command", "facilities", "admin"],
    data: { id: message.id, channel: d.channel, sender: session.name, severity, preview: d.body.slice(0, 80) },
  });
...
export const PATCH = withRoute("messages.manage", async (req: NextRequest) => {
  ...
  if ("error" in g) return NextResponse.json({ error: g.error, detail: g.detail }, { status: g.status });
  const session = g.session;
  const body = await req.json().catch(() => ({})) as { messageId?: string; pin?: boolean; readChannel?: string };

  if (body.readChannel) {
    const since = new Date(Date.now() - 7 * 24 * 3600_000);
    const unread = await db.nxMessage.findMany({
      where: { hospitalId: session.hospitalId!, channelKey: body.readChannel, senderUserId: { not: session.userId }, createdAt: { gte: since }, reads: { none: { userId: session.userId } } },
      select: { id: true },
      take: 200,
    });
    if (unread.length) {
      await db.nxMessageRead.createMany({ data: unread.map((m) => ({ messageId: m.id, userId: session.userId })) });
    }
    await db.nxChannelMember.updateMany({ where: { userId: session.userId, channel: { key: body.readChannel } }, data: { lastReadAt: new Date() } }).catch(() => {});
    return NextResponse.json({ data: { read: unread.length } });
  }

  if (body.messageId && typeof body.pin === "boolean") {
    const msg = await db.nxMessage.findFirst({ where: { id: body.messageId, hospitalId: session.hospitalId! } });
    if (!msg) return fail("not_found", 404, "Message not found.");
    const updated = await db.nxMessage.update({ where: { id: msg.id }, data: { pinned: body.pin } });
    await audit({ hospitalId: session.hospitalId!, actorName: session.name, actorRole: session.role, action: body.pin ? "message.pin" : "message.unpin", entityType: "NxMessage", entityId: msg.id, detail: { channel: msg.channelKey } });
