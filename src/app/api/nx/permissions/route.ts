...
import { PERMISSIONS, ROLE_LABELS, ROLE_PERMISSIONS, invalidatePermCache, type NxRole } from "@/lib/nx/session";
...
export const GET = withRoute("permissions.matrix", async (req: NextRequest) => {
  ...
  const [staff, assignments, grants, delegations] = await Promise.all([
    db.nxStaffUser.findMany({ where: { hospitalId }, select: { id: true, staffCode: true, name: true, role: true, department: true }, orderBy: { name: "asc" } }),
    db.nxUserRoleAssignment.findMany({ where: { user: { hospitalId }, OR: [{ expiresAt: null }, { expiresAt: { gt: new Date() } }] }, include: { user: { select: { staffCode: true, name: true } } } }),
    db.nxPermissionGrant.findMany({ where: { user: { hospitalId }, OR: [{ expiresAt: null }, { expiresAt: { gt: new Date() } }] }, include: { user: { select: { staffCode: true, name: true } } } }),
    db.nxDelegation.findMany({ where: { revokedAt: null, expiresAt: { gt: new Date() }, OR: [{ granter: { hospitalId } }, { delegate: { hospitalId } }] }, include: { granter: { select: { staffCode: true, name: true } }, delegate: { select: { staffCode: true, name: true } } } }),
  ]);
...
export const POST = withRoute("permissions.role.assign", async (req: NextRequest) => {
  ...
  if (parsed.data.action === "assign") {
    ...
    await db.nxUserRoleAssignment.create({
      data: { userId: user.id, roleKey: parsed.data.roleKey, grantedBy: g.session.name, expiresAt: parsed.data.expiresAt ? new Date(parsed.data.expiresAt) : null },
    });
  } else {
    await db.nxUserRoleAssignment.deleteMany({ where: { userId: user.id, roleKey: parsed.data.roleKey } });
  }
  invalidatePermCache(user.id);
  await audit({ hospitalId, actorName: g.session.name, actorRole: g.session.role, action: `permission.role.${parsed.data.action}`, entityType: "nx_staff_user", entityId: user.id, detail: { roleKey: parsed.data.roleKey, staff: user.staffCode, expiresAt: parsed.data.expiresAt ?? null } });
...
export const PATCH = withRoute("permissions.grant", async (req: NextRequest) => {
  ...
  if (!parsed.success) return fail("invalid_request", 400, parsed.error.issues[0]?.message);
  if (!(PERMISSIONS as readonly string[]).includes(parsed.data.permission)) return fail("invalid_request", 400, "Unknown permission key.");
  const user = await db.nxStaffUser.findFirst({ where: { id: parsed.data.userId, hospitalId } });
  if (!user) return fail("not_found", 404, "Staff member not found.");

  await db.nxPermissionGrant.deleteMany({ where: { userId: user.id, permission: parsed.data.permission } });
  if (parsed.data.effect === "allow" || parsed.data.reason) {
    await db.nxPermissionGrant.create({
      data: {
        userId: user.id, permission: parsed.data.permission, effect: parsed.data.effect,
        reason: parsed.data.reason, grantedBy: g.session.name,
        expiresAt: parsed.data.hours ? new Date(Date.now() + parsed.data.hours * 3600_000) : null,
      },
    });
  }
  invalidatePermCache(user.id);
  await audit({ hospitalId, actorName: g.session.name, actorRole: g.session.role, action: `permission.${parsed.data.effect}`, entityType: "nx_staff_user", entityId: user.id, detail: { permission: parsed.data.permission, staff: user.staffCode, reason: parsed.data.reason ?? null } });
  return NextResponse.json({ data: { done: true } });
});
...
export const DELETE = withRoute("permissions.delegation.revoke", async (req: NextRequest) => {
  ...
  const del = await db.nxDelegation.findFirst({ where: { id, OR: [{ granter: { hospitalId } }, { delegate: { hospitalId } }] } });
  ...
  await audit({ hospitalId, actorName: g.session.name, actorRole: g.session.role, action: "permission.delegation.revoke", entityType: "nx_delegation", entityId: id });
