...
export const GET = withRoute("billing.v2.list", async (req: NextRequest) => {
  ...
  if (kind === "export") {
    if (!hasPermission(g.perms, "reports.export"))
      return fail("forbidden", 403, "Export requires reports.export permission.");
  ...
  if (kind === "charges") {
    ...
    const patients = await db.hospitalPatient.findMany({
      ...
      select: { id: true, uhid: true, fullName: true },
    });
  ...
  const [billsAgg, pendingCharges, paymentsAgg, claims] = await Promise.all([
    ...
    db.nxPayment.aggregate({
      where: { hospitalId, refundOfId: null },
      _sum: { amount: true },
      _count: true,
    }),
  ...
  return NextResponse.json({
    data: {
      summary: {
        ...
        claimsByStatus: claims.reduce<Record<string, number>>((acc, c) => {
          acc[c.preAuthStatus] = c._count;
          return acc;
        }, {}),
      },
    },
...
const ChargeSchema = z.object({
  ...
  category: z
    .enum(["room", "procedure", "pharmacy", "lab", "consultation", "consumable", "general"])
    .default("general"),
...
export const POST = withRoute("billing.charge.create", async (req: NextRequest) => {
  ...
  const patient = await db.hospitalPatient.findFirst({
    where: { id: parsed.data.patientId, hospitalId },
  ...
  await audit({
    ...
    entityType: "nx_charge",
    entityId: charge.id,
    patientId: patient.id,
    detail: {
      amount: parsed.data.unitPrice * parsed.data.quantity,
      category: parsed.data.category,
    },
  });
...
export const PUT = withRoute("billing.payment.record", async (req: NextRequest) => {
  ...
  if (parsed.data.refundOfId) {
    const original = await db.nxPayment.findFirst({
      where: { id: parsed.data.refundOfId, hospitalId },
    });
    if (!original) return fail("not_found", 404, "Original payment not found for refund.");
    if (parsed.data.amount > original.amount)
      return fail("invalid_request", 422, "Refund cannot exceed the original payment.");
  }
  ...
  return withIdempotency(
    ...
    async () => {
      ...
      const payment = await db.$transaction(async (tx) => {
        const created = await tx.nxPayment.create({
          data: {
            ...
            refundOfId: parsed.data.refundOfId,
            receivedBy: g.session.name,
          },
        });
        if (parsed.data.billId) {
          const bill = await tx.hospitalBill.findFirst({
            where: { id: parsed.data.billId, hospitalId },
          });
          if (bill) {
            const payments = await tx.nxPayment.aggregate({
              where: { billId: bill.id, refundOfId: null },
              _sum: { amount: true },
            });
            const paid = payments._sum.amount ?? 0;
            await tx.hospitalBill.update({
              where: { id: bill.id },
              data: {
                paymentStatus:
                  paid >= bill.totalPayable ? "paid" : paid > 0 ? "partial" : bill.paymentStatus,
              },
            });
          }
        }
      ...
      await audit({
        ...
        actorName: g.session.name,
        actorRole: g.session.role,
        action: parsed.data.refundOfId ? "billing.refund" : "billing.payment",
        entityType: "nx_payment",
        entityId: payment.id,
        patientId: patient.id,
        detail: { amount: parsed.data.amount, mode: parsed.data.mode },
      });
      return {
        status: 201,
        body: {
