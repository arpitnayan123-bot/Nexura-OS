...
export const GET = withRoute("notes.get", async (req: NextRequest, { requestId }) => {
  ...
  const canRestricted =
    hasPermission(gate.perms, "patient.restricted.view") || gate.session.breakGlass;
  return NextResponse.json({
    data: {
      note: {
        ...
        author: note.doctorId,
        signedAt: note.signedAt,
        signedByName: note.signedByName,
        signedByRole: note.signedByRole,
...
export const PATCH = withRoute("notes.mutate", async (req: NextRequest) => {
  ...
  if (note.locked && body.data.action === "save") {
    return fail(
      "note_locked",
      423,
      "This note is signed and locked. Create an addendum or an amending note instead.",
    );
  }
  ...
  if (body.data.action === "save") {
    ...
    if (session.hospitalId) {
      await audit({
        ...
        entityId: note.id,
        patientId: note.patientId,
        detail: { version: note.currentVersion + 1 },
      });
    }
  ...
  if (body.data.action === "sign") {
    ...
    if (session.hospitalId) {
      ...
      publish({
        event: "note.signed",
        hospitalId: session.hospitalId,
        toRoles: ["doctor", "nurse", "command", "admin", "hospital_admin"],
        data: { noteId: note.id, patientId: note.patientId },
      });
    }
    const fresh = await db.clinicalNote.findUnique({ where: { id: note.id } });
  ...
  if (!note.locked)
    return fail("not_signed", 409, "Addenda apply to signed notes. Save your draft instead.");
  if (!body.data.body || body.data.body.trim().length < 3)
    return fail("invalid_request", 400, "Addendum text is required.");
  ...
  await db.nxNoteVersion.create({
    data: {
      ...
      authorRole: session.role,
      signedAt: new Date(),
      snapshot: JSON.stringify({ fullText: addendum.fullText }),
    },
  });
  if (session.hospitalId) {
    await audit({
...
export const POST = withRoute("notes.create", async (req: NextRequest) => {
  ...
  const patient = await db.hospitalPatient.findFirst({
    where: { id: body.data.patientId, hospitalId: session.hospitalId },
  ...
  await db.nxNoteVersion.create({
    data: {
      ...
      authorName: session.name,
      authorRole: session.role,
      snapshot: JSON.stringify({ fullText: note.fullText, noteType: note.noteType }),
    },
  });
  await audit({
    hospitalId: session.hospitalId,
