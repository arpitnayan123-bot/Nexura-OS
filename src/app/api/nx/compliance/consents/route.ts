...
export const POST = withRoute("compliance.consents.capture", async (req: NextRequest, { requestId }) => {
  const g = await guard(req, "consent.manage");
  if ("response" in g) return g.response;
  const hospitalId = g.session.hospitalId;
  if (!hospitalId) return fail("no_hospital_context", 403, undefined, requestId);
  ...
  await audit({ hospitalId, actorName: g.session.name, actorRole: g.session.role, action: "consent.capture", entityType: "nx_consent", entityId: consent.id, patientId: patient.id, detail: { type: body.data.type, granted: body.data.granted } });
