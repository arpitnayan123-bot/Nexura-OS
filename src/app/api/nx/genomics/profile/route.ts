...
const ComputeSchema = z.object({
  ...
  familyHistory: z.array(z.enum(["cardiac", "diabetes", "cancer", "mental_health", "none"])).max(6).default([]),
});

const CLASS_WEIGHT: Record<string, number> = { pathogenic: 0.35, "risk-allele": 0.15, carrier: 0.1, pharmacogenomics: 0.05, negative: 0 };
const FH_WEIGHT: Record<string, number> = { cardiac: 0.15, diabetes: 0.12, cancer: 0.15, mental_health: 0.08, none: 0 };
...
export const GET = withRoute("genomics.profile.get", async (req: NextRequest, { requestId }) => {
  ...
  const profile = await db.nxGenomicProfile.findFirst({ where: { hospitalId, patientId }, orderBy: { createdAt: "desc" } });
  return ok({
    profile,
    governance: { vaultOnly: "raw genotype never leaves the partner vault", consentRequired: "genomics consent must be granted before computation", standards: "GDPR art.9 / DPDP sensitive-personal-data / HIPAA GINA" },
  }, { requestId });
});

export const POST = withRoute("genomics.profile.compute", async (req: NextRequest, { requestId }) => {
  const g = await guard(req, "patient.restricted.view");
  if ("response" in g) return g.response;
  const hospitalId = g.session.hospitalId;
  if (!hospitalId) return fail("no_hospital_context", 403, undefined, requestId);
  ...
  const consent = await db.nxConsent.findFirst({ where: { hospitalId, patientId: patient.id, type: { in: ["genomics", "data_share"] }, status: "granted" } });
  if (!consent) {
    return fail("consent_required", 403, "Genomic computation requires explicit patient consent (capture via /api/nx/compliance/consents).", requestId);
  }
  ...
  const risk = {
    ...
    pharmacogenomics: body.data.variants.filter((v) => v.classpath === "pharmacogenomics").map((v) => `${v.gene} ${v.variant}`),
  };
  const vaultRef = `vault://genomics/${patient.id.slice(0, 12)}-${Date.now().toString(36)}`;
  const profile = await db.nxGenomicProfile.create({
    data: {
      hospitalId, patientId: patient.id, vaultRef,
      variantsJson: JSON.stringify(body.data.variants),
      riskJson: JSON.stringify(risk), consentId: consent.id,
    },
  });
  await audit({ hospitalId, actorName: g.session.name, actorRole: g.session.role, action: "genomics.risk.compute", entityType: "nx_genomic_profile", entityId: profile.id, patientId: patient.id, detail: { vaultRef } });
  return ok({ profileId: profile.id, vaultRef, risk, disclaimer: "Derived risk signals require clinical genetics interpretation before any patient communication." }, { requestId, status: 201 });
});
