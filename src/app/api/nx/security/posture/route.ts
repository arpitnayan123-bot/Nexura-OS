...
export const GET = withRoute("security.posture", async (req: NextRequest, { requestId }) => {
  ...
  add({
    id: "jwt_secret", title: "Session signing secret",
    status: secret && secret.length >= 32 ? "pass" : secret ? "warn" : "fail",
    detail: secret && secret.length >= 32 ? "Configured (>=32 chars)" : "Weak or missing — sessions forgeable in prod",
    weight: 20,
  });

  add({
    id: "env", title: "Runtime environment",
  ...
  add({
    id: "db", title: "Database durability profile",
  ...
  const mfaUsers = hospitalId ? await db.nxStaffUser.count({ where: { hospitalId, mfaEnabled: true } }) : 0;
  const mfaPct = totalUsers ? Math.round((mfaUsers / totalUsers) * 100) : 0;
  add({
    id: "mfa", title: "MFA adoption (privileged staff)",
  ...
  const idleSessions = hospitalId
    ? await db.nxSessionRecord.count({ where: { revokedAt: null, expiresAt: { gt: new Date() }, lastSeenAt: { lt: new Date(Date.now() - 24 * 3600_000) } } })
    : 0;
  add({
    id: "sessions", title: "Stale live sessions (>24h silent)",
  ...
  const failedLogins = hospitalId
    ? await db.nxLoginAttempt.count({ where: { createdAt: { gte: new Date(Date.now() - 24 * 3600_000) }, success: false } }).catch(() => -1)
    : 0;
  add({
    id: "brute_force", title: "Failed logins (24h)",
    status: typeof failedLogins === "number" && failedLogins >= 0 ? (failedLogins < 50 ? "pass" : failedLogins < 200 ? "warn" : "fail") : "unknown",
    detail: `${failedLogins} attempts — sustained spikes suggest credential stuffing`,
    weight: 10,
  });

  const openBg = hospitalId ? await db.nxBreakGlassEvent.count({ where: { revokedAt: null, expiresAt: { gt: new Date() } } }).catch(() => -1) : 0;
  add({
    id: "break_glass", title: "Open break-glass events",
    status: typeof openBg === "number" && openBg >= 0 ? (openBg === 0 ? "pass" : "warn") : "unknown",
    detail: `${openBg} open — each requires post-hoc review`,
    weight: 10,
  });

  add({
    id: "webhook_inbound", title: "Partner webhook secret",
    status: process.env.NX_INBOUND_WEBHOOK_SECRET ? "pass" : "warn",
    detail: process.env.NX_INBOUND_WEBHOOK_SECRET ? "Inbound pushes authenticated" : "Inbound partner pushes disabled (no secret)",
    weight: 5,
  });

  add({
    id: "dep_scan", title: "Dependency CVE scan",
    status: "unknown",
  ...
  add({
    id: "sse", title: "Live SSE connections (signed alerts)",
    status: "pass",
  ...
  const score = Math.round(
    (checks.reduce((s, c) => s + (c.status === "pass" ? c.weight : c.status === "warn" ? c.weight / 2 : c.status === "unknown" ? c.weight / 3 : 0), 0) /
      checks.reduce((s, c) => s + c.weight, 0)) * 100
  );
  return ok({
    score,
