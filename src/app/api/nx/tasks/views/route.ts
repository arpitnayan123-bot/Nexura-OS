...
export const GET = withRoute("tasks.views.list", async (req: NextRequest) => {
  ...
  const views = await db.nxTaskView.findMany({
    where: { userId: g.session.userId },
    orderBy: { createdAt: "desc" },
  });
  return ok({
    views: views.map((v) => ({ id: v.id, name: v.name, filters: JSON.parse(v.filters || "{}") })),
  });
});
...
export const POST = withRoute("tasks.views.save", async (req: NextRequest) => {
  ...
  const existing = await db.nxTaskView.findFirst({
    where: { userId: g.session.userId, name: body.data.name },
  });
  if (existing) {
    await db.nxTaskView.update({
      where: { id: existing.id },
      data: { filters: JSON.stringify(body.data.filters) },
    });
    return NextResponse.json({ data: { saved: true, updated: true } });
  }
  await db.nxTaskView.create({
    data: {
      userId: g.session.userId,
      name: body.data.name,
      filters: JSON.stringify(body.data.filters),
    },
  });
