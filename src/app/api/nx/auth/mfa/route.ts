...
export const POST = withRoute("auth.mfa.enroll", async (req: NextRequest) => {
  ...
  if (user.mfaEnabled) return fail("mfa_already_enabled", 400, "MFA is already active on this account.");
  const secret = generateTotpSecret();
  await db.nxStaffUser.update({ where: { id: user.id }, data: { mfaSecret: secret, mfaEnabled: false } });
  return ok({
    enrollment: {
      secret,
      otpauthUrl: otpauthUrl(secret, user.email ?? user.staffCode),
      instructions: "Add this secret to your authenticator app, then confirm with a 6-digit code to activate.",
    },
  });
});
...
export const PUT = withRoute("auth.mfa.activate", async (req: NextRequest) => {
  ...
  if (!parsed.success) return fail("invalid_request", 400, "A 6-digit authenticator code is required.");
  const user = await db.nxStaffUser.findUnique({ where: { id: g.session.userId } });
  if (!user?.mfaSecret) return fail("mfa_not_enrolled", 400, "Start MFA enrollment first.");
  if (!verifyTotp(user.mfaSecret, parsed.data.code)) return fail("invalid_mfa_code", 400, "That code didn't match. Try the next one.");
  await db.nxStaffUser.update({ where: { id: user.id }, data: { mfaEnabled: true } });
  if (user.hospitalId) {
    await audit({ hospitalId: user.hospitalId, actorName: user.staffCode, actorRole: user.role, action: "auth.mfa.enabled", entityType: "nx_staff_user", entityId: user.id });
  }
...
export const DELETE = withRoute("auth.mfa.disable", async (req: NextRequest, { requestId }) => {
  ...
  if (user.passwordHash) {
    ...
    if (!okPw) return fail("invalid_credentials", 401, "Enter your account password to disable MFA.");
  } else {
    ...
    if (!stepUp.ok) {
      return fail("stepup_required", 401, "Verify with your PIN or authenticator code to disable MFA.", requestId);
    }
  }
  await db.nxStaffUser.update({ where: { id: user.id }, data: { mfaEnabled: false, mfaSecret: null } });
  if (user.hospitalId) {
    await audit({ hospitalId: user.hospitalId, actorName: user.staffCode, actorRole: user.role, action: "auth.mfa.disabled", entityType: "nx_staff_user", entityId: user.id });
  }
