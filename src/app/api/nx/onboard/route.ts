...
const OnboardSchema = z.object({
  ...
  tenantCode: z.string().max(48).regex(/^[a-z0-9-]+$/).optional(),
...
export const POST = withRoute("onboard.apply", async (req: NextRequest, { requestId }) => {
  ...
  const template = await db.nxHospitalTemplate.findUnique({ where: { code: body.data.templateCode } });
  if (!template) return fail("unknown_template", 404, undefined, requestId);
  const config = JSON.parse(template.configJson || "{}") as { modules?: string[]; locales?: string[]; offlineMode?: boolean };
  let tenantId: string | undefined;
  if (body.data.tenantCode) {
    const tenant = await db.nxTenant.upsert({
      where: { code: body.data.tenantCode },
      create: { code: body.data.tenantCode, name: body.data.tenantName ?? body.data.hospitalName, plan: "standard" },
      update: {},
    });
    tenantId = tenant.id;
  }
  const hospital = await db.hospital.create({
    data: {
      name: body.data.hospitalName, state: body.data.state, district: body.data.district,
      ...(tenantId ? { tenantId } : {}),
    },
  });
  await audit({
    hospitalId: g.session.hospitalId ?? hospital.id,
    actorName: g.session.name, actorRole: g.session.role,
    action: "onboard.hospital", entityType: "hospital", entityId: hospital.id,
  ...
  return ok({
    ...
    nextSteps: ["Create departments/wards from the Admin app", "Invite staff (staff ops)", "Configure escalation policies + AI thresholds in Governance"],
