...
export const POST = withRoute("schedule.waitlist.add", async (req: NextRequest) => {
  ...
  await audit({
    ...
    action: "appointment.waitlist.add",
    entityType: "waitlist",
    entityId: entry.id,
    patientId: patient.id,
  });
...
export const PATCH = withRoute("schedule.waitlist.promote", async (req: NextRequest) => {
  ...
  if (!parsed.success) return fail("invalid_request", 400, parsed.error.issues[0]?.message);
  const entry = await db.nxAppointmentWaitlist.findFirst({
    where: { id: parsed.data.id, hospitalId, status: "waiting" },
  });
  ...
  await db.nxAppointmentWaitlist.update({
    where: { id: entry.id },
    data: { status: "promoted", promotedAppointmentId: appt.id },
  });
  await audit({
    hospitalId,
    actorName: g.session.name,
    actorRole: g.session.role,
    action: "appointment.waitlist.promoted",
  ...
  publish({
    ...
    hospitalId,
    toRoles: ["receptionist", "command", "care_coordinator", "doctor"],
    data: { id: appt.id, waitlistPromoted: true },
  });
