...
export const GET = withRoute("nx.schedule.list", async (req: NextRequest) => {
  ...
  return NextResponse.json({
    ...
    appointments: appointments.map((a) => {
      ...
      return {
        id: a.id,
        time: `${dayIso}T${slot.slice(0, 5)}:00`,
        token: a.tokenNumber,
...
export const POST = withRoute("nx.schedule.book", async (req: NextRequest) => {
  ...
  const clash = await db.hospitalAppointment.findFirst({
    where: {
      ...
      doctorId: doctor.id,
      date: {
        gte: new Date(when.getTime() - 7 * 60000),
        lte: new Date(when.getTime() + 7 * 60000),
      },
  ...
  if (clash) {
    ...
    for (const mins of [30, 45, 60, 90, 120]) {
      ...
      const altClash = await db.hospitalAppointment.findFirst({
        where: {
          ...
          doctorId: doctor.id,
          date: {
            gte: new Date(alt.getTime() - 7 * 60000),
            lte: new Date(alt.getTime() + 7 * 60000),
          },
          status: { notIn: ["cancelled", "no_show"] },
        },
      });
    ...
    return NextResponse.json(
      {
        error: "slot_conflict",
        detail: `${doctor.name} already has an appointment in this slot`,
        suggestions,
      },
      { status: 409 },
    );
  ...
  await audit({
    ...
    entityType: "HospitalAppointment",
    entityId: appt.id,
    patientId: patient.id,
...
export const PATCH = withRoute("nx.schedule.transition", async (req: NextRequest) => {
  ...
  if (appt.hospitalId !== gate.session.hospitalId)
    return NextResponse.json({ error: "forbidden", detail: "cross_hospital" }, { status: 403 });
  ...
  if (body.newDate) {
    ...
    const clash = await db.hospitalAppointment.findFirst({
      where: {
        ...
        id: { not: appt.id },
        date: {
          gte: new Date(newDate.getTime() - 7 * 60000),
          lte: new Date(newDate.getTime() + 7 * 60000),
        },
        status: { notIn: ["cancelled", "no_show"] },
      },
    });
    if (clash)
      return NextResponse.json(
        { error: "slot_conflict", detail: "Doctor already booked in that slot" },
        { status: 409 },
      );
  }
  ...
  const updated = await db.hospitalAppointment.update({
    ...
    data: {
      ...
      ...(newDate
        ? {
            date: newDate,
            timeSlot: newDate.toISOString().slice(11, 16),
  ...
  if (body.status === "no_show" && body.addToWaitlist) {
    await db.nxAppointmentWaitlist.create({
      data: {
        ...
        patientName: (
          await db.hospitalPatient.findUnique({
