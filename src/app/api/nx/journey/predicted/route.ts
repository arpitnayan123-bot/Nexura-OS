...
export const GET = withRoute("journey.predicted", async (req: NextRequest, { requestId }) => {
  ...
  if (admission?.admissionDiagnosis) {
    ...
    const los = sameDx.map((a) => ((a.actualDischargeDate?.getTime() ?? 0) - a.admissionDate.getTime()) / 86400_000).filter((d) => d > 0).sort((a, b) => a - b);
  ...
  for (const run of runs) {
    ...
    for (const s of steps) {
      ...
      predictedNodes.push({
        kind: "pathway_step", pathway: def.name, stepId: s.id, title: s.title,
        withinMin: s.withinMin, critical: Boolean(s.critical), ownerRole: s.ownerRole,
      });
    }
  }
  if (projectedLosDays && admission) {
    ...
    predictedNodes.push({
      kind: "projected_discharge", title: "Projected discharge window",
      eta: dischargeEta.toISOString(), basedOn: `median LOS ${projectedLosDays}d for similar admissions`,
    });
  }
  const annotations = await db.nxJourneyAnnotation.count({ where: { hospitalId, patientId, decisionLog: true } });
  return ok({
    patient: { id: patient.id, name: patient.fullName, uhid: patient.uhid },
    activeAdmission: admission ? { id: admission.id, diagnosis: admission.admissionDiagnosis, since: admission.admissionDate } : null,
    predictedNodes,
