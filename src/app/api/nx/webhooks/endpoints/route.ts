...
export const GET = withRoute("webhooks.endpoints.list", async (req: NextRequest, { requestId }) => {
  ...
  const [endpoints, deliveries] = await Promise.all([
    ...
    db.nxWebhookDelivery.findMany({ where: { hospitalId }, orderBy: { createdAt: "desc" }, take: 25 }),
  ]);
  return ok({
    supportedEvents: WEBHOOK_EVENTS,
    endpoints: endpoints.map((e) => ({ ...e, secret: `${e.secret.slice(0, 6)}…`, events: JSON.parse(e.events || "[]") })),
    recentDeliveries: deliveries,
  }, { requestId });
});

export const POST = withRoute("webhooks.endpoints.upsert", async (req: NextRequest, { requestId }) => {
  const g = await guard(req, "settings.manage");
  if ("response" in g) return g.response;
  const hospitalId = g.session.hospitalId;
  if (!hospitalId) return fail("no_hospital_context", 403, undefined, requestId);
  ...
  const data = {
    name: d.name, url: d.url, events: JSON.stringify(d.events),
    active: d.active ?? true, secret,
  };
  const endpoint = d.id
    ? await db.nxWebhookEndpoint.update({ where: { id: d.id }, data: { name: d.name, url: d.url, events: data.events, active: data.active } })
    : await db.nxWebhookEndpoint.create({ data: { ...data, hospitalId } });
  await audit({ hospitalId, actorName: g.session.name, actorRole: g.session.role, action: d.id ? "webhook.update" : "webhook.create", entityType: "nx_webhook_endpoint", entityId: endpoint.id });
  return ok({ id: endpoint.id, secret: d.id ? undefined : secret, note: d.id ? undefined : "Save this signing secret — shown once." }, { requestId });
});

export const DELETE = withRoute("webhooks.endpoints.delete", async (req: NextRequest, { requestId }) => {
  const g = await guard(req, "settings.manage");
  if ("response" in g) return g.response;
  const hospitalId = g.session.hospitalId;
  if (!hospitalId) return fail("no_hospital_context", 403, undefined, requestId);
