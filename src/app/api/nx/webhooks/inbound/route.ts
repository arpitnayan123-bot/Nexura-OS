...
export const POST = withRoute("webhooks.inbound", async (req: NextRequest, { requestId }) => {
  ...
  if (!secret)
    return fail(
      "inbound_disabled",
      503,
      "Set NX_INBOUND_WEBHOOK_SECRET to enable partner pushes.",
      requestId,
    );
      ...
      order = await db.hospitalOrder.create({
        data: {
          ...
          orderDetails: JSON.stringify({
            items: [{ test: r.testName, source: "partner-webhook" }],
          }),
        },
      });
    }
  ...
  publish({
    event: "webhook.ingested",
    hospitalId,
    data: { uhid: d.uhid, results: created.length, source: "partner" },
  });
  await db.nxIntegrationEvent
    .create({
      data: {
        hospitalId,
        source: "webhook",
        event: d.event,
        payload: JSON.stringify({ uhid: d.uhid, count: created.length }).slice(0, 4000),
      },
    })
    .catch(() => {});
