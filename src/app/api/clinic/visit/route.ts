  ...
  try {
    ...
    const visits = await db.clinicVisit.findMany({
      ...
      include: {
        doctor: { select: { id: true, name: true, specialization: true } },
        meds: true,
        invoices: { select: { id: true, invoiceNo: true, total: true, status: true } },
      },
    });
    return NextResponse.json({ visits });
  } catch (err) {
    log.error("clinic", "visit_list_failed", {
      err: err instanceof Error ? err.message : String(err),
    });
    return NextResponse.json(
      { error: "clinic_visit_failed", detail: "Visits could not be loaded. Please retry." },
      { status: 500 },
    );
  }
}
...
async function POST_impl(req: NextRequest) {
  try {
    ...
    log.error("clinic", "visit_create_failed", {
      err: err instanceof Error ? err.message : String(err),
    });
    return NextResponse.json(
      {
        error: "clinic_visit_create_failed",
        detail: "The visit could not be saved. Please retry.",
      },
      { status: 500 },
    );
  }
