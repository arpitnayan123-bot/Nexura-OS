...
const PREVALENCE: Record<string, { prevalencePer1000: number; source: string; ageRange?: [number, number]; genderBias?: "male" | "female" }> = {
  ...
  "Hypothyroidism": { prevalencePer1000: 110, source: "ICMR Thyroid Survey", genderBias: "female", ageRange: [18, 70] },
  "Anemia": { prevalencePer1000: 570, source: "NFHS-5 (57% women)", genderBias: "female", ageRange: [15, 49] },
  ...
  "Coronary Artery Disease": { prevalencePer1000: 35, source: "ICMR CAD Study", ageRange: [40, 80], genderBias: "male" },
  "GERD": { prevalencePer1000: 80, source: "Indian GI Survey", ageRange: [25, 70] },
  "Migraine": { prevalencePer1000: 25, source: "Indian Headache Society", genderBias: "female", ageRange: [18, 55] },
  "Community-Acquired Pneumonia": { prevalencePer1000: 15, source: "ICMR CAP Study" },
  "Acute Pharyngitis": { prevalencePer1000: 40, source: "Indian Clinical Pattern", ageRange: [5, 40] },
...
const COMPLAINT_MAP: Record<string, string[]> = {
  "fever": ["Viral Fever", "Typhoid", "Dengue", "Malaria", "Acute Pharyngitis", "Community-Acquired Pneumonia", "Tuberculosis", "Chikungunya", "Gastroenteritis"],
  ...
  "shortness of breath": ["Asthma", "Coronary Artery Disease", "Anemia", "Community-Acquired Pneumonia"],
  "cough": ["Community-Acquired Pneumonia", "Asthma", "Tuberculosis", "Acute Pharyngitis"],
  "fatigue": ["Anemia", "Diabetes Type 2", "Hypothyroidism", "Depression", "Hypertension", "Chronic Kidney Disease"],
...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const [key, dxs] of Object.entries(COMPLAINT_MAP)) {
      if (complaint.includes(key) || key.includes(complaint)) { matchedDx = dxs; break; }
    }
    if (matchedDx.length === 0) {
      const words = complaint.split(/\s+/);
      for (const word of words) {
        for (const [key, dxs] of Object.entries(COMPLAINT_MAP)) {
          if (key.includes(word) || word.includes(key)) { matchedDx = [...new Set([...matchedDx, ...dxs])]; }
        }
      }
    }

    if (matchedDx.length === 0) {
      return NextResponse.json({ differentials: [], message: "No matching diagnoses. Please assess manually." });
    }
    ...
    const differentials = matchedDx.map((dx) => {
      ...
      if (prev.ageRange && age !== null) {
        if (age < prev.ageRange[0] || age > prev.ageRange[1]) { score *= 0.3; ageMatch = false; }
        else { score *= 1.2; }
      }
      if (prev.genderBias && gender) {
        if (prev.genderBias !== gender) { score *= 0.6; genderMatch = false; }
        else { score *= 1.3; }
      }
      ...
      const reasoning = [label, `${prev.prevalencePer1000}/1000 (${prev.source})`, !ageMatch ? "Outside age range" : null, !genderMatch ? "Opposite gender bias" : null].filter(Boolean).join(" · ");

      return { diagnosis: dx, score: Math.round(score), rank, label, prevalencePer1000: prev.prevalencePer1000, prevalenceSource: prev.source, ageMatch, genderMatch, reasoning };
    });
