...
> = {
  Hypertension: {
    ...
    whenToRefer:
      "Refer to cardiologist if: BP >180/120 (hypertensive emergency), resistant HTN on 3 drugs, suspected secondary HTN, age <30 with HTN",
    monitoring:
      "Monitor BP every 2 weeks until controlled, then every 3 months. Annual: fundoscopy, ECG, serum creatinine, electrolytes, urine albumin",
  ...
  "Diabetes Type 2": {
    ...
    whenToRefer:
      "Refer if: HbA1c >9% despite 3 drugs, recurrent hypoglycemia, ketones, foot ulcer, nephropathy (eGFR <60)",
    monitoring:
      "HbA1c every 3 months. Fasting & PP blood sugar monthly. Annual: lipid profile, serum creatinine, urine microalbumin, fundus exam, foot exam",
  ...
  Fever: {
    ...
    whenToRefer:
      "Refer if: fever >5 days, temp >103°F, bleeding signs, BP <90 systolic, altered sensorium, pregnancy",
    monitoring:
      "Temperature every 6 hours. CBC if fever >3 days. Dengue NS1 if fever <5 days in endemic area. Widal if >7 days",
  ...
  "Community-Acquired Pneumonia": {
    ...
    firstLine: [
      {
        drug: "Amoxicillin+Clavulanic Acid 625mg",
        dose: "625mg BD",
        duration: "5-7 days",
        route: "Oral",
      },
    ...
    secondLine: [{ drug: "Cefixime 200mg", dose: "200mg BD", duration: "5-7 days", route: "Oral" }],
    nonPharmacological: [
      "Adequate hydration",
      "Rest",
    ...
    whenToRefer:
      "Refer if: CURB-65 score ≥2, SpO₂ <90%, age >65 with comorbidities, no improvement in 72 hours",
    monitoring:
      "Temperature, SpO₂, respiratory rate daily. Chest X-ray if not improving. CBC, CRP baseline",
  ...
  GERD: {
    ...
    whenToRefer:
      "Refer if: dysphagia, weight loss, anemia, hematemesis, recurrent despite 8 weeks PPI, age >55 with new symptoms",
  ...
  Hypothyroidism: {
    ...
    firstLine: [
      {
        drug: "Levothyroxine (Thyronorm)",
        dose: "1.6 mcg/kg/day (start 50mcg OD)",
        duration: "Lifelong",
    ...
    whenToRefer:
      "Refer if: pregnancy (dose needs adjustment), cardiac disease (start 25mcg), myxedema coma, papillary thyroid carcinoma",
    monitoring:
      "TSH after 6 weeks of starting → adjust dose. Once stable: TSH every 6 months. Free T4 if needed",
  ...
  Asthma: {
    ...
    firstLine: [
      {
        drug: "Budesonide+Formoterol (Foracort) 200/6mcg",
        dose: "1 puff BD via inhaler with spacer",
        duration: "Continue",
        route: "Inhalation",
      },
    ],
    secondLine: [
      {
        drug: "Salbutamol (Asthalin) 100mcg",
        dose: "2 puffs PRN",
        duration: "PRN (rescue)",
        route: "Inhalation",
      },
    ...
    whenToRefer:
      "Refer if: not controlled on Step 3 (ICS+LABA+LTRA), frequent exacerbations, ICU admission needed, suspected COPD overlap",
    monitoring:
      "Symptom assessment every 3 months. Peak expiratory flow rate. Spirometry annually. Check inhaler technique every visit",
    icmrTarget:
      "GINA Step-wise: Step up if >2 reliever uses/week. Step down if controlled 3 months. Target: no night symptoms, no exacerbations",
  ...
  "Coronary Artery Disease": {
    ...
    firstLine: [
      {
        drug: "Aspirin (Ecosprin) 75mg",
        dose: "75mg OD after food",
        duration: "Lifelong",
        route: "Oral",
      },
    ...
    secondLine: [
      {
        drug: "Clopidogrel (Clopitab) 75mg",
        dose: "75mg OD (if stented: 12 months DAPT)",
        duration: "Lifelong/12mo",
        route: "Oral",
      },
      { drug: "Metoprolol 25mg", dose: "25mg BD", duration: "Lifelong", route: "Oral" },
    ],
    ...
    whenToRefer:
      "Refer to cardiologist if: unstable angina, MI, planned PCI/CABG, LVEF <40%, recurrent symptoms despite optimal therapy",
    monitoring:
      "BP, HR every visit. Lipid profile every 3 months (target LDL <70). LFT, renal function every 6 months. ECG annually. Echo if symptoms change",
    icmrTarget:
      "LDL target: <70 mg/dL (high risk). HbA1c <7%. BP <130/80. All 3 = optimal secondary prevention (ICMR)",
    pregnancyCategory:
      "Aspirin: avoid in 3rd trimester. Statins: contraindicated. Use heparin instead",
  ...
  Migraine: {
    ...
    secondLine: [
      ...
      {
        drug: "Propranolol 40mg (prophylaxis)",
        dose: "40mg BD",
        duration: "3-6 months if >4 attacks/month",
        route: "Oral",
      },
    ],
    nonPharmacological: [
    ...
    whenToRefer:
      "Refer if: new onset after age 50, worst headache ever, neurological deficit, fever + headache, visual changes, pattern change",
    monitoring:
      "Headache diary (frequency, triggers, response). Assess every 3 months. Consider prophylaxis if >4 attacks/month or >4 headache days/month",
    icmrTarget:
      "Prophylaxis: Propranolol 40mg BD OR Amitriptyline 10mg HS OR Flunarizine 10mg HS. Choose based on comorbidities",
    pregnancyCategory:
      "Paracetamol is safe. Avoid NSAIDs in 3rd trimester. Propranolol: weigh benefit/risk",
  ...
  Anemia: {
    ...
    firstLine: [
      {
        drug: "Ferrous Sulphate 325mg",
        dose: "1 tablet OD with Vit C",
    ...
    secondLine: [
      {
        ...
        dose: "100mg IV alternate days",
        duration: "Till total dose given",
        route: "IV (if oral intolerant)",
      },
    ],
    nonPharmacological: [
    ...
    whenToRefer:
      "Refer if: Hb <7 g/dL, no response to oral iron in 4 weeks, underlying cause (fibroids, GI bleed), pregnancy with Hb <8",
    monitoring:
      "Hb every 4 weeks. Reticulocyte count at 7-10 days (response marker). Ferritin, TIBC at baseline. Continue iron 3 months after Hb normal",
    icmrTarget:
      "NFHS-5: 57% Indian women anemic. Target Hb: >12 (women), >13 (men). Screen all pregnant women, adolescent girls",
    pregnancyCategory:
      "Iron supplementation mandatory in pregnancy (IFA tablet: 60mg iron + 0.5mg folic acid daily)",
...
async function GET_impl(req: NextRequest) {
  try {
    ...
    if (diagnosis) {
      // fuzzy match
      let key = Object.keys(NFI_GUIDELINES).find(
        (k) => k.toLowerCase() === diagnosis.toLowerCase(),
      );
      if (!key)
      ...
      if (key) {
        return NextResponse.json({
          guideline: NFI_GUIDELINES[key],
          source: "National Formulary of India 2024",
        });
      }
      return NextResponse.json({
        guideline: null,
        message: "No NFI guideline found for this diagnosis",
      });
    }

    // return list of available guidelines
    return NextResponse.json({
      guidelines: Object.keys(NFI_GUIDELINES).map((k) => ({
        diagnosis: k,
        icd10: NFI_GUIDELINES[k].icd10,
      })),
      count: Object.keys(NFI_GUIDELINES).length,
      source: "National Formulary of India — Ministry of Health & Family Welfare",
    });
  } catch (err) {
    log.error("clinic", "nfi_guidelines_failed", {
      err: err instanceof Error ? err.message : String(err),
    });
    return NextResponse.json(
      { error: "nfi_failed", detail: "NFI guidelines could not be loaded. Please retry." },
      { status: 500 },
    );
  }
}
