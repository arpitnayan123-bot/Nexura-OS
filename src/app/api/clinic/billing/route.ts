...
async function GET_impl() {
  try {
    ...
    const [bills, paid, unpaid] = await Promise.all([
      db.clinicInvoice.findMany({
        where: { clinicId: ctx.clinic.id },
        orderBy: { createdAt: "desc" },
        take: 30,
        include: { patient: { select: { id: true, mrn: true, name: true } } },
      }),
    ...
    return NextResponse.json({
      ...
      summary: {
        collected: (paid._sum.total ?? 0) / 100,
        outstanding: (unpaid._sum.total ?? 0) / 100,
        count: bills.length,
      },
    });
  } catch (err) {
    log.error("clinic", "billing_list_failed", {
      err: err instanceof Error ? err.message : String(err),
    });
    return NextResponse.json(
      { error: "clinic_billing_failed", detail: "Invoices could not be loaded. Please retry." },
      { status: 500 },
    );
  }
}
