...
const NFI_GUIDELINES: Record<string, {
  ...
  "Hypertension": {
    ...
    notes: "Start single agent. If BP uncontrolled in 2 weeks, add second drug. Target <140/90 (ICMR).",
  },
  "Diabetes Type 2": {
    firstLine: [{ medicine: "Glycomet", salt: "Metformin", dosage: "500mg BD", duration: "Continue" }],
    secondLine: [
  ...
  "Type 2 Diabetes Mellitus": {
    firstLine: [{ medicine: "Glycomet", salt: "Metformin", dosage: "500mg BD", duration: "Continue" }],
    secondLine: [{ medicine: "Glimisave", salt: "Glimepiride", dosage: "2mg OD", duration: "Continue" }],
  ...
  "Fever": {
    ...
    secondLine: [{ medicine: "Combiflam", salt: "Ibuprofen+Paracetamol", dosage: "1 BD", duration: "3 days" }],
    notes: "If fever >5 days, investigate. Avoid NSAIDs if dengue suspected.",
  },
  "Viral Fever": {
    firstLine: [{ medicine: "Dolo", salt: "Paracetamol", dosage: "650mg TID", duration: "3-5 days" }],
    secondLine: [],
  ...
  "Common Cold": {
    ...
    secondLine: [{ medicine: "Ascoril", salt: "Bromhexine+Guaifenesin", dosage: "10ml TID", duration: "5 days" }],
  ...
  "Acute Pharyngitis": {
    ...
    secondLine: [{ medicine: "Azithral", salt: "Azithromycin", dosage: "500mg OD", duration: "3 days" }],
    notes: "Centor score ≥3 → consider antibiotic. Throat swab if recurrent.",
  },
  "Community-Acquired Pneumonia": {
    firstLine: [
      { medicine: "Augmentin", salt: "Amoxicillin+Clavulanic Acid", dosage: "625mg BD", duration: "5-7 days" },
  ...
  "Gastroenteritis": {
    firstLine: [
      { medicine: "ORS", salt: "Oral Rehydration Solution", dosage: "As needed", duration: "3 days" },
      { medicine: "Metrogyl", salt: "Metronidazole", dosage: "400mg TID", duration: "5 days" },
    ],
    secondLine: [{ medicine: "Oflox", salt: "Ofloxacin", dosage: "200mg BD", duration: "3-5 days" }],
  ...
  "Acid Peptic Disease": {
    ...
    secondLine: [{ medicine: "Razo", salt: "Rabeprazole", dosage: "20mg OD", duration: "2-4 weeks" }],
    notes: "Lifestyle modification. H. pylori testing if recurrent.",
  },
  "GERD": {
    firstLine: [{ medicine: "Pan", salt: "Pantoprazole", dosage: "40mg OD", duration: "4-8 weeks" }],
    secondLine: [{ medicine: "Razo", salt: "Rabeprazole", dosage: "20mg OD", duration: "4-8 weeks" }],
  ...
  "Hypothyroidism": {
    firstLine: [{ medicine: "Thyronorm", salt: "Levothyroxine", dosage: "50mcg OD", duration: "Continue" }],
    secondLine: [{ medicine: "Eltroxin", salt: "Levothyroxine", dosage: "100mcg OD", duration: "Continue" }],
    notes: "Empty stomach. Check TSH after 6 weeks.",
  },
  "Asthma": {
    firstLine: [{ medicine: "Foracort", salt: "Budesonide+Formoterol", dosage: "200/6mcg 1-0-1 inhaler", duration: "Continue" }],
    secondLine: [{ medicine: "Asthalin", salt: "Salbutamol", dosage: "100mcg PRN inhaler", duration: "PRN" }],
  ...
  "Anemia": {
    ...
    secondLine: [{ medicine: "Zincovit", salt: "Multivitamin+Zinc", dosage: "1 OD", duration: "1 month" }],
...
const NPPA_PRICES: Record<string, { pricePerStrip: number; controlled: boolean; ceilingPrice?: number }> = {
...
async function POST_impl(req: NextRequest) {
  try {
    ...
    const patientCurrentMeds: string[] = Array.isArray(body?.patientCurrentMeds) ? body.patientCurrentMeds : [];
    ...
    if (!key) {
      return NextResponse.json({ suggestions: [], guidelines: null, message: "No NFI guidelines found for this diagnosis. Please prescribe manually." });
    }
    ...
    // fetch from drug DB
    const dbDrugs = await db.indianDrug.findMany({ where: { brandName: { in: allSuggestions.map(s => s.medicine) } } });
    ...
    const suggestions = allSuggestions.map((s, idx) => {
      ...
      const isAllergyConflict = patientAllergies.some(a => s.salt.toLowerCase().includes(a.toLowerCase()) || s.medicine.toLowerCase().includes(a.toLowerCase()));
      const isDuplicate = patientCurrentMeds.some(m => s.salt.toLowerCase().includes(m.toLowerCase()) || m.toLowerCase().includes(s.salt.toLowerCase()));
      const isBannedFDC = BANNED_FDCS.some(b => s.salt.toLowerCase().includes(b.toLowerCase().split("+")[0]) && s.salt.includes("+"));
      ...
      const pricePerDay = nppa ? Math.round((nppa.pricePerStrip / tabletsPerStrip) * dosagePerDay * 100) / 100 : null;
      ...
      return {
        medicine: s.medicine, salt: s.salt, dosage: s.dosage, duration: s.duration,
        line: idx < guidelines.firstLine.length ? "first" : "second",
        rank, pricePerStrip: nppa?.pricePerStrip || null, pricePerDay, nppaControlled: nppa?.controlled || false,
        ceilingPrice: nppa?.ceilingPrice || null, schedule: drug?.schedule || null, cdcsoBanned: isBannedFDC,
        inStock, allergyConflict: isAllergyConflict, duplicateTherapy: isDuplicate,
        form: drug?.form || "tablet", company: drug?.company || null,
    ...
    return NextResponse.json({
      suggestions,
      guidelines: { firstLine: guidelines.firstLine.length, secondLine: guidelines.secondLine.length, notes: guidelines.notes, diagnosis: key },
