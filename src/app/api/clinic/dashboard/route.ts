  ...
  try {
    ...
    const [
      ...
      (async () => {
        ...
        const bills = await db.clinicInvoice.findMany({
          where: { createdAt: { gte: sevenAgo }, status: "paid" },
    ...
    return NextResponse.json({
      clinic: {
        ...
        ownerName: ctx.clinic.ownerName,
        city: ctx.clinic.city,
        bookingSlug: ctx.clinic.bookingSlug,
      },
      kpis: {
    ...
    log.error("clinic", "dashboard_failed", {
      err: err instanceof Error ? err.message : String(err),
    });
    return NextResponse.json(
      {
        error: "clinic_dashboard_failed",
        detail: "The clinic dashboard could not be loaded. Please retry.",
      },
      { status: 500 },
    );
  }
