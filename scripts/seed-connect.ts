...
async function main() {
  ...
  const kyhDoctors = [
    {
      id: "kyh-doctor-general-01",
      name: "Dr. Aanya Kapoor",
      specialty: "General Physician",
      phone: "+91 98300 10001",
    },
  ];
  ...
  for (let i = 0; i < Math.min(doctors.length, patients.length); i++) {
    ...
    const conn = await db.connectConnection.create({
      data: {
        ...
        source: "hospital",
        lastConsultDate: new Date(Date.now() - i * 86400000),
        whatsappSent: true,
    ...
    const msgs = [
      ...
      {
        fromRole: "doctor",
        text: "Take Paracetamol 650mg TID. Rest and drink fluids. If fever persists beyond 3 days, get CBC done.",
      },
    ...
    for (const m of msgs) {
      await db.connectMessage.create({
        data: {
          ...
          fromRole: m.fromRole,
          fromName: m.fromRole === "doctor" ? doc.name : pat.fullName,
          text: m.text,
          read: m.fromRole === "doctor",
        },
      });
      msgCount++;
    }
    ...
    if (i < 2) {
      await db.connectCall.create({
        data: {
          connectionId: conn.id,
          type: i === 0 ? "video" : "voice",
          status: "ended",
          initiatedBy: "patient",
          durationSec: 300 + i * 120,
          callSummary: "Consultation completed",
          startedAt: new Date(Date.now() - (i + 1) * 3600000),
          answeredAt: new Date(Date.now() - (i + 1) * 3600000 + 10000),
          endedAt: new Date(Date.now() - (i + 1) * 3600000 + 300000),
        },
      });
      callCount++;
    }
    ...
    if (i < 2) {
      await db.connectQueue.create({
        data: {
          connectionId: conn.id,
          requestedMode: i === 0 ? "chat" : "video",
          reason: "Follow-up question about medication",
          status: "waiting",
        },
      });
      queueCount++;
  ...
  const kyhConn = await db.connectConnection.create({
    data: {
      doctorId: kyhDoc.id,
      doctorName: kyhDoc.name,
      doctorSpecialty: kyhDoc.specialty,
      patientId: "kyh-patient-session-demo",
      patientName: "KYH Patient",
  ...
  await db.connectMessage.create({
    data: {
      connectionId: kyhConn.id,
      fromRole: "patient",
      text: "Hi, I used the symptom checker and was connected here",
    },
  });
  await db.connectMessage.create({
    data: {
      connectionId: kyhConn.id,
      fromRole: "doctor",
      fromName: kyhDoc.name,
      text: "Welcome! I've reviewed your symptoms. How can I help you today?",
    },
  });
  connCount++;
  msgCount += 2;
  ...
  console.log(
    `   ${connCount} connections, ${msgCount} messages, ${callCount} calls, ${queueCount} queue entries`,
  );
}
