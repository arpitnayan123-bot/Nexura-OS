...
async function main() {
  ...
  await db.nxApiKey.upsert({
    ...
    create: {
      tenantId: tenant.id,
      name: "LIS partner — Metropolis sandbox",
      prefix: demoKeySecret.slice(0, 12),
      keyHash: sha(demoKeySecret),
      scopes: JSON.stringify(["patients.read", "observations.read"]),
      rateLimitPerMin: 120,
    },
      ...
      data: [
        {
          ...
          resource: "patients",
          patientScope: "ward",
          note: "Nurses see patients in their current ward only",
        },
        {
          hospitalId: hid,
          role: "nurse",
          ...
          timeWindows: JSON.stringify([
            { days: [1, 2, 3, 4, 5, 6, 7], from: "06:00", to: "22:00" },
          ]),
          note: "Medication administration only for assigned patients during shift window",
        },
        {
          hospitalId: hid,
          ...
          resource: "patients",
          patientScope: "any",
          timeWindows: JSON.stringify([{ days: [1, 2, 3, 4, 5], from: "00:00", to: "05:00" }]),
          note: "No patient data access for billing at night — least privilege",
        },
        {
          hospitalId: hid,
          role: "nurse",
          ...
          patientScope: "any",
          timeWindows: JSON.stringify([
            { days: [1, 2, 3, 4, 5, 6, 7], from: "20:00", to: "07:30" },
          ]),
          note: "Night shift: ward-wide visibility for safety (handover window)",
        },
      ],
    });
  }
  ...
  const escTypes = [
    {
      alertType: "result.critical",
      levels: [
        { afterMin: 0, notifyRoles: ["nurse", "doctor"] },
        { afterMin: 10, notifyRoles: ["command", "dept_admin"] },
        { afterMin: 30, notifyRoles: ["leadership", "hospital_admin"] },
      ],
    },
    {
      alertType: "discharge.confirmed",
      levels: [
        { afterMin: 0, notifyRoles: ["nurse", "receptionist"] },
        { afterMin: 45, notifyRoles: ["command"] },
      ],
    },
    {
      alertType: "incident.critical",
      levels: [
        { afterMin: 0, notifyRoles: ["command"] },
        { afterMin: 15, notifyRoles: ["hospital_admin", "leadership"] },
      ],
    },
  ];
  for (const p of escTypes) {
    const exists = await db.nxEscalationPolicy.findFirst({
      where: { hospitalId: hid, alertType: p.alertType },
    });
    if (!exists)
      await db.nxEscalationPolicy.create({
        data: { hospitalId: hid, alertType: p.alertType, levelsJson: JSON.stringify(p.levels) },
      });
  }
  ...
  const pathways = [
    {
      ...
      steps: [
        {
          id: "lactate",
          title: "Draw lactate + blood cultures",
          withinMin: 60,
          critical: true,
          ownerRole: "nurse",
        },
        {
          id: "antibiotics",
          title: "Broad-spectrum antibiotics administered",
          withinMin: 60,
          critical: true,
          requires: ["lactate"],
          ownerRole: "nurse",
        ...
        {
          id: "fluids",
          title: "30ml/kg crystalloid bolus",
          withinMin: 180,
          critical: true,
          requires: ["lactate"],
          ownerRole: "nurse",
        ...
        {
          id: "reassess",
          title: "Reassess perfusion (BP/urine/lactate)",
          withinMin: 240,
          requires: ["fluids"],
          ownerRole: "doctor",
          detail: "Escalate to ICU if MAP < 65",
        },
      ],
    },
    {
      code: "discharge-hf",
      name: "Heart failure discharge pathway",
      specialty: "cardiology",
      steps: [
        {
          id: "vitals_stable",
          title: "24h stable vitals confirmed",
          withinMin: 1440,
          critical: true,
          ownerRole: "nurse",
        },
        {
          ...
          withinMin: 2880,
          requires: ["vitals_stable"],
          ownerRole: "doctor",
        },
        {
          id: "education",
          title: "Fluid/diet/weighing education given",
          withinMin: 2880,
          requires: ["vitals_stable"],
          ownerRole: "nurse",
        },
        {
          id: "followup",
          title: "Follow-up OPD booked within 7 days",
          withinMin: 4320,
          requires: ["med_rec"],
    ...
    },
    {
      ...
      steps: [
        {
          id: "ecg",
          title: "12-lead ECG within 10 minutes",
          withinMin: 10,
          critical: true,
          ownerRole: "nurse",
        },
        ...
        {
          id: "review",
          title: "Doctor review of ECG + troponin",
          withinMin: 45,
          requires: ["trop"],
          ownerRole: "doctor",
        },
        {
          id: "decision",
          title: "Admit / rule-out decision documented",
          withinMin: 120,
          requires: ["review"],
          ownerRole: "doctor",
        },
  ...
  if (whExists === 0) {
    await db.nxWebhookEndpoint.create({
      data: {
        hospitalId: hid,
        name: "Metropolis LIS gateway",
        url: "https://hooks.metropolis-sandbox.example/nexura",
        secret: `whsec_${sha(hid).slice(0, 24)}`,
        events: JSON.stringify(["result.critical", "discharge.confirmed"]),
        active: false,
      },
    });
  }
      ...
      data: [
        {
          hospitalId: hid,
          patientId: patients[0].id,
          accession: "ACC-2026-0417",
          studyUid: "1.2.410.200010.2026.9.1.1123",
          modality: "CXR",
          bodyPart: "CHEST",
          seriesCount: 2,
          status: "reported",
        ...
        },
        {
          ...
          accession: "ACC-2026-0418",
          studyUid: "1.2.410.200010.2026.9.1.1124",
          modality: "CT",
          bodyPart: "ABDOMEN",
          seriesCount: 48,
          status: "reporting",
  ...
  if (genExists === 0 && patients[2]) {
    await db.nxGenomicProfile.create({
      data: {
        ...
        vaultRef: `vault://genomics/${sha(patients[2].id).slice(0, 16)}`,
        variantsJson: JSON.stringify([
          {
            gene: "CYP2C19",
            variant: "*2",
            zygosity: "heterozygous",
            classpath: "pharmacogenomics",
          },
      ...
      data: [
        {
          ...
          value: 88,
          unit: "bpm",
          capturedAt: new Date(now - 2 * 3600_000),
        },
        {
          deviceId: dev.id,
          metric: "hrv",
          value: 28,
          unit: "ms",
          capturedAt: new Date(now - 3 * 3600_000),
        },
        {
          deviceId: dev.id,
          metric: "spo2",
          value: 94,
          unit: "%",
          capturedAt: new Date(now - 4 * 3600_000),
        },
        {
          deviceId: dev.id,
          ...
          value: 302,
          unit: "min",
          capturedAt: new Date(now - 12 * 3600_000),
        },
      ],
    });
  }
  ...
  if (contractExists === 0 && patients[4]) {
    await db.nxInsuranceContract.create({
      data: {
        ...
        insurer: "Star Health",
        policyNo: "SH-2026-77123",
        milestonesJson: JSON.stringify([
          {
            id: "admission",
            title: "Admission registered",
            amount: 20000,
            condition: "admission.created",
            met: true,
            metAt: new Date(Date.now() - 3 * 86400_000),
          },
          {
            id: "surgery",
            title: "Surgery completed",
            amount: 90000,
            condition: "ot.completed",
            met: false,
            metAt: null,
          },
          {
        ...
        historyJson: JSON.stringify([
          {
            ...
            state: "active",
            actor: "seed",
            note: "Contract activated on admission",
          },
        ]),
      },
    });
  ...
  const simDefaults = [
    {
      code: "anaphylaxis-ward",
      title: "Ward anaphylaxis: first 10 minutes",
      audience: "clinician",
      difficulty: "intermediate",
      tree: {
        id: "start",
        question: "A post-op patient develops urticaria and stridor. What first?",
        options: [
          {
            label: "Call for help + IM adrenaline 0.5mg",
            correct: true,
            next: {
              id: "q2",
              question: "Second action?",
              options: [
                { label: "IV fluids + airway monitoring", correct: true },
                { label: "Oral antihistamine only", correct: false },
              ],
            },
          },
    ...
    {
      code: "dengue-triage",
      title: "Dengue fever triage (OPD)",
      audience: "student",
      difficulty: "beginner",
      tree: {
        id: "start",
        question: "Fever 4 days, platelet 60k, tourniquet positive. Next step?",
        options: [
          {
            label: "Admit for IV fluids + daily CBC",
            correct: true,
            next: {
              id: "q2",
              question: "Warning signs to educate on?",
              options: [
                { label: "Abdominal pain, bleeding, restlessness", correct: true },
                { label: "Rash persistence", correct: false },
              ],
            },
          },
  ...
  for (const s of simDefaults) {
    await db.nxSimulationScenario.upsert({
      ...
      create: {
        ...
        treeJson: JSON.stringify(s.tree),
      },
  ...
  if (vcExists === 0 && patients[6]) {
    const claims = {
      id: `did:nexura:${patients[6].uhid.toLowerCase()}`,
      name: patients[6].fullName,
      uhid: patients[6].uhid,
      bloodGroup: patients[6].bloodGroup,
      allergies: patients[6].allergy,
    };
    await db.nxVerifiableCredential.create({
      data: {
        ...
        issuerId: `did:nexura:hospital:${hid}`,
        signature: createHmac("sha256", process.env.JWT_SECRET || "nexura-dev")
  ...
  const templates = [
    {
      code: "ngo-rural",
      name: "NGO Rural Health Center",
      kind: "ngo-rural",
      config: {
        modules: ["patients", "tasks", "labs", "pharmacy", "schedule"],
        locales: ["hi", "mr"],
        offlineMode: true,
        settings: { retentionProfile: "strict" },
      },
    },
    {
      code: "tier2-nursing",
      name: "Tier-2 Nursing Home (30 beds)",
      kind: "nursing-home",
      config: {
        modules: ["patients", "tasks", "beds", "schedule", "pharmacy", "billing"],
        locales: ["hi", "gu", "ta"],
        offlineMode: true,
      },
    },
    {
      ...
      kind: "clinic",
      config: { modules: ["patients", "schedule"], locales: ["en"], offlineMode: false },
    },
    {
      code: "multi-specialty",
      name: "200-bed Multi-specialty",
      kind: "multispecialty",
      config: {
        modules: [
          "command-center",
  ...
  const plugins = [
    {
      code: "glucometer-bridge",
      name: "Ward Glucometer Bridge",
      version: "1.0.0",
      vendor: "Nexura Labs",
      permissions: ["patients.read", "labs.result.enter"],
      entry: { kind: "panel", route: "plugins/glucometer", slots: ["patient_record"] },
    },
    {
      code: "northstar-triage",
      name: "NorthStar Pediatric Triage",
      version: "0.9.2",
      vendor: "NorthStar Health",
      permissions: ["patients.read"],
      entry: { kind: "panel", route: "plugins/northstar", slots: ["ed_board"] },
    },
  ];
  for (const p of plugins) {
    await db.nxPlugin.upsert({
      ...
      create: {
        ...
        version: p.version,
        vendor: p.vendor,
        permissionsJson: JSON.stringify(p.permissions),
        entryJson: JSON.stringify(p.entry),
        enabled: p.code === "glucometer-bridge",
      },
      update: {},
    });
  }
  ...
  const counts = {
    tenants: await db.nxTenant.count(),
    apiKeys: await db.nxApiKey.count(),
    abac: await db.nxAbacPolicy.count(),
    escalations: await db.nxEscalationPolicy.count(),
    pathwayDefs: await db.nxPathwayDef.count(),
    pathwayRuns: await db.nxPathwayRun.count(),
    aiThresholds: await db.nxAiThreshold.count(),
    dicom: await db.nxDicomStudy.count(),
    genomics: await db.nxGenomicProfile.count(),
    wearables: await db.nxWearableDevice.count(),
    contracts: await db.nxInsuranceContract.count(),
    sims: await db.nxSimulationScenario.count(),
    vcs: await db.nxVerifiableCredential.count(),
    templates: await db.nxHospitalTemplate.count(),
    plugins: await db.nxPlugin.count(),
    teleconsults: await db.nxTeleConsult.count(),
  };
