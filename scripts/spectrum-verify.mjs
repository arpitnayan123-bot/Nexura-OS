...
const heroBtn = await page.locator("button.mat-btn--textured", { hasText: "Start your health scan" }).count();
const connectNeon = await page.locator("a[href='/connect'].group").first().evaluate((el) => getComputedStyle(el).boxShadow).catch(() => "none");
console.log("HERO_CTA:", heroBtn > 0 ? "OK" : "FAIL", "| CONNECT_CHIP_GLOW:", connectNeon !== "none" ? "OK" : "FAIL");
...
console.log(`OSGLANCE_CARDS: clay=${glanceClay} neon=${glanceNeon} aurora=${glanceAurora}`, glanceClay > 0 && glanceNeon > 0 && glanceAurora > 0 ? "OK" : "FAIL");
...
console.log(`DIY_STRIP: card=${diyCount} cta=${diyCta}`, diyCount > 0 && diyCta > 0 ? "OK" : "FAIL");
...
console.log("CRISIS_NEON_ANIM:", crisisAnim.includes("breathe") || crisisAnim.includes("mat") ? `OK (${crisisAnim})` : `FAIL (${crisisAnim})`);
