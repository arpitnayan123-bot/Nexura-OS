...
async function main() {
  ...
  const doctors = await Promise.all([
    ...
    db.hospitalDoctor.create({
      data: {
        ...
        languagesSpoken: '["Hindi","English","Malayalam"]',
      },
    }),
    db.hospitalDoctor.create({
      data: {
  ...
  const wards = await Promise.all([
    ...
    db.hospitalWard.create({
      data: { hospitalId: hospital.id, name: "ICU", wardType: "icu", totalBeds: 8, floorNumber: 2 },
    }),
    db.hospitalWard.create({
      data: { hospitalId: hospital.id, name: "HDU", wardType: "hdu", totalBeds: 6, floorNumber: 2 },
    }),
    db.hospitalWard.create({
      data: {
        hospitalId: hospital.id,
  ...
  for (const ward of wards) {
    for (let i = 1; i <= ward.totalBeds; i++) {
      const status =
        i <= Math.floor(ward.totalBeds * 0.65) ? "occupied" : bedStatuses[i % bedStatuses.length];
      beds.push({
        hospitalId: hospital.id,
        wardId: ward.id,
        ...
        status,
        lastCleanedAt:
          status === "cleaning" ? new Date(Date.now() - 3600000) : new Date(Date.now() - 86400000),
      });
    }
  }
  ...
  const patientData = [
    ...
    },
    {
      ...
      bloodGroup: "B+",
      lang: "Punjabi",
      phone: "+91 98201 10011",
      district: "Delhi",
      insurance: "Heritage Health",
      policy: "HH-2024-011",
      pmjay: false,
      chronic: "COPD",
    },
  ...
  for (let i = 0; i < 15; i++) {
    ...
    const status =
      i < 4 ? "completed" : i < 7 ? "in_consultation" : i < 10 ? "waiting" : "scheduled";
  ...
  for (let i = 0; i < 8; i++) {
    ...
    await db.hospitalBed.update({
      where: { id: bed.id },
      data: { currentPatientUhid: patient.uhid },
    });
    admissions.push(adm);
  }
  ...
  const meds = [
    ...
    },
    {
      ...
      salt: "Calcium Carbonate 1250mg + Vitamin D3 250IU",
      company: "Torque",
      schedule: "otc",
      nppa: false,
      stock: 150,
    ...
    },
    {
      ...
      generic: "Mecobalamin+Vitamins",
      salt: "Mecobalamin 1500mcg + B1+B6+B12",
      company: "Merck",
      schedule: "otc",
      nppa: false,
      stock: 60,
    ...
    },
    {
      ...
      salt: "Etofylline 77mg + Theophylline 23mg",
      company: "Zydus",
      schedule: "schedule_h",
      nppa: false,
      stock: 40,
  ...
  const otSurgeries = [
    {
      ...
      patient: 2,
      status: "completed",
      start: new Date(Date.now() - 4 * 3600000),
  ...
  console.log(
    `   ${appts.length} appointments, ${vitalsCount} vitals, ${notesCount} clinical notes`,
  );
