...
if (!pgDump) { console.error("pg_dump not found (PATH or /usr/lib/postgresql/*)"); process.exit(1); }
...
execFileSync(pgDump, ["--no-password", "-Fc", "-f", out, DATABASE_URL], { stdio: ["ignore", "ignore", "inherit"] });
const size = statSync(out).size;
if (size < 1024) { console.error(`backup suspiciously small (${size}B) — failing`); process.exit(1); }
