...
4. **Share-capability hardening** — consider binding grants to the assessment chosen
   at creation (the current GET serves the subject's _latest_ assessment at view
...
9. **LLM provider review** — if a real provider replaces the deterministic
   ...
   audits with `isSafeRephrase` enforcement _before_ render, and a re-review of the
