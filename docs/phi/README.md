...
_screening summary_ — risk signals, possible patterns, protective/contributing factors,
...
- **Not probabilistic.** v0.1 emits confidence _categories_ (`LOW_CONFIDENCE`,
...
- **Not a replacement for professional care.** The disclaimer is stamped on every
  assessment: _"Nexura Predictive Health Intelligence helps users identify health-risk
  signals and preventive opportunities. It does not diagnose disease or replace
  professional medical care."_
...
| 4   | Data-quality assessment                | Completeness 0–100 + warnings; _missing is never normal_                                                |
...
- **`PHI_ALLOW_UNREVIEWED_RULES=1` semantics** (`triage/ruleset.ts` →
  ...
  `throw()`. Setting the env var is therefore a _dev convenience_, never a production
  path.
