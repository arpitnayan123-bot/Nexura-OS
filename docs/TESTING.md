...
|---|---|---|
| Unit (Vitest) | `bun run test` | 39 tests: permission-matrix invariants (least privilege, finance/clinical separation, break-glass scope), RFC-6238 TOTP (drift/encoding/otpauth), pagination edge cases, CSV escaping, rate limiting, event-bus tenant isolation/audience/ordering |
| API smoke (bash) | `bun run test:api` (dev server up) | 29 end-to-end checks: both auth paths, lockout, session, patient privacy scoping (403), task lifecycle + illegal-transition guard, note immutability (sign→edit 423→addendum), RBAC denials per role, billing separation, idempotent payment replay, SSE hello, validation errors, security headers |
